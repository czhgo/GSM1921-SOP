...
> **文档类型**：技术规划 | **最后更新**：2026-05-06 | **关联文件**：`docs/src/components/calendar.js`、`docs/src/components/inspector.js`、`docs/src/core/state.js`
> **⚠️ 架构状态说明**：本文档记录日历功能的前瞻性规划蓝图。关于日历视图范围限定（仅对参与者/组织者/书记/组长展示，条条支委不展示日历）的已落地规则，请参阅 [SOP_WEB.md §B.4](./SOP_WEB.md#b4-日历视图范围限定)。
...
当前日历实现位于 [calendar.js](../../../docs/src/components/calendar.js)，`renderCalendarByActivities()` 函数硬编码为**月视图表格布局**：
...
```javascript
// 在 core/state.js 的 appState 中新增
...
- [calendar.js L88-L92](../../../docs/src/components/calendar.js#L88-L92)：参与者视图使用 `truncate` CSS 类（`overflow: hidden; white-space: nowrap; text-overflow: ellipsis`）实现单行截断
- [styles.css L743-L757](../../../docs/src/styles.css#L743-L757)：`.cal-task-tag` 同样使用 `white-space: nowrap; text-overflow: ellipsis` 单行截断
...
domain.js (数据定义 + mockDB 内存数据库)
    ↓
services/mock.js (CRUD 操作 + localStorage 持久化)
    ↓
services/runtime.js (运行时插槽，USE_MOCK=true)
    ↓
core/state.js (appState 全局状态 + setState 触发渲染)
    ↓
entries/main-entry.js (renderUI 统一渲染入口)
    ↓
components/calendar.js / components/inspector.js / events.js (视图模块)
```
...
CLAUDE.md 中已规划 Phase 2（Supabase Backend），`services/runtime.js` 的 `USE_MOCK` 开关已为此预留。迁移路径清晰：
...
当前 `Activity` 和 `Task` 的字段定义在 [domain.js](../../../docs/src/core/domain.js) 中已有完善的 JSDoc 注释。以下是对日历功能最有影响的字段分析及扩展建议：
...
当前点击日历日期格的行为链（[calendar.js L137-L143](../../../docs/src/components/calendar.js#L137-L143)）：
...
**重要说明**：当前项目定位为党支部内部 SOP 引擎，活动数据量级在可预见的未来（1-2 年内）不会超过 500 条活动。虚拟滚动目前为**远期优化项**，不建议在 Phase 1/2 中优先实现。CLAUDE.md 中 Phase 3（Workflow Engine）完成后再评估是否需要。
...
4. 所有交互元素必须有可见的 `:focus-visible` 样式（当前已有全局 focus-visible 规则在 [styles.css L798-L804](../../../docs/src/styles.css#L798-L804)）

### 5.4 打印/导出兼容性

当前 [styles.css L1178-L1198](../../../docs/src/styles.css#L1178-L1198) 已包含基础打印样式，隐藏了 header 和 sidebar。
...
| 优先级 | 功能模块 | 依赖 | 预估工作量 | 关联 CLAUDE.md |
|--------|---------|------|-----------|-------------|
...
| 文件 | 关键函数/区域 | 说明 |
|------|-------------|------|
| [docs/src/components/calendar.js](../../../docs/src/components/calendar.js#L15-L144) | `renderCalendarByActivities()` | 月视图日历渲染核心 |
| [docs/src/components/calendar.js](../../../docs/src/components/calendar.js#L152-L197) | `populateMonthSelector()` | 月份选择器填充 |
| [docs/src/components/inspector.js](../../../docs/src/components/inspector.js#L16-L26) | `filterTasksByManagementRole()` | 管理角色任务过滤器 |
| [docs/src/components/inspector.js](../../../docs/src/components/inspector.js#L31-L49) | `renderInspectorFromState()` | 检查器状态路由分发 |
| [docs/src/components/inspector.js](../../../docs/src/components/inspector.js#L87-L174) | `renderInspectorList()` | 活动列表视图渲染 |
| [docs/src/components/inspector.js](../../../docs/src/components/inspector.js#L179-L336) | `renderInspectorDetail()` | 活动详情视图渲染 |
| [docs/src/core/state.js](../../../docs/src/core/state.js#L66-L86) | `appState` 初始定义 | 全局状态结构 |
| [docs/src/core/state.js](../../../docs/src/core/state.js#L101-L112) | `setState()` | Immutable 状态更新 |
| [docs/src/core/domain.js](../../../docs/src/core/domain.js#L11-L60) | JSDoc typedef | 数据结构完整定义 |
| [docs/src/services/mock.js](../../../docs/src/services/mock.js#L19-L72) | `SANDBOX_MODE` + `saveDB()`/`loadDB()` | 持久化引擎 |
| [docs/src/services/runtime.js](../../../docs/src/services/runtime.js#L14) | `USE_MOCK` 开关 | 后端切换点 |
| [docs/src/styles.css](../../../docs/src/styles.css#L696-L757) | `.cal-cell-*` 类 | 日历单元格样式 |
| [docs/src/entries/main-entry.js](../../../docs/src/entries/main-entry.js#L20-L153) | `renderUI()` | 统一渲染入口 |
| [CLAUDE.md](../../../CLAUDE.md#L66-L86) | Phase 2/3 | Supabase + Workflow Engine 规划 |
