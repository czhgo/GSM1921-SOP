...
import { showToast, flashHighlight } from '../core/utils.js?v=20260912j';
import { dutyCardHtml } from './workforce-duty-card.js?v=20260912j';
import { TodoStore, seedTodos, TodoStatus } from '../services/todo.js?v=20260912j';
import { IssueStore } from '../services/issues.js?v=20260912j';
import { AuthStore } from '../services/auth.js?v=20260912j';
import { solidAccentStyle, dotDarkVars } from '../core/constants.js?v=20260912j';
import { loadActivities } from '../services/activity.js?v=20260912j';
import { loadActiveAttendanceRecords } from '../services/attendance.js?v=20260912j';
import { loadInspectionRecords, getOverdueRecords } from '../services/inspection.js?v=20260912j';
// S3③（2026-09-12）：补课口径统一——概况补课缺口与「补课制度」表同源（services/makeup.js）
import { loadMakeupTasks } from '../services/makeup.js?v=20260912j';
...
async function _renderOverviewDetail(container, detail, accent, onBack) {
  ...
  if (detail.kind === 'activity') {
    const { renderActivityView } = await import('./activity-view.js?v=20260912j');
    renderActivityView(host, { highlightId: detail.id, accent });
  } else {
    const { renderTaskforceView } = await import('./taskforce-view.js?v=20260912j');
