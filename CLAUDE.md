...
| ID | 事项 | 引用流程 | 状态 |
| --- | --- | --- | --- |
| 已闭环（2026-09-09~10） | E-1~E-5、S-1~S-4、R-1/R-2、R-3/R-4/R-5、W-1、**C-1 全站卡片对比度收口**——详见 EXECUTION_LOG **T-2026-09-077** 与 commits（`22120e64`/`60dfa271`/`5138e638`/`dac678e4`/`9e7a088f`/对比度批）：活动写入改造（逐人结果/默认精简/UI 规范）、卡片普查合并（取消 KPI 五连卡·不设出勤率 KPI、补 5 处入口）、催办与兜底白名单+副书同权、归属时序修复+未登录中性占位、门面通用化、只读端对齐、对比度 AA 0 违规 | 附录⑦ 判据 + H60.6/60.7 | ✅ 已清 |
| R-6 | **遗留（功能）**：通知弱定位（无 targetUrl 时落列表）、代归档未深链到宣传归档页、代填复盘无承载表单、催办通知文案待书记定稿、党委「进入支部」在 API 会话按设计拒绝 | H60.7 | 待办 |
| R-7 | **遗留（对比度）**：`styles.css` 内 `.ov-sub-tab-active` / `.tint-pill` 仍用原色 `var(--app-accent/--tint)`，浅色主题下可能欠对比（styles.css 属禁改，待书记裁决是否特批）；深色模式未纳入机测（仅 `--acc-text-dark` 逻辑保障） | 禁改清单 + COMPONENT_SPEC | 待办 |
| P-15 | **功能实现评议下一轮候选（待书记逐条判定）**：①上报党委三节点通知缺失（高）②纪检超期提醒仅打卡化未触达（中）③组织专班看板置首口径 ④培养考察期满/公示期/预备期满系统提示缺失 ⑤branchDocs 未按 branchId 隔离 ⑥CF「组织委员可改模块设置」与 §9h 冲突 ⑦宣传看板置首口径 ⑧党委台账缺书记任期状态 | REVIEW_QUEUE 附录⑧ + H60 | 待办 |
