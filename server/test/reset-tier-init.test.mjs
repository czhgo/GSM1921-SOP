...
} = await import('../../docs/src/services/init-reset.js?v=20260917b');
// 接线冒烟（⑤）：走 services/mock.js loadDB 可改 reset 链（数据源默认 mock）
const { loadDB, saveDB } = await import('../../docs/src/services/mock.js?v=20260917b');
// C2 修复（⑥）：浏览器加载链多轮 loadDB 稳态断言（内存 mockDB 与浏览器同源单例）
const { mockDB } = await import('../../docs/src/core/domain.js?v=20260917b');
...
test('C2 修复：loadActivities 读兜底——init 态空态返回 []（不回退演示种子）；无哨兵保持原回退', async () => {
  const { loadActivities } = await import('../../docs/src/services/activity.js?v=20260917b');
  const { ls } = stubGlobals({ search: '', href: 'http://127.0.0.1:3000/index.html', store: { page_pref: 'x' } });
