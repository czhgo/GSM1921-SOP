...
> 来源：CLAUDE.md H2.2.D — 差异化视图设计规范之子记录关联结构。
...
| **disc-commissioner** (纪检委员) | 是 | 是 | 否 (不设专班) | 是（确认后录入考勤总表） | 是 | 是 | 是（确认后录入考察总表） | 是（复盘确认后录入后台） | 否 |
| **leader** (党小组组长) | 是 | 是 | 否 | 否 | 是 | 否 | 否 | 否 | 否 |
| **organizer** (组织者) | 是 | 是 | 否 | 否 | 是 | 否 | 否 | 否 | 否 |
| **deep** (深度参与者) | 是 | 否 | 否 | 否 | 是 | 否 | 否 | 否 | 否 |
| **participant** (默认参与者) | 是 | 否 | 否 | 否 | 是 | 否 | 否 | 否 | 否 |

**宣传委员活动视图同步规则**：宣传委员不可直接创建活动（须走写入链路：支委提出需求 → 组织委员协调 → 党小组组长承办写入），但任何活动创建后应自动出现在宣传委员的视图中，确保宣传委员版面及时同步，不因无创建权限而遗漏活动信息。
