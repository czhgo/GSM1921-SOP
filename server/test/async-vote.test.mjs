...
test('AV5 线上党员大会：支书发起 → 预备党员只读/403 → 正式党员 UI 表决 → 硬校验拦截 → 补足后通过', async () => {
  ...
  await prepPage.evaluate(async () => {
    const { persist } = await import('/src/core/data-adapter.js?v=20260921k');
  ...
  const p5Row = await memberPage.evaluate(async ({ activityId, ag1 }) => {
    ...
    const { enableApiMode } = await import('/src/services/runtime.js?v=20260921k');
    enableApiMode(token);
  ...
  try {
    ...
    await secPage.evaluate(async ({ id }) => {
      const { setState } = await import('/src/core/state.js?v=20260921k');
      const { mockDB } = await import('/src/core/domain.js?v=20260921k');
