...
import { dutyCardHtml } from './workforce-duty-card.js?v=20260921a';
import { TodoStore, seedTodos, TodoStatus } from '../services/todo.js?v=20260921a';
import { IssueStore } from '../services/issues.js?v=20260921a';
import { AuthStore } from '../services/auth.js?v=20260921a';
import { solidAccentStyle, dotDarkVars, isActivityEnded, isActivityArchived } from '../core/constants.js?v=20260921a';
...
import { loadInspectionRecords, getOverdueRecords } from '../services/inspection.js?v=20260921a';
...
async function _renderOverviewDetail(container, detail, accent, onBack) {
  ...
  if (detail.kind === 'activity') {
    const { renderActivityView } = await import('./activity-view.js?v=20260921a');
    renderActivityView(host, { highlightId: detail.id, accent });
  } else {
    const { renderTaskforceView } = await import('./taskforce-view.js?v=20260921a');
