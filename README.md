...
同一份数据，不同角色看到不同的视角——党建管组织活动，党务管人员发展，两者都是"管理事，服务人"。
...
- **专班** — 跨小组跨职能抽调人手集中推进某项工作、不限时间不限地点、推动支部长期建设的组织形式；专班是活动之外考察积极分子的载体，赋权是运行支撑机制，工作量记录是运行保障机制；发起是提出需求，招募是统筹执行，两者分离
- **条块二元** — "条"与"块"是理解权责关系的两把尺子：从职能视角看，三委员按专业职能分工（条），专业线穿透所有党小组；从单元视角看，党小组组长按小组划分（块），小组线覆盖本组成员。谁做什么，由岗位职责定义决定，不由条块推出；书记是条块间的协调节点
- **扁平化设计** — 组织者和深度参与者之间无上下级，只是分工不同；组织者核心职能是分工记录，深度参与者承担具体分工
- **差异化视图** — 同一数据源，不同视角展示；视图按需取用，不为做而做——没有应用场景就不做视图决策

这些理论不是抽象的口号，而是每一条制度设计背后的理由。理解了它们，才能理解为什么系统是这样组织的，也才能在场景变化时判断原则在什么条件下成立。

→ 完整的理论阐述见 [书记重要论断汇编](content/01_strategy/SECRETARY_PRONOUNCEMENTS.md)（项目顶级战略文档，23 条论断，其中 P-045/P-047 为元命题引子）和 ["管理事、服务人"战略路线](content/01_strategy/DEVELOPMENT_PATH.md)
...
| **后台（服务）** | `docs/src/services/`（19 个 service JS）+ `docs/src/mock/`（10 个） | 数据 CRUD、权限计算、持久化 |
