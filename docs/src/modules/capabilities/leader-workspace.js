...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', render: (ctx) => import('../../entries/tabs/leader/todo-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    { id: 'overview', label: '工作概况', groupLabel: '工作台', render: (ctx) => import('../../entries/tabs/leader/overview-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    { id: 'write', label: '活动管理', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/leader/write-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    { id: 'attendance', label: '考勤上传', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/leader/attendance-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    { id: 'inspection', label: '考察上传', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/leader/inspection-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    // IA-C2 收敛（2026-09-06）：复盘提交入口统一归组织者/深度参与者（成员端「我的复盘」），本 tab 只读展示本组复盘状态
    { id: 'review', label: '复盘状态', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/leader/review-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    // 组员进展（书记 2026-08-10 裁定：全员可见性矩阵落地——组长看本组组员，P-011 知情边界看≠做）
    { id: 'members', label: '组员进展', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/leader/members-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
    ...
    { id: 'my-dispatch', label: '我的处置', groupLabel: '反馈', render: (ctx) => import('../../entries/tabs/leader/my-dispatch-tab.js?v=20260907b').then(m => m.renderContent(ctx)) },
  ],
});
