...
import { showToast, flashHighlight } from '../core/utils.js?v=20260827c';
import { TodoStore, seedTodos, TodoStatus } from '../services/todo.js?v=20260827c';
import { IssueStore, REPORT_CATEGORIES } from '../services/issues.js?v=20260827c';
...
import { loadInspectionRecords, getOverdueRecords } from '../services/inspection.js?v=20260827c';
...
import { PEOPLE } from '../mock/people.js?v=20260827c';
...
async function _renderOverviewDetail(container, detail, accent, onBack) {
  ...
  if (detail.kind === 'activity') {
    const { renderActivityView } = await import('./activity-view.js?v=20260827c');
    renderActivityView(host, { highlightId: detail.id, accent });
  } else {
    const { renderTaskforceView } = await import('./taskforce-view.js?v=20260827c');
