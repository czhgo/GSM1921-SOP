...
//     这是一个很全局性的需要思考和改进的工程！」→ 支书裁定：**宽表默认**（long form 降为明细/导出下钻），
...
function _stateOf(key) {
  if (!_states.has(key)) _states.set(key, { mode: 'byPerson', showAll: false });
...
export function renderRelationMatrix(host, cfg) {
  ...
  const cellOf = (pid, iid) => {
    ...
    return html ? `<td class="text-center">${html}</td>` : '<td class="text-center text-gray-400">—</td>';
  };
  ...
  const itemRowHead = (it) => `<td class="sticky left-0 bg-white">
      <div class="text-xs font-medium text-gray-800 max-w-[180px] truncate" title="${esc(it.title)}">${esc(it.title)}</div>
  ...
  const headRow = byPerson
    ...
    : `<th class="sticky left-0 align-top">${esc(cfg.itemLabel || '项目')}</th>${persons.map(p => `<th class="text-center whitespace-nowrap">${esc(p.name)}</th>`).join('')}`;
  ...
  const body = byPerson
    ...
    : cols.map(it => `<tr>
        ${itemRowHead(it)}
        ${persons.map(p => cellOf(p.id, it.id)).join('')}
      </tr>`).join('');

  const span = 1 + (byPerson ? cols.length : persons.length);
