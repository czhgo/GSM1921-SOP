...
import { SEED_SIGNUPS } from '../mock/seed.js?v=20260914s';
...
import { TodoStore, TodoSourceType, TodoActionType, TodoCategory, TodoStatus } from './todo.js?v=20260914s';
...
async function _writeSource(sourceType, sourceId, personId, role) {
  if (sourceType === 'activity') {
    ...
    if (role === 'organizer' || role === 'deep') {
      ...
      const { updateActivity } = await import('./mock.js?v=20260914s');
    ...
    if (s.status === SignupStatus.APPROVED) {
      if (s.sourceType === 'activity') {
        ...
        if (act && Array.isArray(act.assignments) && act.assignments.some(x => x.personId === personId)) {
          ...
          import('./mock.js?v=20260914s').then(({ updateActivity }) => {
