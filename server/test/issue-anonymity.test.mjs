...
import { hashSubmitterToken } from '../../docs/src/core/constants.js?v=20260913f';
...
test('mock 形态：匿名提交后存储/支书端界面均无提交人；实名可见；tokenHash 前后端同源', async () => {
  ...
  try {
    ...
    const out = await page.evaluate(async () => {
      const { IssueStore } = await import('/src/services/issues.js?v=20260913f');
      const { PersonStore } = await import('/src/services/person.js?v=20260913f');
      localStorage.clear();
      ...
      const fb = await import('/src/entries/tabs/secretary/feedback-tab.js?v=20260913f');
      fb.renderContent();
