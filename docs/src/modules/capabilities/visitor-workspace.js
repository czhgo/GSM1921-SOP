...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/visitor/todo-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    // 工作概况（支书 2026-08-10 裁定：全部角色新增——汇报/卡点/在办三区总览，参与者仅自我聚合）
    { id: 'overview', label: '工作概况', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/visitor/overview-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    { id: 'projects', label: '项目分工', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/projects-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    { id: 'activities', label: '活动动态', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/activities-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    { id: 'attendance', label: '考勤概况', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/attendance-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    { id: 'inspection', label: '我的考察', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/inspection-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    // 2026-08-30 思想汇报数字化：参与者系统内提交，算法自动归档
    { id: 'thought-report', label: '思想汇报', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/thought-report-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
    // T-304 C1 组织者承载面：复盘提交归组织者（组织者/深度参与者在自己工作台提交）
    { id: 'review', label: '我的复盘', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/review-tab.js?v=20260914m').then(m => m.renderContent(ctx)) },
  ],
});
