...
date: "2026-09-10"
version: "v22"
milestone: "2026-09-10 续批（同日第二批）：功能实现评议下一轮 8 候选全闭环（通知对象级深链+守卫 / 代归档放行+待归档区 / 代填复盘单一表单 / 催办文案定稿 / 党委下钻放开只读 / 上报三节点通知 / 纪检超期真实触达 / 发展节点提醒 / branchDocs 支部隔离 / 党委台账任期行 / 上级看板口径与 CF 冲突修正）+ R-7 对比度遗留（styles.css 特批 4 行）+ 最小三成本第 6 轮 6 条（发展数据只读化·建活动默认预选·无假日期·外发行内化·党委总览去重·周报周次派生）+ 门面口径统一（示例组织前缀）+ 待办生命周期退出（乙部仅余 R-8 深色残留）；版本串 20260910a / CODE_VERSION 97"
---
...
> **上版**: v21 (2026-09-10)
> **变更来源**: 2026-09-10（同日两批，T-077 追加批次 1~3；见 v22/v21 里程碑行；测试/文件计数一律以实测为准）
