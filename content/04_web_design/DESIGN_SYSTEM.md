...
2. **最小三成本（Min Cost First）**：任务流默认直接展示在工作台，不要求用户额外操作才能看到"我需要做什么"。包含三个维度：①**信息成本**——用户获取"我需要做什么"所需的信息搜寻成本，任务流默认直接展示；②**操作成本**——从"进入工作台"到"看到可执行事项"的点击/操作次数，进入即见，系统主动推送；③**适应学习成本**——新用户理解工作分工、工作任务、信息流运作方式的学习成本，通过系统设计本身自解释。适用于全系统所有角色工作台。
   - **知情权优先（书记裁定 2026-08-08）**：无职责不代表没有知情权。支委/角色即便业务上不负责活动/专班的操作（如纪检无专班职责、组织无活动 tab），仍有权查看其详情。凡首页跳转携带 `activityId`/`taskforceId`/`view=activities` 参数，各角色工作台均须有对应「查看」承载（激活相关 tab 或复用活动/专班查看组件），不得因"无职责"而丢弃参数、让用户信息成本升高。
...
9. **信息密度精确原则（Precision of Information Density）（书记原话 2026-08-10，战略级）**："执行委托与知情监督是对应的，统一的。统筹是最大的管理！"——赋权链（执行委托的操作边界）与按人视图（知情监督的信息边界）对应统一，是同一枚硬币的两面；"一个尽可能降低管理摩擦的方向是任何角色的信息可见范围，应精确等于其职责空间所需的最小充分信息"——**信息可见性 = 职责空间的投影**。信息密度分层：**L0 个人视角**（自己）/ **L1 条线视角**（上级看下级的条线在办：谁在干什么、有无异常）/ **L2 全局视角**（书记看全部条线+块块）。向上越层需职责依据（赋权链），向下缩层是对他人职责空间的保护。**看 ≠ 做**：知情边界是组织内控的第四道防线（[P-011 知情边界](../01_strategy/SECRETARY_PRONOUNCEMENTS.md#p-011-组织内控总论职责分离主动回避书记仲裁与知情边界)）——监督停留在"方向把握"（P-027②），不沉入"细节干预"；内控失效的常见入口是"知情过载"（看到他人操作细节→越过职责边界干预/代劳）。可验证条件：某角色可见信息超出其职责空间所需（可见他人操作细节但无监督关系）→ 违反；按人视图按赋权链投影、不含操作细节 → 通过。设计落点：书记工作台全局概况"按维度/按人"双视图（按人视图=各角色在办任务清单，异常高亮可直达）。
10. **高频零跳转（High-Frequency Zero-Jump）（书记原话 2026-08-10）**：最小三成本的最终验收标准不是"信息是否展示"，而是"**完成一个高频工作，需要操作多少次？信息的展示是否和操作在同一个地方，便于参考？**"。①**答复类置顶待办**：汇报答复是书记最高频动作，待办 tab 顶部内建「待答复」收件箱（问题优先排序、行内填写即发、零跳转），而非藏在全局概况→按人→汇报区多层之下；②**答复回路全角色化**：信息流畅是双向的——其他角色同样有答复机制（组长可答复本组组员，书记仍全局可见，P-011 知情边界不破坏）；③**工作概况三区总览**：全部角色新增「工作概况」tab，汇报（待我行动，信息展示与操作同屏）/卡点（我的超期+条线缺口）/在办（我的聚合+条线态势）三区上下排布、问题优先，让"看到"与"操作"在同一可视区域。可验证条件：① 完成任一高频工作的点击/跳转次数 > 2 → 违反（呼应原则 2 操作成本）；② 某数据与其操作入口不在同一可视区域（需滚动/跳转才能从看到变为操作）→ 违反；③ 某角色存在单向汇报却无答复回路 → 违反；④ 待办列表顶部存在比"答复类"更高频的动作类型 → 违反。孤零零数据行（不属于任何卡片的裸 `div` 字段，数值与结论与卡片冗余）属信息成本冗余，须整合进卡片或直接删除。
11. **工作进度指标设计（Progress Indicator Design）（书记原话 2026-08-10，战略级）**：工作进度的指标设计与显示必须谨慎——**"已完成 N"是无信息量的存量统计**，只可作为归档统计信息存在于归档库；面向"要完成的工作"，进度指标必须指向"进行时与未完成"（待办/待完成/超期/缺口/进行中），并提供信息增量与操作入口。①**只显未完成类**：工作界面（工作台 tab、概况、详情）不得出现"已完成 N/总数"式无差异存量统计；"已完成"仅在归档库/归档统计中出现；②**可下钻**：进度数字/聚合须可点击进入具体任务清单或详情（信息展示与操作同地，呼应原则 10）；③**从"看"到"做"**：概况"在办"区列出具体可点击条目（待办→定位工作台待办 tab 对应任务；活动/专班→下钻只读详情视图），杜绝"只有数字没有入口"。可验证条件：① 工作界面出现"已完成 N/总数"类存量统计（归档库除外）→ 违反；② 进度数字聚合不可点击、无下钻入口 → 违反；③ 概况在办区仅显示聚合数字、无具体条目 → 违反。
12. **工作台集成制（Workspace Integration）（书记原话 2026-08-10）**：角色单页制后，**视图切换（header 工作台切换 / sidebar 切换）已失去作用——每个人就是每个人**；组织者与深度参与者的所有信息均集成在工作台中，任务来了就都在工作台内完成，不再依赖任何"切换到某角色视角"的入口。可验证条件：出现需"切换到某角色/视角"才能完成的操作路径 → 违反；信息展示或操作入口越出工作台边界（依赖别的页面）→ 违反。
13. **文件流外发确认闭环（External Dispatch Confirmation）（书记原话 2026-08-10，战略级）**：任务流/信息流均可通过系统实现，**唯一缺口是"文件流"——文件本体暂靠微信传输**。当任务/材料需微信外发给对方确认时，系统不得静默——发送方须在系统中标记「已通过微信发送给 XX」（外发确认），接收方在系统内「确认收到」，形成可审计闭环（谁 / 何时 / 发给谁 / 何时确认）。**"总是有内控设计的方法的！！"**——内控设计的必然要求就是：外部依赖（微信）发生时，系统内必须有状态记录兜底。面向书记目标（低成本小程序/服务号与微信原生协同），记录模型保持简单可迁移——"标记已发送"演进为"发送到对方微信"，"确认收到"保持同构。可验证条件：① 存在需微信外发文件的工作环节但系统无外发标记/确认入口 → 违反；② 外发记录缺发送方/接收方/时间任一要素 → 违反；③ 接收方确认后记录未闭环（审计时间线不完整）→ 违反。设计落点：宣传材料上传后「文件外发确认」模态（标记已微信外发+选择接收方）；接收方工作台卡点区/异常队列「文件待确认 → 确认收到」。
14. **KPI 行为异化防御（KPI Behavioral Integrity）（书记原话 2026-08-10，战略级）**：**指标设定必须考虑行为激励——"完成率 100%"类指标会诱导凑数提交，目标即异化**（例：复盘率设 100% → 大家随意提交凑数，违背"从最初设定上我们就只是希望大家提交真问题"的初衷）。面向"要完成的工作"，指标应指向工作的真实意图而非形式完成：复盘要的是"真问题"（问题导向）而非"完成率"；承诺要的是"兑现质量"而非"次数"。可验证条件：① 某指标为百分比/计数类"完成率"且目标设为 100%（或接近）→ 须审查是否诱导凑数；② 指标计量对象与工作真实意图（产出真问题/真成果）不一致 → 违反；③ 指标被形式达成但对工作无信息增量 → 违反。设计落点：复盘 KPI 由「复盘完成率 目标100%」改为「复盘问题 真问题导向」（计量活跃活动复盘中提出的真问题数量，复盘表单内建"提出的真问题"填写）。

### 减负空间排查（分批规划，书记 2026-08-10 发起）
...
- **验收标准**：每批产出「排查清单 + 精简决策」，完成一批书记审核一批；排查结论沉淀回本规范或对应 SOP。
...
> **主题色 / 功能色 / 品牌色三色区别（2026-08-14 书记裁决明确定义）**：
>
> | 层 | 名称 | 驱动 | 是否随用户自选 | 用途 |
> |----|------|------|--------------|------|
...
>
> **使用铁律**：四层各司其职禁止混用——状态红≠品牌红≠主题红；同色相不同层按语境区分（§2.8 判例）。身份不再绑定固定色值（2026-08-14 书记裁决），角色识别一律走主题色体系。
...
> **定义（2026-08-08 书记三审修正）**：金色是党建合规色（`--party-gold` 定义于 `:root`，不经角色派生链路），用于**全站统一视觉**场景。**但金不强制充当"主题色"**——主题色语义（tab 激活态、选中项、chips）回归角色识别层（§2.3.2/§2.8 ②），因金是品牌合规色、含义多、受对比度牵制反复变色（半透明偏淡→实色偏艳，均不被接受）。金色仅保留于：强调卡边框、主题党日类别、待办行动按钮、通知徽章、聚焦反馈。
...
| 金黄文字 | `#A16207` | ██ | 金浅底/金底上的文字（2026-08-10 书记裁定"再黄一点"，由 `#B45309` 改，对比度达标） |
| 金浅底 | `rgba(255,215,0,0.12~0.14)` | ██ | 浅金色背景（类型徽章、图标底、待办次要按钮） |
| 金边框 | `rgba(255,215,0,0.35~0.45)` | ██ | 金色边框（强调卡、待办行动按钮、类型徽章） |

> **G3 裁定（2026-08-08 书记实测）**：金色按钮一律用**金浅底** `rgba(255,215,0,0.12)`，**禁用纯亮金实底** `#FFD700`——书记实测「去处理」纯亮金实底「过于艳丽，饱和度太高」，而 tab 的金浅底「因为有透明度，黄色非常舒服」。金浅底与 tab 激活态同源。
> **G3 裁定（边框）**：金色作为主题色时，普通按钮/标签**不加边框**，与其他主题色结构完全一致（书记「别的有他也有，别的没有他也必须没有」）；仅 visitor 品牌金待办行动按钮保留金边框（与详情按钮同页一致）。
...
| `--accent-participant` | `#A16207`（三件套=主题党日胶囊） | ██ | 普通参与者 | 金色系（金黄） | 「金」主题色 = 主题党日胶囊：亮金底 `rgba(255,215,0,0.12)` + 金黄字 `#A16207` + 亮金边框 `rgba(255,215,0,0.35)`（2026-08-08 四审改，2026-08-10 书记裁定"再黄一点"改 #A16207） |
...
- **首页统计卡四相语义色·定稿**（书记 2026-08-10 两次裁定，最终版）：本月活动=蓝 `#3B82F6`（信息/载体）、活跃专班=亮金 `#F59E0B`（琥珀金，**参考工作台 tab 亮色呈现——底色必须是亮的：浅金底+亮金字**，告别深金 `#A16207`）、未读通知=红 `#DC2626`（待处理/告警语义）、我的考勤=绿/琥珀/红（状态语义，考勤三色书记称"很好"保留）——红蓝亮金三原色 + 考勤状态色的平衡组合。
- **主题色应用范围在工作台内可适当放宽**（书记 2026-08-08）：工作台内 tab 激活态与强调色挂靠主题色；侧边栏选择器统一命名「**主题选择**」（浅色/深色模式与颜色选择同一排，2026-08-08 书记更名：称「主题」而非「工作台颜色」）。**每个角色 tab 各有各的主题色渲染**（`bootstrapPage → resolveAccentRole` 支持自选覆盖，2026-08-08 三审强调）。
- **「金」主题色 = 主题党日胶囊**（书记 2026-08-08 四审定稿，原话"金色就应该和主题党日的胶囊颜色一致，那个我觉得很漂亮！"）：`ACCENT_COLORS.participant` 增配 `bg/border` 覆盖字段（亮金底 `rgba(255,215,0,0.12)` + 金黄字 `#A16207` + 亮金边框 `rgba(255,215,0,0.35)`），`getAccentColors` 优先消费覆盖；侧边栏色板「金」色块显示亮金 `#FFD700`。文字色仍守金黄 `#A16207` 保对比度（2026-08-10 书记裁定"再黄一点"，较原 `#B45309` 更黄），金不再因对比度牵制而变色。
...
**语义动作类与颜色对应法则（2026-08-08 书记指令 #1/#4）**：

> 背景：tab 激活态已随主题色，但确认赋权/发布通知/批复等按钮仍是硬编码红，党小组选中态与 radio 仍是原生样式——"改了 tab 其他没跟"。书记裁定：**颜色使用必须有全局对应法则**，用语义类统一收敛，新增代码禁止硬编码色值。
...
**无上限数据分页铁律（2026-08-08 书记指令 #3）**：凡数据量随使用无限增长的列表一律分页。模式：每页 10 条 + 上一页/下一页 + 页码窗口（5 页）+ 「共 N 条 · 第 x/y 页」摘要；搜索/筛选/翻页触发整段重渲染，搜索时页码归 1。已落地：归档库三分页（党建活动/专班/通知）、查询视图（query-view.js）；后续新增无限增长列表必须自带分页。

**执行层仪表盘（2026-08-08 书记指令 #2，管理科学视角重设计书记「全局概况」）**：弃用"各模块数字罗列"，改为执行层决策视图：
...
### 4.15 反馈管理规范（2026-08-08 书记指令 #7 增强）

> 反馈采用 GitHub Issue 风格：草稿审核（书记通过/驳回）→ 公开 → 指派 → 处置 → 待终审 → 关闭。2026-08-08 按书记指令三项增强定稿。
...
> **确立背景（2026-08-09 书记指令）**："每一个色块的设计，请都要考虑【深色模式】。只要是主题色，每一个颜色都必须平行——如果一个在深色模式下是淡色的底深色的字，别的也应该是淡色的底深色的字（总之是一致的，且易用的）。对于功能色，因为不随主题色变化而变化，也要记得去做好深色模式下可见度、可分辨率的评估。"
...
> 深色模式属于"机器因截图原因难以客观判定、需人工实测"的检查项。以下按「人在哪里 → 做什么 → 应该看到什么」逐条核验（书记 2026-08-09 指定人工检查）。
