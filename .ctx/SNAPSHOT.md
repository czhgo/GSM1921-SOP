...
date: "2026-09-12"
version: "v30"
milestone: "2026-09-12 续批（第十批）：dogfood 七身份真机走查（书记/组织/纪检/宣传/组长/普通党员/党委，71 条真实复现问题）→ 去重 12 类系统性缺陷 S1~S12 + 单点，书记裁定全修——通知详情单一取数口（派生通知按 id 现算重建）、?tab= 深链与内容一致、待办/角标/未读同源、今天落点不空、党委只读下钻封死写入口、宣传归档链路（去归档落点+筛选后按钮）、深色徽章色义恢复（styles.css 特批）、Toast 文字、统计可下钻、人数口径同源、提交校验与反馈、危险操作二次确认；单点含反馈 undefined 兜底/首页登录态标题/人才库搜索/通知行键盘可达/草稿跨刷新/周报周次派生/专班截止态/组长上传下拉排除已取消并标已上传/看板卡与活动列表行键盘可达/组长台副标题改「复盘状态查看」；乙部余 R-16/R-17（R-18 当日闭环退出）；批次 12/13 续收口（R-18 明细筛选、窄屏 390px 实测核销、8 项单点真机实证 + 通知行操作常显/专班详情页截止态两处补修、交接「确认接收」实证可用）；版本串 20260912d / CODE_VERSION 114"
---

# System Snapshot — v30

> 当前活跃基线。历史快照见 `.ctx/snapshots/`。
> **生成（v30）**: 2026-09-12 续批 — 第十批（dogfood 七身份体验问题全修，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 11）：**走查** 7 身份串行真机 dogfood（书记/组织委员/纪检委员/宣传委员/党小组组长/普通党员/党委组织员，浅色+深色各一遍，只读全域点击），产出 **71 条真实复现问题** → 去重 **12 类系统性缺陷 S1~S12 + 单点**；书记裁定「**全修 S1~S12 + 单点**」。**S1~S12**：S1 通知详情单一取数口（先 hydrate 再按 id 取；派生通知按 id/sourceId/noticeId 从待办现算重建，确认读取兜底销待办）→ 消除全部入口「通知不存在或已过期」；S2 `?tab=` 深链与内容区一致（消除「记忆 tab 异步回写覆盖 URL tab」竞态）；S3 待办条数与概况/角标/首页未读同源；S4 「今天」落点不空；S5 党委只读下钻同步封死写入口；S6 宣传「去归档」落点 + 归档页筛选后按钮（事件委托）；S7 深色徽章色义恢复（裸 `.badge` 深色规则限定 `:not([class*="badge--"])`，**样式表特批**）；S8 空白 Toast（下发/上报恢复文字）；S9 统计行可下钻；S10 人数口径同源（本支部在册）；S11 提交校验与反馈；S12 危险操作二次确认。**单点**：反馈 `undefined` 兜底+条目键盘可达、首页登录态标题、人才库搜索筛选、通知行键盘可达、活动草稿跨刷新、周报周次派生+倒序、专班截止态、组长上传下拉排除已取消并标「已上传」、看板卡与活动列表行键盘可达、组长台副标题「复盘提交」→「复盘状态查看」（对齐组长复盘只读裁定）。**验证**：module-load 1/1；**全量 475/475 全过**（此前一次连续跑出现 4 项 `page.goto` 超时，单独复跑 16/16 全过=串行资源竞争抖动）；真机回归 13 项 + 定点复验 4 项全 PASS。**乙部**：余 R-16/R-17（R-18 当日闭环退出）。**收口补全（批次 12/13）**：R-18 组长台账明细增「按活动/来源筛选」（实测 53 行→3 行、40 行→3 行）；窄屏 390×844 实测——党委「支部配置」溢出（390/411）修复归零、名册/组长两页实测本就无溢出；8 项单点逐项真机实证（6 项通过：草稿跨刷新、人才库搜索、周报周次起止、考勤空态、反馈无 undefined、首页登录态标题）**并修出两处真未修**（通知行「删除/编辑」纯悬停可见 → 常显；专班详情页过截止日仍「招募中」→ 派生「报名已截止」+ 截止说明）；批次 11 遗留「交接确认接收」实证可用（pending→done + toast + 销待办）。版本串 **20260912d** / CODE_VERSION **114**
