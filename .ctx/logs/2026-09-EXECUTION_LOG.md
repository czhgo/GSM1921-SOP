...
- **登记/遗留（2026-09-11 收口后状态）**：乙部仅余 **R-14**（浅色存量需改样式表：styles.css `.ref-*`、about.css 浅色约 52 处、status-badge pending 议题——待专批）；R-3~R-13 各批、功能实现评议下一轮 8 项、最小三成本第 6 轮 6 条、黑话第 6 轮、拖拽机测、成员写链语义端点与副书同权、深色四轮、浅色存量核销均已闭环并**完整退出**（权威登记=本日志 T-077 追加批次 1~7 与 REVIEW_QUEUE 附录①~⑧）；排期表当前为空（新轮次暂不启动）；丙部 P.14 push 放行待批（**ahead 12 笔**，推送前先 `git status`）。
