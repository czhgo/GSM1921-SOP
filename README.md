...
1. **打开网页**：访问 [公网地址](https://czhgo.github.io/GSM1921-SOP/) 或本地双击 `docs/index.html`
2. **写入活动**：党建工作台 → 选角色（党支书/组长）→ 填日期/场景/名称/地点 → 写入活动
3. **查自己要做什么**：左侧选角色按钮 → 系统自动跳转至对应工作台

| 我想做的事 | 操作路径 |
|-----------|---------|
| 把新活动加进日历 | 党建工作台 → 选党支书/组长 → 填日期/场景/名称/地点 → 写入活动 |
| 查阅标准流程手册 | [常见工作场景快速指南](content/SOP/常见工作场景快速指南.md) |
| 发现流程有问题想改 | [SOP 优化提案反馈卡](content/references/模板库/FEEDBACK_FORM.md) |
| 查看活动历史记录 | 左侧点击【归档库】按钮 |
| 查看考勤情况 | 党建工作台 → 选纪检委员 → 考勤管理 |
...
| 新任支委 | [常见工作场景快速指南](content/SOP/常见工作场景快速指南.md) + [定人定责定岗说明](content/SOP/支委与党小组定人定责定岗说明.md) | — |
...
> 以下是按**阅读目的**组织的文件导航，帮助你快速找到需要的内容。
...
| 目录 | 内容 | 核心文件 |
|------|------|---------|
| [governance/](content/guides/governance/) | 治理规范——术语标准、角色分类、文档导航 | [TERMINOLOGY.md](content/guides/governance/TERMINOLOGY.md)（术语唯一权威）、[DOC_MAP.md](content/guides/governance/DOC_MAP.md)（全局文档导航） |
| [architecture/](content/guides/architecture/) | 架构设计——数据模型、权限矩阵、管理模式 | [DATA.md](content/guides/architecture/DATA.md)（数据模型唯一权威）、[MANAGEMENT_MODE.md](content/guides/architecture/MANAGEMENT_MODE.md)（权限矩阵） |
| [design/](content/guides/design/) | 功能设计——委员系统、日历、SOP 联动、品牌活动 | [COMMISSIONER_SYSTEM.md](content/guides/design/COMMISSIONER_SYSTEM.md)（委员系统与专班制）、[SOP_WEB.md](content/guides/design/SOP_WEB.md)（SOP 与网页双向修改指南） |
...
| 层 | 位置 | 职责 |
|----|------|------|
| **前台（UI）** | `docs/*.html`（19 页面） | 页面骨架，零硬编码逻辑 |
...
```
content/SOP/ → docs/src/services/ → docs/src/entries/ → docs/*.html
```
...
```
主页（index.html）
├── 党建工作台（workspace.html → 角色选择面板）
│   ├── 党支书工作台（ws-secretary.html）
│   ├── 党小组组长工作台（ws-leader.html）
│   ├── 组织者工作台（ws-organizer.html）
│   ├── 深度参与者工作台（ws-deep.html）
│   ├── 组织委员工作台（ws-org-commissioner.html）
│   ├── 宣传委员工作台（ws-prop-commissioner.html）
│   ├── 纪检委员工作台（ws-disc-commissioner.html）
│   └── 参与者只读面板（ws-visitor.html）
├── 党务管理（party.html → 角色选择面板）
