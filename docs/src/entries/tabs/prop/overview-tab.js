...
import { renderWorkOverview } from '../../../components/work-overview.js?v=20260921l';
import { AuthStore } from '../../../services/auth.js?v=20260921l';
import { beginOverviewShell, countOwnPendingReports, mountOverviewDispatchBar } from '../../../components/overview-dispatch-bar.js?v=20260921l';

export async function renderContent(ctx) {
  const el = document.getElementById('prop-tab-content');
  if (!el) return null;
  const personId = AuthStore.getCurrentUser()?.personId || 'p12';
  // U3：进入概况先骨架/槽位占位（防 0 高弹跳与渲染后插条下推）；计数失败不阻断概况渲染（降级=不挂条）
  beginOverviewShell(el);
  let count = 0;
  try { count = await countOwnPendingReports(personId); } catch (e) { console.warn('[prop-overview] 待答复计数失败（不挂直达条）', e); }
  mountOverviewDispatchBar(el, { count, prefix: 'prop', jumpTab: 'my-dispatch', accent: ctx.accent });
