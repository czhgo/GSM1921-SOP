...
import { STATE, getAppState, setState, registerRenderCallback } from '../core/state.js?v=20260922i';

import { bootstrapPage } from '../core/bootstrap.js?v=20260922i';
import { renderTabBar, tabContentSkeletonHtml } from './tab-bar.js?v=20260922i';
import { flashHighlight, escHtml } from '../core/utils.js?v=20260922i';
...
import { TodoStore } from '../services/todo.js?v=20260922i';
import { AuthStore } from '../services/auth.js?v=20260922i';
import { BranchService } from '../services/runtime.js?v=20260922i';
import { applyTabPolicy, getBranchIdOfPerson, getBranchById } from '../services/branch.js?v=20260922i';
// 设置中心批2（2026-09-09 支书批准 v3）：个人 tab 顺序覆盖（个人层；支部层=applyTabPolicy 之上叠加）
import { applyPersonalTabOrder } from '../services/preferences.js?v=20260922i';
...
export async function createWorkspaceShell(opts) {
  ...
  function _ensureTabBar(state) {
    ...
    // 旧 ?tab=<旧id> 落到承接 tab；仅当清单不含该 id 时按别名解析（清单命中优先，零影响现行 id）。
    // 已废止的旧 id（如纪检台「制度与文本」mailbox）不设别名。
    const _legacyTabAliases = { 'workspace:disc': { makeup: 'attendance' } }[scope] || {};
    const _resolveTabId = (id) => {
      if (!id) return id;
      if (tabs.some(t => t && t.id === id)) return id;
      return _legacyTabAliases[id] || id;
    };

    // S2：tabs 定稿后计算初始激活优先级（URL ?tab= > 有待办必见待办 > 记忆/默认）
    const navTabId = _navTarget && _navTarget.tab ? _resolveTabId(_navTarget.tab) : undefined;
    const navTab = (navTabId && tabs.some(t => t && t.id === navTabId))
      ? navTabId
      : undefined;
  ...
  function renderUI(state) {
    ...
    if (activities.length === 0 && fallbackData) {
      ...
      if (all.length > 0) {
        ...
        setState({ activities: filterActivitiesForViewer(all, AuthStore.getCurrentUser()?.role).map(mapFallbackActivities), ...(fallbackExtras || {}) });
