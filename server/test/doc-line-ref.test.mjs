...
//       机器可复核者逐条核对（当前 2 条：`副组长` 在 `docs/` 零命中；`AI_API_BASE_URL` 在代码中无消费点）。
...
test('R4 「零命中 / 检索不到消费点」类关键词取证仍成立（防取证过期）', () => {
  // ① README-server.md §7.3#17：「`docs/` 与 `server/` 全仓检索『副组长』零命中」→ 复核 docs/
  const hitsDoc = [];
  (function walk(dir) {
    for (const n of readdirSync(dir)) {
      const f = join(dir, n);
  ...
  assert.deepEqual(hitsDoc, [], `README-server.md 断言「\`docs/\` 内『副组长』0 命中」，实测命中：${hitsDoc.join(' / ')}` +
    `（若已实现副组长，须同批改准该条现状，或撤下该断言）`);
