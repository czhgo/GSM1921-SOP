...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/disc/todo-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    // 工作概况（支书 2026-08-10 裁定：全部角色新增——汇报/卡点/在办三区总览 + 条线数据注入）
    { id: 'overview', label: '工作概况', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/disc/overview-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    { id: 'attendance', label: '考勤管理', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/disc/attendance-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    { id: 'review', label: '活动监督复盘', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/disc/review-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    { id: 'inspection', label: '考察管理', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/disc/inspection-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    ...
    { id: 'mailbox', label: '制度与文本', groupLabel: '制度与答复', render: (ctx) => import('../../entries/tabs/disc/policy-text-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    ...
    { id: 'tf-view', label: '知情查看', groupLabel: '知情查看', render: (ctx) => import('../../entries/tabs/disc/tf-view-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
    { id: 'my-dispatch', label: '我的处置', groupLabel: '反馈', render: (ctx) => import('../../entries/tabs/disc/my-dispatch-tab.js?v=20260914k').then(m => m.renderContent(ctx)) },
  ],
});
