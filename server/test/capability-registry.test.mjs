...
test.before(async () => {
  const src = await readFile(
    new URL('../../docs/src/core/registry.js?v=20260901m', import.meta.url),
    'utf8',
  );
...
test('data-source.js 注册 mock/api 数据源能力（env/deps/apply）', async () => {
  const src = await readFile(
    new URL('../../docs/src/modules/capabilities/data-source.js?v=20260901m', import.meta.url),
    'utf8',
  );
...
test('sop-scenarios.js 注册场景能力（scope/scenario + list/get）', async () => {
  const src = await readFile(
    new URL('../../docs/src/modules/capabilities/sop-scenarios.js?v=20260901m', import.meta.url),
    'utf8',
  );
...
test('decision-tree 消费点经注册表读取场景（getScenario 回退 sopDatabase）', async () => {
  const src = await readFile(
    new URL('../../docs/src/services/decision-tree.js?v=20260901m', import.meta.url),
    'utf8',
  );
...
test('bootstrap 数据源选择经注册表（行为零变化：有 token 走 api，回退 mock）', async () => {
  const src = await readFile(
    new URL('../../docs/src/core/bootstrap.js?v=20260901m', import.meta.url),
    'utf8',
  );
...
test('components.js 注册共享组件能力（scope: component，M6）', async () => {
  const src = await readFile(
    new URL('../../docs/src/modules/capabilities/components.js?v=20260901m', import.meta.url),
    'utf8',
  );
