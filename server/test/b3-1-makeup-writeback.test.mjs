...
await p.evaluate(async () => {
  ...
  const { loadAttendanceRecords, saveAttendanceRecords } = await import('/src/services/attendance.js?v=20260914i');
  ...
  const att = records.find(r => r.id === 'att38');
  if (att) {
    att.status = 'absent';
    att.overdue = true; // 前置为逾期 → 回写后必须清除（保持「overdue 清除」断言的回归检出力）
    delete att.madeUpAt;
