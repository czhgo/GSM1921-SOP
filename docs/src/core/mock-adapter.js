...
import { mockDB, SCHEMA_VERSION } from './domain.js?v=20260912h';
import { generateId } from './id.js?v=20260912h';
// 计票方式（ballotMode）单一源：正式表决无记名落库口径与 server/routes/committee.js 同源（constants.js）
import { ballotModeOfActivity } from './constants.js?v=20260912h';
...
import { ACTIVITIES } from '../mock/activities.js?v=20260912h';
import { SEED_TASKS, SEED_ASSIGNMENTS, SEED_ARCHIVE_RECORDS, SEED_SIGNUPS } from '../mock/seed.js?v=20260912h';
// Seed 增量合并用（2026-08-05）：attendance.js/notices.js 为纯数据模块，
// 经 services/person.js（只依赖 domain/people）→ 无指向本文件的循环依赖
import { ATTENDANCE_RECORDS } from '../mock/attendance.js?v=20260912h';
import { MOCK_NOTICES } from '../mock/notices.js?v=20260912h';
import { BRANCHES } from '../mock/branches.js?v=20260912h';
...
export const MockAdapter = {
  ...
  loadDB() {
    ...
    import('./data-adapter.js?v=20260912h').then(({ notifyDataLoaded }) => notifyDataLoaded())
      .catch(() => {});
  },
  ...
  activities: {
    ...
    create(data) {
      return _withDelay(() => {
        ...
        import('../services/todo.js?v=20260912h').then(({ LifecycleTodoDeriver }) => {
    ...
    archive(id) {
      return _withDelay(() => {
        ...
        import('../services/notice.js?v=20260912h').then(({ NoticeStore }) => {
  ...
  notices: {
    ...
    create(data) {
      return _withDelay(() => {
        ...
        _saveToStorage();
        import('../services/todo.js?v=20260912h').then(({ NoticeTodoDeriver }) => {
  ...
  users: {
    list() {
      return _withDelay(() => import('../services/person.js?v=20260912h')
        .then(({ getBaseMemberRecords }) => getBaseMemberRecords()));
    },

    create(data) {
      return _withDelay(async () => {
        const { PersonStore } = await import('../services/person.js?v=20260912h');
