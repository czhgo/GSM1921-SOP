...
import { generateId } from '../core/id.js?v=20260901q';
import { persist } from '../core/data-adapter.js?v=20260901q';
import { SEED_SIGNUPS } from '../mock/seed.js?v=20260901q';
import { getPersonById } from '../mock/index.js?v=20260901q';
import { TodoStore, TodoSourceType, TodoActionType, TodoCategory, TodoStatus } from './todo.js?v=20260901q';
...
async function _writeSource(sourceType, sourceId, personId, role) {
  if (sourceType === 'activity') {
    ...
    if (role === 'organizer' || role === 'deep') {
      ...
      const { updateActivity } = await import('./mock.js?v=20260901q');
    ...
    if (s.status === SignupStatus.APPROVED) {
      if (s.sourceType === 'activity') {
        ...
        if (act && Array.isArray(act.assignments) && act.assignments.some(x => x.personId === personId)) {
          ...
          import('./mock.js?v=20260901q').then(({ updateActivity }) => {
