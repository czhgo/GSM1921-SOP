...
//  展示位：支书/副支书在活动详情（inspector）「表决汇总」卡片下方（记录决议视图）——
//  每条已记录「通过」的议程（决议）下：勾选/录入待落实项（事项/责任人/时限）→ 保存后
//  services/resolution-followup.js 落库并派生责任人跟进待办（到期催办）；逾期进支书台待办。
//  数据读写全部走服务层，本组件只做视图与事件；保存/销项后经 onChanged 触发整体重绘。
//  责任人候选 = 支委角色 + 具体成员（与 workforce-panel 发起分工的负责人下拉同口径）。
// ════════════════════════════════════════════════════════════════

import { escHtml as esc, showToast } from '../core/utils.js?v=20260915e';
...
} from '../services/resolution-followup.js?v=20260915e';
