...
const README = fileURLToPath(new URL('../../README-members.md', import.meta.url));

test('README-members.md 功能地图标记块与实时生成一致（防漂移）', () => {
  const readme = readFileSync(README, 'utf8');
  assert.ok(readme.includes('<!--FUNC-MAP:START-->'), 'README-members.md 已含标记块');
  const m = /<!--FUNC-MAP:START-->([\s\S]*?)<!--FUNC-MAP:END-->/.exec(readme);
  assert.ok(m, 'README-members.md 已含标记块');
