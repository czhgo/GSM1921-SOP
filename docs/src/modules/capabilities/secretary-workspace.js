...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/secretary/todo-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    { id: 'overview', label: '全局概况', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/secretary/overview-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    // ── 我的职责（写操作） ──
    { id: 'calendar', label: '活动管理', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/secretary/calendar-tab.js?v=20260919k').then(m => m.renderContent(ctx?.appState)) },
    { id: 'assign', label: '赋权管理', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/secretary/assign-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    { id: 'notification', label: '通知发布', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/secretary/notification-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    // P3 党委后台（2026-09-02）：支部关键事项上报党委（发展节点/活动报备；党委批驳结论回传本页）
    { id: 'report-up', label: '上报党委', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/secretary/report-up-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    ...
    { id: 'group-progress', label: '党小组', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/secretary/group-progress-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    ...
    { id: 'committee-meeting', label: '支委会会议', groupLabel: '我的职责', render: () => import('../../entries/tabs/secretary/committee-meeting-tab.js?v=20260919k').then(m => m.renderContent()) },
    ...
    { id: 'tf-view', label: '知情查看', groupLabel: '知情查看', render: (ctx) => import('../../components/insight-view.js?v=20260919k').then(m => m.renderInsightView(document.getElementById('secretary-tab-content'), { defaultView: 'taskforce', highlightTfId: ctx?.highlightTfId || null })) },
    // L4 支部工作地图（2026-09-03 支书裁决放行）：支部分工 · 平铺/按人双视图（支书/副支书共用支书台）
    { id: 'work-map', label: '支部分工', groupLabel: '知情查看', render: (ctx) => import('../../entries/tabs/secretary/work-map-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
    // ── 制度与答复 ──
    { id: 'feedback', label: '反馈管理', groupLabel: '制度与答复', render: (ctx) => import('../../entries/tabs/secretary/feedback-tab.js?v=20260919k').then(m => m.renderContent(ctx)) },
  ],
});
