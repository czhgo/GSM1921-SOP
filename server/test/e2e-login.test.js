...
test('账号密码登录后直达工作台，切换 API 数据源且后端数据可达', async () => {
  ...
  try {
    ...
    await page.evaluate(async ({ uniqueTitle, dateStr }) => {
      ...
      const { mockDB } = await import('/src/core/domain.js?v=20260901f');
      const { persist } = await import('/src/core/data-adapter.js?v=20260901f');
    ...
    for (let attempt = 0; attempt < 3 && !landed; attempt++) {
      ...
      if (!landed) {
        await page.evaluate(() => import('/src/core/data-adapter.js?v=20260901f').then((m) => m.persist()));
      }
    }
    if (!landed) {
