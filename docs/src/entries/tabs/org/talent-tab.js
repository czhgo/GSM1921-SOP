...
// S1–S4 滞留党员设计（2026-09-06 书记已批）+ IA-C2 收敛（2026-09-06）：本页只读查看
// 经 C 批确权与 IA-C2 收敛，成员「在校/滞留」直改位移除（登记 2026-09-06）——
// 阶段/在册/滞留变更统一走「成员名册」报送书记确认后生效（roster-tab 已实现 C 批双确认链）；
// 人才库保留：状态徽标 + 备注 + 留痕历史 查看；发展对象考察画像等其它状态维护不受影响。
...
/** 成员状态展示区（详情面板内 · IA-C2 收敛 2026-09-06：直改位移除，仅只读查看）
 *  登记：经 C 批确权与 IA-C2 收敛，直改位移除 2026-09-06——
 *  阶段/在册/滞留变更请到「成员名册」报送书记确认后生效（本页保存控件与写逻辑已移除）。 */
function _residenceSectionHtml(person) {
  // 党委组织员（组织级角色）不属于本支部：只读说明，不涉及名册报送
...
function _renderTalentDetail(personId) {
  ...
  // IA-C2 收敛（2026-09-06）：原「成员状态保存（组织委员位）」写逻辑随直改位移除——
  // 阶段/在册/滞留变更走 成员名册 报送书记确认；本页仅保留上方只读展示区（登记）。
}
