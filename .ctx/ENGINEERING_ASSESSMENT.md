...
| R28 「同一病灶只修一处」当成了「修完」 | 支书批次 32 实报的「提交报必填、框却不在位」修好后，**同一形态全站还有 92 处**（提交动作的字段级必填校验，20+ 文件）——已修且经真机验证的仅 **1 处**；**没有「同类规模」这一步**，于是「修完一处」被当成「修完一类」 | ① 立**台账** `server/test/form-loop-registry.mjs`（92 条：file / 字段 / 流程 / 可否自动化，71 条非自动化逐条写明 reason）；② 建**真机闭环普查** `server/test/form-loop-sweep.test.mjs`（10 条流程：空必填点提交必须出现可见提示，且**提示点名的字段须有可见载体**；载体不在位却报字段级必填＝违规；既无提示也无提交成功＝静默失败＝违规）；③ 台账守卫 S0–S4（规模不得静默缩水 / 白名单必带 reason / machine:true 全部纳入覆盖 / 无僵尸条目 / 出处文案存在）；④ 纪律入 `DATA_CONSISTENCY_CHECKLIST` **第十三范本 + §0.1 第十三问「同类规模」** 与 `CLAUDE.md R-67` | `form-loop-sweep.test.mjs`（S0–S4 + 10 条真机闭环，15 项） |
...
- **模块化分演进**：75 → **78**（批 25–35，541→566）→ **80**（批 37–39，566→569）→ **81**（批 41，569→571）→ **82**（批 42，571→586）→ **83**（批 46①+47A/B/C，586→624）→ **84**（批 47-H/I，624→**631**）；开源化 74 → **75**（批 42，未动）。**47-B 为何只 +1**：口径下沉单一源与「索引 ↔ 守卫」双向校验是实质加分，但 47-B 实测暴露一处**隐式耦合**（单档口径原受 CDN Tailwind 加载顺序支配）——「单一源」当时并不真的单源，两相抵后记净 +1。**47-H/I 为何 +1**：47-I 把**聚合口径**从「组长台 tab 文件内联」抽为**服务端同源纯函数**（两端一套口径 + 服务端承担汇总职责）＋新增两条常驻守卫（S12 授权声明、S1–S4 单一源/双态同源）是模块化实质进步；但**同时新增了 1 个测试文件**（与 47-F「合并减量」方向相反），两相抵记净 +1。**未给更高分的已知短板**：`server/test` **107 个文件 / 1.8 万行**未整合（支书已点出，见 `REVIEW_QUEUE` 批次 47-F）；综合（五维均值 84/75/75/84/80 = **79.6**）按「未过半分不进位 + 重改进不唯分」仍记 **≈79**。
- **收敛台账规模**：§3.4 表内 **R13–R28 共 16 条**；另 **R28-附**（工具脚本改写数据，批 46① 根治）与 **R29**（授权声明须可核验，批 47-H）**已入 `DATA_CONSISTENCY_CHECKLIST §0.2` 索引 + `CLAUDE.md`（R-70），本表（§3.4）行待补**——两者口径并不同步，已登记 `Q-23-43`（**不假称已对齐**）。`CLAUDE.md` 新纪律 **R-68**（真机量测须与生产同环境；档位须直接声明）、**R-69**（不得以「等提交/等裁定」结束回合，该问就问）、**R-70**（授权声明必须可核验，功能不得随批次「一起进」）；可迁移要点 **①–⑫**；自查问句 **十五问**；范本 **第六–第十六**（第十六＝辅助小字样本学习，**学习中未定稿**）；判例集 `TEST_AND_VERIFICATION.md`（含「单一源搬迁后引用改签遗漏」「工具脚本副作用」等）。
- **测试规模演进（全量实测口径）**：541 → 566 → 569 → 571 → 586（批 42）→ 595（批 43）→ 批 44 新增定向 16 项（`form-loop` 15 + `doc-consistency` S9）→ **624**（批 46①+47A/B/C 实测：46① 新增 6 项、47-A 2 项、47-B 4 项；47-C 只改普查内部断言、未增删用例）→ **631**（批 47-H/I 实测：47-H `doc-consistency` **S12** 1 项、47-I `member-progress` **4 项**；47-D 的真机流程与 47-H 的接分页**落在既有断言内、不增用例数**）→ **632**（批 47-D ③ `secretary-notice-edit` 新增 1 条真机流程；**47-F 试点组合并 5 文件 → 1 文件为「等量转移」——用例数不变**）；按新协议**不再每批全量复跑**（定向为主、交付前才全量，见 §5.4 与 `README.md` 开发纪律）。
