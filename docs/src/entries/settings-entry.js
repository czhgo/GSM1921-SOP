...
import { readLoginSnapshot } from '../core/login-snapshot.js?v=20260912j';
import { ROLE_LABELS, ROLE_PAGE_MAP, getAccentColors } from '../core/constants.js?v=20260912j';
import { resolveAppliedAccentRole } from '../core/theme.js?v=20260912j';
import { appearanceControlsHTML, bindAppearanceControls } from '../components/appearance-controls.js?v=20260912j';
import { icon } from '../core/icons.js?v=20260912j';
import { escHtml as esc } from '../core/utils.js?v=20260912j';
...
} from '../services/preferences.js?v=20260912j';
...
import { registerApiAdapter, setDataSource, init as dataInit } from '../core/data-adapter.js?v=20260912j';
import { ApiAdapter } from '../core/api-adapter.js?v=20260912j';

// 2026-09-09 支部归属显式化/审计内核：变更记录展示与回滚按钮需要操作者姓名、单键回滚白名单
import { getPersonName } from '../services/person.js?v=20260912j';
import { CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260912j';
...
function loadAuth() {
  if (!_authModule) _authModule = import('../services/auth.js?v=20260912j');
  return _authModule;
}
...
async function buildMyWorkspaceModel(role, personId) {
  ...
  const { applyTabPolicy, getBranchIdOfPerson } = await import('../services/branch.js?v=20260912j');
...
async function renderBranchGovSection(panel, sectionId) {
  ...
  try {
    const br = await import('../services/branch.js?v=20260912j');
    ...
    if (sectionId === 'branch-info-wizard') {
      ...
      await import('../modules/capabilities/secretary-workspace.js?v=20260912j');
...
async function openWizardEmbed(panel) {
  ...
  try {
    const { mountOrgSetupWizard } = await import('../components/org-setup-wizard.js?v=20260912j');
