...
test('S2 渲染桥：kind 映射/字段收拢/无障碍关联正确', async () => {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      const { THEME_PARTY_DAY_MANIFEST, TASKFORCE_RUN_MANIFEST } = await import('/src/workflow/blocks/manifests.js?v=20260903c');
      const { renderManifestFields, manifestFieldCatalog, fieldDomId } = await import('/src/workflow/blocks/form-renderer.js?v=20260903c');
