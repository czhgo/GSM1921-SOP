...
//  背景：spec 五——components/work-overview.js 为受保护/长期定稿组件，其内汇报区摘要化需支书特批；
...
//  支书台（secretary/overview-tab.js 受保护）不挂：其待办页/概况已有「待答复」收件箱既有入口（登记 2026-09-06）；
//  参与者台 2026-09-15 起已有「我的处置」tab（反馈答复入口），但处理位仍是概况汇报区行内 + 顶栏一键汇报角标——概览页不挂重复直达条（登记）。
...
import { IssueStore } from '../services/issues.js?v=20260916a'; // 计数依赖（U3 前置计数须显式引用；勿裸依赖全局）
