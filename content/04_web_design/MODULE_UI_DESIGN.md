...
- **「党建」Tab 分组（原党务管理模块）**：角色工作台内的功能板块，涵盖发展党员全流程追踪、补课制度跟踪、意见反馈、公邮查收提醒等子功能。
...
## 二、「党建」Tab 分组界面设计（原党务管理模块）

### 模块定位与功能边界

#### 模块定位

「党建」Tab 分组承载原党务管理模块的全部子功能，是**工作台**内的功能板块——2026-08-03 起党建与党务统一承载于角色工作台（Tab 分组统一为「党建」），不再作为与工作台并列的独立模块。
