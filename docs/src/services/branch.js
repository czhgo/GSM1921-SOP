...
import { PARTY_COMMITTEE } from '../mock/branches.js?v=20260914o';
import { getAdapter, persist, getDataSource } from '../core/data-adapter.js?v=20260914o';
import { listCapabilities } from '../core/registry.js?v=20260914o';
// P1a 单向权威（2026-09-03）：config 净化唯一实现 = core/config-clean.js（server PATCH /branches/:id/config 同源）
import { sanitizeConfigBlocks, sanitizeConfigModules, sanitizeConfigWorkforce, sanitizeConfigOrg, sanitizeConfigPolicyOverrides, applyBranchPolicyOverrides } from '../core/config-clean.js?v=20260914o';
// 审计内核共享常量（2026-09-09 支书批）：why 透传/单键回滚白名单/历史上限单一源 = config-clean
// （server resources.js 同源 import，双形态防失同步）
import { CONFIG_HISTORY_MAX, CONFIG_ROLLBACK_WHAT, CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260914o';
// L4（2026-09-03）：支部工作地图模块目录单一源 = core/work-map.js（11 模块/缺省分工/快照展开）
import { expandWorkforce } from '../core/work-map.js?v=20260914o';
// 批4（2026-09-09 支书批「域参数」）：policyOverrides 顶层节白名单（覆盖写口校验用）
import { POLICY_OVERRIDE_SECTIONS } from '../core/policy-defaults.js?v=20260914o';
import { randomHex } from '../core/id.js?v=20260914o';
// 核心组判定单一源（2026-09-14 支书裁定·tab 全盘重设）：由「显示标签反推」改为「注册表 coreTab 显式声明」
import { isCoreTab } from '../core/constants.js?v=20260914o';
...
export async function rollbackBranchConfig(branchId, { by = null, targetEntryAt, index } = {}, why) {
  ...
  if (getDataSource() === 'api') {
    try {
      const { ApiAdapter } = await import('../core/api-adapter.js?v=20260914o');
