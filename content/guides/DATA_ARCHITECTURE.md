...
| 应用状态 (appState) | state.js (内存) | 页面生命周期内 | UI 视图状态，不持久化 |
...
| 角色常量 (ROLE_LABELS/COLORS) | constants.js (静态代码) | 静态，代码级维护 | 9 种角色的中文标签与视觉配色 |
...
> 角色类型定义位于 [state.js](file:///d:/GitHub/GSM1921-SOP/src/state.js#L18-L28)，标签/颜色位于 [constants.js](file:///d:/GitHub/GSM1921-SOP/src/constants.js)
...
> 定义位于 [state.js](file:///d:/GitHub/GSM1921-SOP/src/state.js#L126-L146)
...
| 键名 | 存储内容 | 格式 | 读写位置 |
|---|---|---|---|
| `workflowos_branch_db_v1` | 完整 mockDB 状态 | JSON: `{ _schema, users, activities, tasks, attendances, deliverables }` | [service.mock.js](file:///d:/GitHub/GSM1921-SOP/src/service.mock.js#L14) |
...
| **org-commissioner** (组织委员) | 是 | 是 | 是 | 否 | 是 | 是 | 是 | 是 | 是 (专班协调) |
| **prop-commissioner** (宣传委员) | 是 | 是 | 是 | 否 | 是 | 否 | 否 | 是 | 否 |
| **disc-commissioner** (纪检委员) | 是 | 是 | 否 (不设专班) | 是 (唯一) | 是 | 否 | 否 | 否 | 否 |
| **leader** (党小组组长) | 是 | 是 | 否 | 否 | 是 | 否 | 否 | 否 | 否 |
| **organizer** (活动组织者) | 是 | 是 | 否 | 否 | 是 | 否 | 否 | 否 | 否 |
...
| party 子: 纪检委员面板 | disc-commissioner | `data-commissioner="inspector"` | 补课跟踪、公邮提醒、出勤汇总、复盘督办 |
...
> 定义位于 [state.js](file:///d:/GitHub/GSM1921-SOP/src/state.js#L32-L37)
