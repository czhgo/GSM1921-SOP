...
//       其他支委可查看其他支委页面（只读模式�?
// 详见: T-2026-07-006 �?轮·四�?A2 决策

import { AuthStore } from '../services/auth.js';
import { getBasePath, showToast } from '../core/utils.js';

// party 页面配置：role �?{ page, label, accentColor }
const PARTY_PAGES = {
  'secretary':         { page: 'secretary.html', label: '党支�?,     accent: '#B91C1C' },
  'deputy-secretary':  { page: 'secretary.html', label: '副书�?,     accent: '#B91C1C' },
  'org-commissioner':  { page: 'org.html',        label: '组织委员',   accent: '#1D4ED8' },
  'prop-commissioner': { page: 'prop.html',       label: '宣传委员',   accent: '#0E7490' },
  'disc-commissioner': { page: 'disc.html',       label: '纪检委员',   accent: '#047857' },
};
...
export function renderPartyCrossNav(currentPageRole, mountEl) {
  ...
  // 当前编辑角色：用户实际角色或只读视角
  // 当前页面角色：currentPageRole（如 'secretary'�?org-commissioner'�?
  // 是否只读：当前页面角�?�?用户实际角色（书�?副书记特殊处理）
  const isSecretaryOrDeputy = (userRole === 'secretary' || userRole === 'deputy-secretary');
  const isEditingThisPage = isSecretaryOrDeputy
    ? (currentPageRole === 'secretary')  // 书记/副书记在 secretary.html 是编辑模�?
    : (userRole === currentPageRole);

  // 书记/副书记在 secretary.html 显示一个合�?Tab（书�?副书记）
  // 其他支委 Tab 单独显示
  ...
  tabs.forEach(tab => {
    ...
    const style = tab.isActive
      ? `--tab-accent:${tab.accent};--tab-accent-bg:${tab.accent}1A;--tab-accent-border:${tab.accent}40;`
  ...
  mountEl.querySelectorAll('.party-nav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (targetRole === 'secretary') {
        // 书记/副书记访�?secretary 页面：清除只读视角（编辑模式�?
