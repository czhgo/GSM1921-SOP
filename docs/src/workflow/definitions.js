...
import { ROLE_KEYS } from '../core/constants.js?v=20260922d';
  ...
  subStates: {
    attendance: {
      ...
      stepsLabel: {
        ...
        submitted:  '考勤汇总提交（纪检委员汇总考勤→提交支委会，组织委员接收建档；考察记录同此通道）',
      },
    },
    publicity: {
      name: 'publicity',
  ...
  states: [
    ...
    },
    {
      ...
      metadata: {
        ...
        sopSupervisor: 'leader',
        sopTimeOffset: 7,
        sopDesc: '纪检汇总：考察记录 + 考勤（同通道→支委会，组织委员接收建档）。宣传统筹：对接深度参与者整理宣传产出。组织者完成复盘。',
      },
    },
    {
      ...ARCHIVED(),
 ...
 *
 * 属性标签：
 * - isBrand（品牌标签）：布尔属性，**认定＝支委/党小组组长提案 → 支委会审议通过后确定**（2026-09-21 批次 132 · 支书口径二；
 *   无「点一下即认定」入口）。品牌是活动的属性标签，
