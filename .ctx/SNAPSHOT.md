...
date: "2026-09-10"
version: "v29"
milestone: "2026-09-12 续批（第九批）：列表可点扫修（判据 ≤1→≤2 跳修订；9 类卡内不可点实体条目全补入口与落点，含书记台「活动查询」→activity.html?id=、党委监控活动、宣传归档行、支部卡「进入支部」、人才库成员卡 1 跳等） + 意见反馈真匿名（新 issues 表与 GET/POST /api/v1/issues；不存任何可反查字段、书记亦不可见；随机 tokenHash 判重限频；处置公开发布、取消定向回推；历史身份字段读取清除；issue-anonymity 6/6） + 正式表决无记名（voteConfig.ballotMode named|anonymous 发起时选定、制度明文 formal 场景强制锁定+服务端 400 校验；无记名=参与记录+tally、逐人选项不落库；书记端矩阵改已投/未投+汇总；门槛改参与+tally） + 判据入库（操作入口判据修订 + 匿名判据）；乙部余 R-16/R-17；版本串 20260912a / CODE_VERSION 111"
---
...
> **上版**: v28 (2026-09-12)
> **变更来源**: 2026-09-10/11/12（八批 + 本批共九批，T-077 追加批次 1~10；见 v29/v28/v27/v26/v25 里程碑行；测试/文件计数一律以实测为准）
