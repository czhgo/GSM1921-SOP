...
import { mockDB, SCHEMA_VERSION } from './domain.js?v=20260917c';
import { generateId } from './id.js?v=20260917c';
// 计票方式（ballotMode）单一源：正式表决无记名落库口径与 server/routes/committee.js 同源（constants.js）
import { ballotModeOfActivity } from './constants.js?v=20260917c';
...
import { ACTIVITIES } from '../mock/activities.js?v=20260917c';
import { SEED_TASKS, SEED_ASSIGNMENTS, SEED_ARCHIVE_RECORDS, SEED_SIGNUPS, SEED_REVIEW_REQUESTS, SEED_MAKEUP_TASKS } from '../mock/seed.js?v=20260917c';
// Seed 增量合并用（2026-08-05）：attendance.js/notices.js 为纯数据模块，
// 经 services/person.js（只依赖 domain/people）→ 无指向本文件的循环依赖
import { ATTENDANCE_RECORDS } from '../mock/attendance.js?v=20260917c';
import { MOCK_NOTICES } from '../mock/notices.js?v=20260917c';
import { BRANCHES } from '../mock/branches.js?v=20260917c';
...
export const MockAdapter = {
  ...
  loadDB() {
    ...
    import('./data-adapter.js?v=20260917c').then(({ notifyDataLoaded }) => notifyDataLoaded())
      .catch(() => {});
  },
  ...
  activities: {
    ...
    create(data) {
      return _withDelay(() => {
        ...
        import('../services/todo.js?v=20260917c').then(({ LifecycleTodoDeriver }) => {
    ...
    archive(id) {
      return _withDelay(() => {
        ...
        import('../services/notice.js?v=20260917c').then(({ NoticeStore }) => {
  ...
  notices: {
    ...
    create(data) {
      return _withDelay(() => {
        ...
        _saveToStorage();
        import('../services/todo.js?v=20260917c').then(({ NoticeTodoDeriver }) => {
  ...
  users: {
    list() {
      return _withDelay(() => import('../services/person.js?v=20260917c')
        .then(({ getBaseMemberRecords }) => getBaseMemberRecords()));
    },

    create(data) {
      return _withDelay(async () => {
        const { PersonStore } = await import('../services/person.js?v=20260917c');
