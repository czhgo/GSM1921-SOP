...
date: "2026-09-12"
version: "v30"
milestone: "2026-09-12 续批（第十批）：dogfood 七身份真机走查（书记/组织/纪检/宣传/组长/普通党员/党委，71 条真实复现问题）→ 去重 12 类系统性缺陷 S1~S12 + 单点，书记裁定全修——通知详情单一取数口（派生通知按 id 现算重建）、?tab= 深链与内容一致、待办/角标/未读同源、今天落点不空、党委只读下钻封死写入口、宣传归档链路（去归档落点+筛选后按钮）、深色徽章色义恢复（styles.css 特批）、Toast 文字、统计可下钻、人数口径同源、提交校验与反馈、危险操作二次确认；单点含反馈 undefined 兜底/首页登录态标题/人才库搜索/通知行键盘可达/草稿跨刷新/周报周次派生/专班截止态/组长上传下拉排除已取消并标已上传/看板卡与活动列表行键盘可达/组长台副标题改「复盘状态查看」；乙部余 R-16/R-17（R-18 当日闭环退出）；批次 12/13 续收口（R-18 明细筛选、窄屏 390px 实测核销、8 项单点真机实证 + 通知行操作常显/专班详情页截止态两处补修、交接「确认接收」实证可用）；**批次 14 权限专项**（8 角色×10 动作权限矩阵审计 + 真机 API 实证 3 处缺口并修复：后端活动写门、副书同权补齐、组长本组上传约束/纪检组织者上传位/组长改派守卫/mock 表态名单校验；R-16 台账行可点闭环；R-17 书记裁定维持现状、活动创建权保持现状、P.14 暂不推送；新遗留 R-19/R-20/R-21；权限体验真机走查 7 项 PASS 5、FAIL 2 当场复修：副书记「截止表态」按钮不渲染（前端 canLock 未同步）→ 修 + 无权限时明示「由书记/副书记操作」；组长上传下拉空态无声 → 补「本组暂无可上传活动」说明）；**批次 15 三遗留收口**（R-19 通知写门：角色名单上收 constants.js 由支委集派生、前后端同源 + 新增用例锁定；R-20「应到」两口径在表决/分工/考勤各展示点加明示；R-21 补第二/第三党小组小组会种子 act-32/act-33 并给 act-26 补 hostGroup）；版本串 20260912g / CODE_VERSION 117"
---
