...
1. **定位权威源最近变更**：读 [USAGE_POLICY.md](content/03_doc_system/USAGE_POLICY.md) §1 术语表 + [SECRETARY_PRONOUNCEMENTS.md](content/01_strategy/SECRETARY_PRONOUNCEMENTS.md) 最新 P 编号，确认最近一次定义/术语/层级变更是什么（如 2026-08-03 党建/党务 T1 官方定义取代两分法）
...
**为什么必须顺链接读下游**：权威源更新 ≠ 下游同步。书记原话、USAGE_POLICY、insights、SOP 母本、架构文档分层散落，任何一层未同步都会造成"旧定义赫然在重要文件"（2026-08-09 党建/党务两分法残留 5 处即此病）。只改权威源不查下游，等于没反思。
...
5. **术语精确性**：党支部委员简称是"支委"不是"委员"；党建 = 三会一课、主题党日、专班工作；党务 = 党员发展、考勤考察；两者都是"管理事，服务人"的工作，不用"服务同学 vs 供支委决策"区分（书记论断，P-007 已从论断汇编迁出，知识见 insights §1.2 同源不同视角）。
...
- **按需启动**：书记说"做一次评议"时启动
- **周期触发**：每周一次，作为 OPERATIONS\_GUIDE.md §17 周期任务
...
| C-5 | **术语与表达审计**：定期扫描全仓库过时术语（活动建设/组织建设残留、之上/之下、归档→人才库混用等）与**已降级定义残留**（术语权威层级降级后，旧定义须同步全仓为 T1 官方定义，如党建/党务两分法→USAGE\_POLICY §1.1.1），并调用 USAGE\_POLICY §二.2.3 五条可复用检查规则扫描 AI 没吃透精神的表达模式（"而非"对立/"所以"因果链/反面假设/私加强调/元叙事标签），发现后归并至权威源 | H2.1 一改具改 + OPERATIONS\_GUIDE.md §1 + USAGE\_POLICY.md §二.2.3                           | 全仓库                                | 🔄 持续 |
...
| T-127 | **文档【逻辑顺序】重组**：基于 spec `.trae/specs/document-logical-order-restructure-continuation/` 执行阶段 1 续（机械性修复：30 个 .md + 7 个代码文件路径替换 + §5.2/§5.3 编号修复）+ 阶段 2（设计性重组：DEVELOPMENT\_PATH.md 5 章→3 章+附录 A/B、SECRETARY\_PRONOUNCEMENTS.md P-045/P-047 移到第一章 + P-008/P-021 移出至 insights 文件 2、insights 拆分为双文件、CLAUDE.md H2.4 规则 2 "正反两面论"→"按需正反两面论"）。46/46 验证项全部通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | H2.1 一改具改 + H2.2 母本子本 + H2.4 经验沉淀 + OPERATIONS\_GUIDE §13.7 文档【逻辑顺序】                                | 全仓库                                                                                                               | ✅ 完成            |
...
| T-200 | **书记原话逐条复核**（长期）：书记指令（2026-08-03 T-199）："我意识到 书记原话 需要每一条都由书记再过一遍！！请写入乙部！方便后续进行"。SECRETARY_PRONOUNCEMENTS.md 每一条论断需由书记逐条过一遍，确认是否为准确原话；P-014 为第一例（2026-08-03 T-199 已重写为「条块=理解手段」，删除 AI 推导的「条管功能决策/块管信息触达」句）。随书记节奏逐条推进，AI 不得自行改动未经书记确认的原话。**第二轮已完成**（2026-08-09 T233）：P-001~P-005 逐条评议（给足上下文）——P-001 补日期（2026-07-01）+补展开、P-002 保持双录仅补观察点3扩充、P-003 无异议、P-004 原话"工作"→"建设"（随书记 2026-08-05 最新表述统一，加注 P-044 关联）、P-005 照录不动（疑似转录瑕疵不轻改）。**第三批已完成**（2026-08-09 T233/T234）：P-006/P-007 整章迁出（书记裁决：区分两分法给成员带来理解压力，作为 meta 级论断从汇编删除，知识以 T1 保留于 USAGE_POLICY §1.1.1 与 insights §1.1/§1.2，全仓文档统一 T1 叙述）+ P-009 大改（书记裁决原标题"扁平化≠无分工，而是无上下级"为错误反论，改"扁平化不能与没有'程序'等同"；原话补全"身份不是最重要的，年级和政治面貌不决定是否胜任"；新增 2026-08-09 两句原话——推广到整个组织链条 + 可持续的管理；删除 AI 提炼句"分工依据事情本身"）+ 章节重编号（11 章）+ 附录索引更新 + 全仓下游引用联动（USAGE_POLICY/SERVICE_CATALOG/FLAT_DESIGN/sop INDEX/定人定责/CLAUDE.md L183/OPERATIONS_GUIDE 反论案例/工程演进 §3.7）+ P-010 重构（书记裁决"分工记录就是管理闭环的重要部分"——旧标题"分工记录而非管理闭环"为 AI 自造、完全错误；正文 6 句确认为书记原话；设计初衷锚定 P-009）+ P-011 弃用并入 P-010（书记裁决"深度参与者也要思考；组织者也可以执行，这个比喻应当抛弃"）+ 全仓下游联动（FLAT_DESIGN 删核心比喻与 §（三）/README/SOP_WEB/定人定责/help.html/about-entry.js duty 标签平实化/工程演进 §6.18 判例回滚）。方法沉淀：预审报告（出处/母本/五条检查/最新表述差异）→ 书记逐条裁决 → 修订；**元原则**（书记 2026-08-09）：书记所有原话都是一个 big picture 下的不同侧面，须体悟统一性与一致性、融会贯通（已写入 H5 开头）。**第四批已完成**（2026-08-09 T236）：P-012 重构为专班总论（书记："我认为更重要的是介绍'为什么要设立专班'。把这些条目都可以统合进去"；"活动之外**继续**考察"删"继续"——书记："不是'继续'，是活动之外考察积极分子的载体。这两者没有'继续'关系。要务必谨慎"，该句升为正文原话；正文新增 2026-07-12 专班互斥原话"专班是因为——不限时间不限地点，这一定和【活动】是互斥的"）+ P-013 标题换表达（书记："提出需求 为什么概括成了 发起。我觉得有点怪。所以你可以换一个表达"——标题改"提出需求与招募统筹分离"；引用块删 AI 因果"确保权责分离"；补"组织委员能承担这一全局评估与其人才库维护职能分不开"（书记 2026-08-09 补充）+ 最小成本原则原话（2026-07-31）关联）+ P-005 补原话"最大的礼貌是'负责'……非正式的表达中更能迸发创新的火花"（框架内对话的延伸）+ P-009 补原话"谈心谈话的默认认知①②"（"可持续的管理"的推论，书记提示非"永远不推敲"）+ 全仓下游联动（insights §3.1 删"继续"、§3.2 与命题表"发起≠招募"同步换表达、组织委员工作流程指南同步、第四章标题/目录/过渡句/附录索引/锚点更新）。**P-014 定稿已完成**（2026-08-09 续，书记对预审稿裁决"需微微调整，大体是认同的"）：①"两把尺子"为 AI 误记（书记从未说过）→改"条块是理解权责关系的方式"；②删 AI 对仗句"条的边界是垂直穿透、块的边界是横向覆盖"（书记："这种话毫无意义书记怎么可能说"）；③删反论句"条条管事、块块管人"（书记："人正常第一次看不会这么想反论"）；④补"组织关系上，支委会领导党小组"；母子本全仓联动（README/COMMISSIONER_FRAMEWORK §A.3/insights §2.1/help.html 4.4，"方式"口径 + 删穿透/覆盖对仗，零残留验证通过）。**章节重组与跨文件夹迁移已完成**（2026-08-09 续，书记裁决："01 突出'人'的一面，03/04/05 对书记原话用加粗/括号附注即可"）：01 汇编 11 章→4 章（元命题引子 / "组织性"的展开 / 支部怎么组织——扁平化→条块→专班 / 党建的根本目标），P-012~P-015 章节号同步更新；P-016/P-017/P-018/P-024 → 03_doc_system（OPERATIONS_GUIDE §1.1/§15/§16 加粗附注）、P-026 → 04_web_design（DATA_ARCHITECTURE §3.3 人才库附注）、P-029 退役记录 → 04_web_design（SOP_WEB §G.3）、P-027 拆解 → 01（P-005 报备审批例证）+ 02（COMMISSIONER_FRAMEWORK §审批流程规范·双向关系）+ 04（SOP_WEB §G.4 书记全局概览补充意见设计任务，见 T-208）；附录索引更新为 16 条保留 + 迁出指针；修正记录表增迁移记录） | H5 书记评议 | content/01_strategy/SECRETARY_PRONOUNCEMENTS.md | 🔄 进行中 |
