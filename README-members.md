...
- **支部治理（按角色分区）**：书记 / 副书记在这里做「支部信息与换组织向导 / 工作台默认顺序 / 支部制度参数」（副书记与书记同权限）；纪检 / 组织 / 组长各自有「职责参数」卡，调自己那一域的参数（可管则见）；普通成员、访客看不到别人的治理区。
...
完整阐述见 [党支书工作交接文档](content/01_strategy/SECRETARY_DIRECTIVES.md)（书记路线级论断全集）与 [DEVELOPMENT_PATH.md](content/01_strategy/DEVELOPMENT_PATH.md)。
...
4. **支部配置**：侧边栏右下角 设置 → 支部治理（书记 / 副书记，副书同权）或 wizard.html 内分步向导可视化完成（模块启停、分工、术语指引、验证重置）
