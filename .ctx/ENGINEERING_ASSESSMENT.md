...
- **模块化分演进**：75 → **78**（批 25–35，测试 541→566）→ **80**（批 37–39，566→569）→ **81**（批 41，569→571）→ **82**（批 42，571→586）；开源化 74 → **75**（批 42）。综合（五维均值 82/75/75/84/80 = **79.2**）按「未过半分不进位 + 重改进不唯分」仍记 **≈79**。
- **收敛台账规模**：§3.4 **R13–R28 共 16 条**（每条＝一病灶类 → 判据 → 修法 → 守卫）；可迁移要点 **①–⑫**；自查问句 **十四问**；范本 **第六–第十四**；判例集 `TEST_AND_VERIFICATION.md`（含「单一源搬迁后引用改签遗漏」「工具脚本副作用」等）。
- **测试规模演进（全量实测口径）**：541 → 566 → 569 → 571 → 586（批 42）→ 595（批 43）→ 批 44 新增定向 16 项（`form-loop` 15 + `doc-consistency` S9），**按新协议不再每批全量复跑**（定向为主、交付前才全量，见 §5.4 与 `README.md` 开发纪律）。
