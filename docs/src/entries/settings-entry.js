...
import { readLoginSnapshot } from '../core/login-snapshot.js?v=20260922h';
import { ROLE_LABELS, ROLE_PAGE_MAP, getAccentColors } from '../core/constants.js?v=20260922h';
import { resolveAppliedAccentRole } from '../core/theme.js?v=20260922h';
import { appearanceControlsHTML, bindAppearanceControls } from '../components/appearance-controls.js?v=20260922h';
...
} from '../services/preferences.js?v=20260922h';
// 批4（2026-09-09 支书批「域参数」）：制度默认单一源 = policy-defaults（设置页展示「制度默认」行与域参数默认值）
import { POLICY_DEFAULTS, ACTIVITY_APPROVAL_MODES, ACTIVITY_APPROVAL_MODE_LABELS } from '../core/policy-defaults.js?v=20260922h';
...
import { registerApiAdapter, setDataSource, init as dataInit } from '../core/data-adapter.js?v=20260922h';
import { ApiAdapter } from '../core/api-adapter.js?v=20260922h';

// 2026-09-09 支部归属显式化/审计内核：变更记录展示与回滚按钮需要操作者姓名、单键回滚白名单
import { getPersonName } from '../services/person.js?v=20260922h';
import { CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260922h';
...
function loadAuth() {
  if (!_authModule) _authModule = import('../services/auth.js?v=20260922h');
  return _authModule;
}
...
async function buildMyWorkspaceModel(role, personId) {
  ...
  await import(`../modules/capabilities/${stem}-workspace.js?v=20260922h`);
  ...
  const { applyTabPolicy, getBranchIdOfPerson } = await import('../services/branch.js?v=20260922h');
...
async function renderBranchGovSection(panel, sectionId) {
  ...
  try {
    const br = await import('../services/branch.js?v=20260922h');
    ...
    if (sectionId === 'branch-info-wizard') {
      ...
      await import('../modules/capabilities/secretary-workspace.js?v=20260922h');
...
async function openWizardEmbed(panel) {
  ...
  try {
    const { mountOrgSetupWizard } = await import('../components/org-setup-wizard.js?v=20260922h');
