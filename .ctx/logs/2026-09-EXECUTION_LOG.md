...
- **登记/遗留（2026-09-11/12 收口后状态）**：乙部仅余 **R-15**（深色 about 灰字：`--ab-ink-faint:#7E7870` 在 3 个无深色分支 `ab-*` 元素 4.15/3.82/4.15——需另行专批）；R-3~R-14 各批、功能实现评议下一轮 8 项、最小三成本第 6 轮 6 条、黑话第 6 轮、拖拽机测、成员写链语义端点与副书同权、深色四轮、浅色存量核销、R-14 样式表专批均已闭环并**完整退出**（权威登记=本日志 T-077 追加批次 1~8 与 REVIEW_QUEUE 附录①~⑧）；排期表当前为空（新轮次暂不启动）；丙部 P.14 push 放行待批（**ahead 13 笔**，推送前先 `git status`）。
