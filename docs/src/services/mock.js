...
import { getDataSource, notifyDataLoaded, persist } from '../core/data-adapter.js?v=20260921p';
import { bumpToken, resetAllTokens } from '../core/version-token.js?v=20260921p'; // P0 域缓存失效（spec §二.3/§二.4）
...
import { MockAdapter } from '../core/mock-adapter.js?v=20260921p';
...
import { handleInitResetIfRequested, stripSeedRecordsIfInitState } from './init-reset.js?v=20260921p';
// 批4（2026-09-09 支书批「域参数」）：数据加载完成 → 读侧有效默认注入
// （当前人所属支部 config.policyOverrides merge 进 POLICY_DEFAULTS；无 overrides = 保持默认）
import { applyEffectivePolicyDefaultsForPerson } from './branch.js?v=20260921p';
...
export function createActivity(data) {
  return _withDelay(() => {
    ...
    if (!newItem.assignments || newItem.assignments.length === 0) {
      import('./todo.js?v=20260921p').then(({ LifecycleTodoDeriver }) => {
