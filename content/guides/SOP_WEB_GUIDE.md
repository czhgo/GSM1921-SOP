---
role: "[人机]"
title: "SOP 网页优化与同步指南"
type: guide
last_updated: "2026-05-06"
version: "4.0"
milestone: "T29 — 全面架构收束：Mode统一、看板体系、日历限定、系列活动模型"
---

# SOP 网页优化与同步指南

> 本指南是党支部管理引擎（GSM1921-SOP）设计思路的**最先进最完整体现**，整合了 SOP 文档优化方法论、网页架构设计方案、数据映射规则、优化提案机制与 Mock/真实鉴权辨析，为 SOP-网页联动维护提供一站式参考。

---

## A. 问题诊断

### A.1 内容两极分化

当前 SOP 文档和模板库存在两极分化问题：

| 问题类型 | 症状 | 影响 |
|---------|------|------|
| **内容冗余** | 部分 SOP 过多重复的上下文铺垫、过长的引言段落 | 核心流程被淹没，可读性差 |
| **内容粗糙** | 部分模板仅占位、缺少可操作的步骤指引 | 模板无法直接使用，实用性低 |
| **结构不统一** | 文档间信息层级不一致（有的3层，有的5层） | 导航混乱，查找成本高 |

### A.2 跨文档系统性缺陷

| 编号 | 问题 | 影响范围 | 优先级 | 建议 |
|------|------|---------|--------|------|
| G-01 | **三份 SOP 均缺少「一句话摘要」** | 全部 | **P0** | 立即补充，这是网页角色卡片 hover 面板的必需要素 |
| G-02 | **无统一的 Front Matter `summary` 字段** | 全部 | **P0** | 在 YAML front matter 中增加 `summary: "≤30字摘要"`，网页和 AI 均可直接读取 |
| G-03 | **内嵌模板与 SOP 正文混排** | 纪检/组织/宣传 | **P1** | 所有代码块格式的内嵌模板一律提取至 `content/references/模板库/`，SOP 中仅保留链接引用 |
| G-04 | **目录锚点不稳定** | 全部 | P2 | 锚点含中文、括号、特殊符号时，GitHub 锚点转换行为不可预测 |
| G-05 | **文档更新记录全部内嵌于 SOP 末尾** | 全部 | P3 | 版本记录在网页展示中无价值却占据空间；仅保留最近 3 条 |
| G-06 | **无统一的「关联文档」尾节** | 组织 | P3 | 组织委员 SOP 缺少独立的「关联文档」节，与纪检/宣传委员不一致 |

---

## B. 网页架构设计（最先进设计思路）

### B.1 MPA 多页面架构

系统采用 **MPA（Multi-Page Application）** 架构，共有 **7 个独立 HTML 页面**：

| 页面 | 文件 | 用途 | 共享组件 |
|------|------|------|---------|
| **主页** | `docs/index.html` | 通知 + 人员招募 + 活动日历摘要 | header.js, sidebar.js |
| **党建工作台** | `docs/workspace.html` | 活动创建写入、三委员看板、专班管理 | header.js, sidebar.js, workspace-entry.js |
| **党务管理** | `docs/party.html` | 三委员合规面板、党员发展、档案管理 | header.js, sidebar.js, party.js |
| **归档库** | `docs/archive.html` | 历史活动/专班归档查询 | header.js, sidebar.js |
| **资料查询** | `docs/search.html` | 参考资料、网站群、规章文件 | header.js, sidebar.js |
| **意见反馈** | `docs/feedback.html` | SOP 反馈卡提交、反馈列表 | header.js, sidebar.js, feedback-entry.js |
| **关于** | `docs/about.html` | 分章节系统说明书 | header.js, sidebar.js |

**共享组件架构**：
- `header.js` — 全局顶栏（含 view-mode-switcher 模式切换，mode 的唯一主控入口）
- `sidebar.js` — 全局侧边栏（角色选择器，不再包含 mode 选择逻辑）
- `styles.css` — 全局样式（四层色盘系统 + Flat Design System）
- `service.auth.js` — 权限判定（赋权链 + ViewModeStore）

### B.2 管理模式 / 只读模式 架构

> **这是 T26-T29 轮的核心架构决策。**

#### 设计原则

- **身份 ≠ 权限**：选择"我是谁"（角色）和"我能做什么"（管理模式/只读模式）是两个独立的决策
- **Header 为 mode 唯一主控入口**：view-mode-switcher 是 mode 切换的唯一 UI
- **Sidebar 为角色选择器**：点击角色卡片直接选择身份，不再弹出 mode picker
- **Mode 控制权 100% 归 header**，sidebar 只是角色选择器

#### 模式流转

```
用户进入页面
    │
    ▼
Header view-mode-switcher 显示当前 mode badge
    │  管理模式: 红色 #CE1126 · 只读模式: 灰色 #4B5563
    │
    ├─ 点击 switcher → 切换 manage ↔ observe
    │    → 派发 view:mode-change 事件
    │    → 主内容区根据新 mode 重新路由
    │
    └─ Sidebar 角色卡片点击
         → dispatch sidebar:role-select (仅 role, 不含 mode)
         → Header _bindSidebarSync 自动同步 badge
         → Workspace 根据 role + 当前 mode 渲染对应面板
```

#### ViewModeStore

```javascript
// service.auth.js — sessionStorage 持久化
ViewModeStore.getMode(module)  → 'manage' | 'observe'
ViewModeStore.setMode(module, mode)
ViewModeStore.canManage(role, module) → boolean
```

#### 页面 mode 白名单

| 页面 | 支持 mode 切换 | 说明 |
|------|:---:|------|
| 党建工作台 (workspace) | ✅ | 管理模式=职能面板；只读模式=全量日历 |
| 党务管理 (party) | ✅ | 管理模式=支委编辑面板；只读模式=只读概览 |
| 主页 (index) | ❌ | 始终为概览模式 |
| 归档库 (archive) | ❌ | 始终为只读查询 |
| 资料查询 (search) | ❌ | 始终为只读查询 |
| 意见反馈 (feedback) | ❌ | 独立表单页 |
| 关于 (about) | ❌ | 纯展示页 |

### B.3 三委员看板视图体系

> **T29 核心创新：条条支委在管理工作台不需要日历，需要任务看板。**

#### 视图路由（党建工作台 管理模式）

| 角色 | 路由面板 | 核心组件 |
|------|---------|---------|
| **secretary / leader** | 活动写入面板 + 日历 | write panel + calendar |
| **org-commissioner** | 专班协调看板 | 2列看板（待启动/进行中）+ 发布招募表单 + 专班详情展开 |
| **prop-commissioner** | 活动与专班视图 | 2列看板 + 卡片展开子任务详情 |
| **disc-commissioner** | 活动与专班视图 | 2列看板 + 卡片展开子任务详情 |
| **commissioner-group** | 支委选择重定向 | 3张支委卡片，点击后切换至具体支委 |
| **organizer / deep** | 日历视图 | 何时何地参会（日历是给参与者用的工具） |

#### 看板设计原则

- **仅两列**：待启动 + 进行中 —— 无"已完成"列
- 活动/专班全部完成后 → 归档，不在看板中保留
- 子任务完成后不消失：灰显文字 + 删除线 + opacity:0.5 + 绿色对勾 ✓
- 完成的子任务沉到子任务列表底部
- 设计精神：「完成不放飞，但不下沉视野」

#### 宣传/纪检「活动与专班视图」

- 卡片区分 `type`（活动/专班），用 domain tag 着色
- 点击任意卡片 → 展开子任务详情面板
- 子任务分"进行中"和"已完成"两组
- 宣传委员蓝色调，纪检委员琥珀色调
- 宣传/纪检在党建维度**服务于**活动和专班，不直接写入活动

#### 组织委员「专班协调视图」

- 最高优先级：专班管理（党建工作域职能）
- 组织委员不是唯一发起者（党小组组长、条条支委、书记均可发起专班）
- 组织委员是**唯一招募统筹者**
- 含「发布招募」按钮 → 展开招募表单（名称/人数/技能/周期/说明）
- 「发起专班」按钮 — canInitiateTaskForce()

### B.4 日历视图范围限定

> **T29 关键修正：日历 = 参与者工具；看板 = 管理者工具。**

| 场景 | 显示日历 | 原因 |
|------|:---:|------|
| 只读模式（全员） | ✅ | 信息浏览基线 |
| 管理模式 · secretary/leader | ✅ | 需要同时管理活动和看日历 |
| 管理模式 · organizer/deep | ✅ | 需要知道何时何地参会 |
| 管理模式 · org-commissioner | ❌ | 只看专班协调看板 |
| 管理模式 · prop-commissioner | ❌ | 只看活动与专班视图 |
| 管理模式 · disc-commissioner | ❌ | 只看活动与专班视图 |

### B.5 赋权链 (Authorization Chain)

> **2026-05-05 全面修正，对齐 ROADMAP §0.2A+§0.3**

```
党支书（secretary）
  ├──→ 党小组组长（leader）
  ├──→ 组织委员（org-commissioner）
  ├──→ 宣传委员（prop-commissioner）
  └──→ 纪检委员（disc-commissioner）

党小组组长（leader）→ 活动组织者 / 深度参与者（活动域赋权）

组织委员（org-commissioner）
  ├──→ 活动组织者 / 深度参与者（专班域赋权）
  └──→ 专班成员（招募赋权）

宣传委员 / 纪检委员 → 无赋权能力（职能性质决定）
```

**权限判定函数** (`service.auth.js`)：
- `canManage(role)` → 角色是否有管理权限
- `canWriteActivity(role)` → 能否写入活动（仅 secretary/leader）
- `canRecruitTaskForce(role)` → 能否招募专班（仅 secretary/org-commissioner）
- `canAuthorize(role)` → 能否赋权他人（secretary/leader/org-commissioner）
- `canInitiateTaskForce(role)` → 能否发起专班（secretary/leader/org-commissioner/prop-commissioner/disc-commissioner）

### B.6 数据模型全家福

| 模型 | ID | 核心字段 | 状态 |
|------|------|------|------|
| **ActivityRecord** | B1.1 | id, title, type, date, location, duration, direction, leaders[], subRecords[] | ✅ 已落地 |
| **TaskForceRecord** | B1.2 | id, name, sponsorRole, members[{contributions[]}] | ✅ 已落地 |
| **Notice** | B1.3 | id, title, content, priority, publisher, targetModule | ✅ 已落地 |
| **SeriesRecord** | B1.4 | id, title, recurrenceRule{frequency,interval,startDate,endDate}, subActivityIds[] | 🟡 模型已设计 |

#### 系列活动模型 (SeriesRecord B1.4)

> **2026-05-06 新增设计。** 长期活动 ≠ 专班 —— 三者独立但关联。

| 概念 | 维度 | 管理角色 |
|------|------|---------|
| **短期活动** | 事件（一次性） | 党小组组长写入 |
| **系列活动** (= 长期活动) | 时间模式（重复性） | 党小组组长创建、书记审核 |
| **专班** | 人员组织（功能性） | 组织委员招募统筹 |

```
系列活动 (SeriesRecord)
  │ 创建时选择 recurrenceRule (weekly/monthly/quarterly + interval)
  │ 系统自动展开为多个子 ActivityRecord
  ▼
├── 子活动 1 ← 宣传专班服务于此
├── 子活动 2 ← 宣传专班服务于此
├── 子活动 3 ← 宣传专班服务于此
  │
  │ 全部子活动完成 → series.archived = true → 归档
  ▼
归档库
```

---

## C. 优化方法论

### C.1 核心策略：网页可视化驱动的倒推优化

> **以网页可视化展示为下游应用目标，推动 SOP 内容的结构化与优化。**

网页不是 SOP 的"复制粘贴展示区"，而是 SOP 的**可视化检验工具**：
- 当 SOP 在网页上展示效果差 → SOP 本身需要先优化
- 当模板在网页上无法直接填充使用 → 模板结构需要先调整

### C.2 联动优化闭环（4步循环）

```
STEP 1: 网页展示效果审查
  将当前 SOP / 模板渲染到网页界面上
  目视检查：信息层级是否清晰？核心流程是否能一眼定位？

STEP 2: 倒推 SOP 修改点
  根据展示问题反向推导 SOP 缺陷
  产出: SOP 修改建议清单

STEP 3: 执行 SOP 更新
  按修改建议清单逐项更新 SOP 源文件
  更新 SOP Front Matter 中的 last_updated

STEP 4: 同步更新网页渲染
  将更新后的 SOP 重新渲染到网页
  记录: SOP vX.Y → 网页更新 → 验证展示效果一致
```

### C.3 SOP 结构化标准

每份 SOP 文档理想结构：

```markdown
# [文档标题]

> **一句话**: [30字以内的核心摘要]

## 适用角色

## 核心流程
### [阶段1]: [名称]（时间窗口）
1. [动作1]
2. [动作2]

## Checklist
- [ ] [检查项1] — [负责人] / [时限]

## 关联文档
- [文档A](链接) — [关系说明]
```

### C.4 字数精简原则

| 当前问题 | 优化原则 |
|---------|---------|
| 大段政治背景铺垫 | 移至独立的"背景说明.md"，正文只保留 1 行引用链接 |
| 重复的"为什么要做这件事" | 每份 SOP 只在开头有一段 ≤80 字的 Why |
| 历史过程叙述 | 不放在操作指南中，移至 Insights 的经验沉淀 |

---

## D. 展示问题清单

> 以下为 3 份核心 SOP 执行 Step 1「网页展示效果审查」的产出。

### D.0 审查标准

| 检查维度 | 合格条件 |
|---------|---------|
| **一句话摘要** | H1 下方 ≤30 字的 `> **一句话**:` 引用块 |
| **信息层级** | 小标题层级 ≤ 4 层（H2→H3→H4），无跳级 |
| **核心流程可定位** | 从首屏能 3 秒内定位到核心工作流 |
| **冗余铺垫** | 无大段背景铺垫（>100 字），Why 段 ≤ 80 字 |
| **文档长度** | 单一 SOP ≤ 500 行（含附录），超长则拆分 |

### D.1 纪检委员工作流程指南.md

| 编号 | 问题 | 严重度 | 网页展示后果 |
|------|------|--------|------------|
| J-01 | 缺少"一句话摘要" | P0 | 网页角色卡片 hover 面板空白 |
| J-02 | 「文档说明」段过长 | P1 | 首屏被说明文字占满 |
| J-03 | 内嵌考勤表模板混入正文 | P1 | ASCII 表格破坏排版流 |
| J-04 | 文档更新记录过长 | P2 | 占据大量空间 |
| J-05 | 缺少「党建工作台 — 活动与专班视图」说明 | P0 | 三委员最核心的党建域操作完全缺失 |

**结构健康度**：7.3/10

### D.2 组织委员工作流程指南.md

| 编号 | 问题 | 严重度 | 网页展示后果 |
|------|------|--------|------------|
| Z-01 | 缺少"一句话摘要" | P0 | 同 J-01 |
| Z-02 | 文档总长超标（688行） | P0 | 加载慢、阅读体验差 |
| Z-03 | 缺少「党建工作台 — 专班协调视图」说明 | P0 | 组织委员党建域最高优先级职能完全缺失 |
| Z-04 | 四大工作场景与核心职责表重复 | P2 | 信息冗余 |
| Z-05 | 党员发展流程过于详细（89行） | P2 | 应独立为子文档 |

**结构健康度**：5.8/10

### D.3 宣传委员工作流程指南.md

| 编号 | 问题 | 严重度 | 网页展示后果 |
|------|------|--------|------------|
| X-01 | 缺少"一句话摘要" | P0 | 同 J-01 |
| X-02 | 文档总长超标（672行） | P0 | 同 Z-02 |
| X-03 | 缺少「党建工作台 — 活动与专班视图」说明 | P0 | 宣传委员党建域的核心操作完全缺失 |
| X-04 | 内嵌3个完整模板示例 | P1 | 大段模板代码破坏排版 |
| X-05 | 宣传档案队伍建设（学期级）混入日常指南 | P2 | 加重文档重量 |

**结构健康度**：6.0/10

---

## E. 数据映射规则

### E.1 数据源架构

当前系统采用 **MPA 多页面 + 模块化 JS** 架构。HTML 页面通过 `<script type="module">` 加载对应入口 JS，不依赖编译代理。

| 页面 | 入口 JS | 数据源 |
|------|---------|--------|
| workspace.html | `workspace-entry.js` | ActivityRecordStore (localStorage) + ViewModeStore (sessionStorage) + AuthStore (localStorage) |
| party.html | `party.js` | ActivityRecordStore + AuthStore + FeedbackStore |
| feedback.html | `feedback-entry.js` | FeedbackStore (localStorage) |
| index.html | `main.js` | ActivityRecordStore + TaskForceRecordStore + NoticeStore |

### E.2 跨页面状态管理

由于 MPA 架构下各页面为独立文档，跨页面状态通过以下机制共享：

| 机制 | 存储 | 用途 |
|------|------|------|
| **ViewModeStore** | sessionStorage | 管理模式/只读模式（跨页面一致） |
| **AuthStore** | localStorage | 赋权记录（跨会话持久化） |
| **ActivityRecordStore** | localStorage | 活动数据（跨页面共享） |
| **CrossPageState** | sessionStorage | 页面间临时传参（如从主页跳到工作台时传递选中活动ID） |

### E.3 同步安全区（低成本变更）

| 操作类型 | 示例 | 成本 |
|---------|------|------|
| 修改步骤的描述文本 | 优化措辞 | 极低 |
| 修改联系人姓名 | 更新人事 | 极低 |
| 调整步骤的时间偏移 | T-7天 → T-5天 | 极低 |
| 在现有场景末尾追加新步骤 | 新增一步 | 低 |
| 修改合规模板文件名 | 新版文件名 | 低 |

### E.4 架构雷区（高危变更）

| 危险操作 | 正确做法 |
|---------|---------|
| 擅自改变时间锚点书写格式 | 必须保持 `` `[时间: T-N天]` `` 固定格式 |
| 擅自增加新角色类别 | 必须先扩展 role-btn 路由再写标签 |
| 修改场景标题的领域前缀 | 保持 `## 【党建工作】...` 或 `## 【党务工作】...` |
| 在中间插入步骤而不重新编号 | 后续步骤全部重新编号 |
| 合并多步骤为一行 | 每个需独立过滤的步骤必须单独一行 |

---

## F. 优化提案机制

> 本表单是支委向系统提交规则修改的唯一接口。拒绝形式主义，只需写明"哪里卡壳"以及"期望怎么改"。

### F.1 影响范围 (Scope)

- [ ] **底层架构与原则建议**（Permanent）
- [ ] **全局通用业务规则**（Global）
- [ ] **支委分工与权责调整**（Role）
- [ ] **特定业务场景专用**（Scenario-Specific）

### F.2 现场痛点 (Pain Point)

- **卡壳的 SOP 文件与步骤**
- **实际执行困难**（一句话说明）

### F.3 期望的规则更改 (Proposed Fix)

- **业务建议**（直接给出新规则逻辑）

---

## G. Mock 数据 vs 真实鉴权

### G.1 当前处于 Mock 状态的功能

| 功能 | Mock 方式 | 未来真实方式 |
|------|----------|------------|
| 用户身份识别 | sessionStorage 手动选择角色 | OAuth/SSO 登录后从数据库读取 |
| 赋权记录 | localStorage `_defaultAuth()` Mock 数据 | 数据库 `auth_records` 表 |
| 条条支委权限 | 代码硬编码始终为 true | 数据库查询用户是否持有对应支委角色 |
| 反馈提交人 | 手动输入姓名（选填，默认"匿名"） | 自动从登录 session 读取 |

### G.2 赋权链的 Mock 与真实形态

**当前 Mock 逻辑**：
- 条条支委天然有自身板块管理权限（无需赋权）
- secretary 和 leader 有全域管理权限
- organizer 和 deep 需 leader 赋权
- 当前所有需赋权角色均已预设 Mock 赋权记录

**未来真实逻辑**：
1. 用户通过学校统一认证（SSO）登录
2. 后端根据 uid 查询 `user_roles` 表 → 返回角色列表
3. 支委角色由党委在后台直接指派
4. 活动组织者/深度参与者由党小组组长在系统内赋权产生 `auth_records`
5. 管理模式可进入性由后端鉴权接口实时判定

### G.3 不变的设计原则

- **身份 ≠ 权限**：角色选择和模式选择是两个独立决策
- **条条支委天然权限**：组织/宣传/纪检委员在自己的职能域内无需赋权
- **赋权仅针对活动域角色**：只有 organizer 和 deep 需要被赋权
- **只读模式永不设限**：任何角色都可以只读浏览全部信息

---

*本指南整合了 SOP 优化方法论、网页架构设计、数据映射规则、优化提案机制与 Mock/真实鉴权辨析。2026-05-06 v4.0 更新：全面反映 T25-T29 架构决策——MPA 架构、Mode 统一、三委员看板体系、日历限定、赋权链全量重写、系列活动模型。*
