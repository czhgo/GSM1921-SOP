...
- **模块化分演进**：75 → **78**（批 25–35，测试 541→566）→ **80**（批 37–39，566→569）→ **81**（批 41，569→571）→ **82**（批 42，571→586）→ **83**（批 46①+47A/B/C，586→**624**）；开源化 74 → **75**（批 42，本批未动）。**本批为何只 +1**：口径继续下沉单一源（补戳判据 `version-next.mjs`、普查门槛 S0、单档 `styles.css`）与「索引 ↔ 守卫」双向校验（S9/S10/S11）都是实质加分；但 47-B 实测暴露一处**隐式耦合**——单档口径原**受 CDN Tailwind 加载顺序支配**（继承输给直接声明），说明"单一源"当时并不真的单源，两相抵后记净 +1。**未给更高分的已知短板**：`server/test` **106 个文件 / 17,969 行**未整合（支书 2026-09-15 已点出，见 `REVIEW_QUEUE` 批次 47-F）；综合（五维均值 83/75/75/84/80 = **79.4**）按「未过半分不进位 + 重改进不唯分」仍记 **≈79**。
...
- **测试规模演进（全量实测口径）**：541 → 566 → 569 → 571 → 586（批 42）→ 595（批 43）→ 批 44 新增定向 16 项（`form-loop` 15 + `doc-consistency` S9）→ **624**（批 46①+47A/B/C 实测：46① 新增 6 项、47-A 2 项、47-B 4 项；47-C 只改普查内部断言、未增删用例）；按新协议**不再每批全量复跑**（定向为主、交付前才全量，见 §5.4 与 `README.md` 开发纪律）。
