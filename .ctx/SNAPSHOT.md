...
| `workspace/visitor.html` | 子页面 | 党建·参与者只读（活动动态+专班进展+考勤+日历/列表切换） |
...
| 视图模式三分类 | 管理模式(基类) / 管理者只读(继承-写入) / 参与者只读(独立视图) | CLAUDE.md H2.5 |
