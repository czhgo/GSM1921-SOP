...
date: "2026-09-10"
version: "v23"
milestone: "2026-09-11 续批（第三批）：R-8/R-9 深色模式对比度收口（组件侧 11 文件 + 特批 styles.css 3 处：`.tab-btn-active`/`.is-today`，深色 1.74→8.31、2.28→5.69 等，浅/深两态目标项 0 违规）+ 拖拽调序改 Playwright 真实拖拽机测（16/16，撤销真机手测依赖）+ 名册确权链 API 形态修复（方案 B：新增 `POST /api/v1/members/:id/develop-stage` 语义端点 + 修 person.js name 必填误伤，双形态与越权 403 用例全过）+ 双形态/双主题/机测优先判据入库；乙部仅余 R-10（API 形态其余写链）/R-11（深色第三批）；全量 npm test 457/457；版本串 20260911a / CODE_VERSION 99"
---
...
> **上版**: v22 (2026-09-10)
> **变更来源**: 2026-09-10/11（同日三批，T-077 追加批次 1~4；见 v23/v22/v21 里程碑行；测试/文件计数一律以实测为准）
