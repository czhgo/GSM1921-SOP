...
// 背景（2026-09-13 dogfood 彻查，书记指令「上述问题都不是孤例」）：
//   反复出现三类同源缺陷，本测试以静态扫描锁住，防止再次回潮——
//   ① 面向用户的考核 / 机器话术（KPI、以此计量、完成率）；
//   ② 身份写死（签发落款「书记提醒：」、销项主语写死「书记」、反馈指派用演示占位 ID u_*）；
...
test('② 落款与销项主语不得写死「书记」单一角色（副书同权：应含副书记）', () => {
  // 注：「副书记销项」含「书记销项」子串，故用否定后顾只拦未带「副」的写法
  const BAN = [/书记提醒：/, /由书记销项/, /(?<!副)书记销项/, /由书记在/];
  const hits = [];
  for (const { file, text } of collectSources()) {
    for (const re of BAN) {
  ...
  assert.deepEqual(hits, [], `发现写死单一角色的落款/销项文案（应经当前登录角色或「书记/副书记」表达）：\n${hits.join('\n')}`);
});

// ── ③ 反馈处置 / 我的处置：身份不得用演示占位 ID（u_sec/u_org/u_prop/u_disc/u_leader_*）─
// 判例：书记指派落占位 ID，而「了解进展」请求落真实 personId → 委员/组长收不到；三组长共用 u_leader_1。
