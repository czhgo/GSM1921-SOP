...
import { readLoginSnapshot } from '../core/login-snapshot.js?v=20260912c';
import { ROLE_LABELS, ROLE_PAGE_MAP, getAccentColors } from '../core/constants.js?v=20260912c';
import { resolveAppliedAccentRole } from '../core/theme.js?v=20260912c';
import { appearanceControlsHTML, bindAppearanceControls } from '../components/appearance-controls.js?v=20260912c';
import { icon } from '../core/icons.js?v=20260912c';
import { escHtml as esc } from '../core/utils.js?v=20260912c';
...
} from '../services/preferences.js?v=20260912c';
// 批4（2026-09-09 书记批「域参数」）：制度默认单一源 = policy-defaults（设置页展示「制度默认」行与域参数默认值）
import { POLICY_DEFAULTS } from '../core/policy-defaults.js?v=20260912c';
...
import { registerApiAdapter, setDataSource, init as dataInit } from '../core/data-adapter.js?v=20260912c';
...
import { CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260912c';
...
function loadAuth() {
  if (!_authModule) _authModule = import('../services/auth.js?v=20260912c');
  return _authModule;
}
...
async function buildMyWorkspaceModel(role, personId) {
  ...
  const { applyTabPolicy, getBranchIdOfPerson } = await import('../services/branch.js?v=20260912c');
...
async function renderBranchGovSection(panel, sectionId) {
  ...
  try {
    const br = await import('../services/branch.js?v=20260912c');
    ...
    if (sectionId === 'branch-info-wizard') {
      ...
      await import('../modules/capabilities/secretary-workspace.js?v=20260912c');
...
async function openWizardEmbed(panel) {
  ...
  try {
    const { mountOrgSetupWizard } = await import('../components/org-setup-wizard.js?v=20260912c');
