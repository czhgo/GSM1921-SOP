...
import { getBaseMemberRecords } from './person.js?v=20260914m';
import { DEVELOP_STAGE_OPTIONS, buildPreviewTemplate } from './org-base-data-preview.js?v=20260914m';
...
import { groupOptions } from './party-group.js?v=20260914m';
...
import { RESIDENCE } from '../core/constants.js?v=20260914m';
...
export function buildBranchRosterTemplate() {
  ...
  return {
    ...
    usage: '空支部成员名册包（整支部导入/替换·正式保存）：模板列出成员档案中的现有成员（每行一名），'
      ...
      + 'id 为成员唯一标识不可改（不在现有成员档案中的行整条忽略）；非法枚举回退该成员档案原值、空姓名整条忽略。'
      + '导入 = 净化后预览统计 → 确认后一次保存（仅空支部可用；非空支部请逐人编辑成员档案，勿整表替换）。',
    people: base.people,
  };
}
