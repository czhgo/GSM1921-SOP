...
function _showParticipantModal(act) {
  ...
  card.innerHTML =
    ...
    + '<button class=" text-sm text-white py-2.5 rounded-lg mt-4 w-full transition-colors" '
...
function _showCloseBlockModal(activity, missing) {
  ...
  card.innerHTML =
    ...
    + '</ul>'
    + '<button class="text-sm text-white py-2.5 rounded-lg w-full transition-colors" style="background:#CE1126;">知道了</button>';
  card.querySelector('button').addEventListener('click', () => overlay.remove());
  overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
  card.addEventListener('click', e => e.stopPropagation());
  overlay.appendChild(card);
