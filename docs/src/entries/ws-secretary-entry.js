...
function _ensureSecTabBar() {
  ...
  _secTabBar = renderTabBar({
    prefix: 'secretary',
    tabs: [
      { id: 'todo', label: '待办', groupLabel: '工作台', render: () => import('./tabs/secretary/todo-tab.js?v=20260808h').then(m => m.renderContent()) },
      { id: 'overview', label: '全局概况', render: () => import('./tabs/secretary/overview-tab.js?v=20260808h').then(m => m.renderContent()) },
      { id: 'calendar', label: '活动管理', groupLabel: '党建', render: () => import('./tabs/secretary/calendar-tab.js?v=20260808h').then(m => m.renderContent(getAppState())) },
      { id: 'assign', label: '赋权管理', render: () => import('./tabs/secretary/assign-tab.js?v=20260808h').then(m => m.renderContent()) },
      { id: 'notification', label: '通知发布', groupLabel: '党建', render: () => import('./tabs/secretary/notification-tab.js?v=20260808h').then(m => m.renderContent()) },
      { id: 'feedback', label: '反馈管理', groupLabel: '反馈', render: () => import('./tabs/secretary/feedback-tab.js?v=20260808h').then(m => m.renderContent()) },
    ],
