  ...
  if (DATA_SOURCE === 'mock') {
    ...
    try {
      ...
      if (!mockDB.attendances.length || !mockDB.inspections.length) {
        try {
          const { ATTENDANCE_RECORDS } = await import('../mock/attendance.js?v=20260810a');
          const { INSPECTION_RECORDS } = await import('../mock/inspection.js?v=20260810a');
      ...
      if (!mockDB.todos.length) {
        try {
          const { SEED_TODOS } = await import('../services/todo.js?v=20260810a');
