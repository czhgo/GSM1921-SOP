...
registerCapability({
  ...
  tabs: () => [
    { id: 'todo', label: '待办', groupLabel: '工作台', render: (ctx) => import('../../entries/tabs/visitor/todo-tab.js?v=20260829a').then(m => m.renderContent(ctx)) },
    // 工作概况（书记 2026-08-10 裁定：全部角色新增——汇报/卡点/在办三区总览，参与者仅自我聚合）
    { id: 'overview', label: '工作概况', groupLabel: '工作台', render: (ctx) => import('../../entries/tabs/visitor/overview-tab.js?v=20260829a').then(m => m.renderContent(ctx)) },
    { id: 'projects', label: '项目分工', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/visitor/projects-tab.js?v=20260829a').then(m => m.renderContent(ctx)) },
