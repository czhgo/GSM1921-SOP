...
//   · party-staff / 本支部现任书记 均可写；副书记 403 → 副书同权（2026-09-09 书记批）：本支部副书记同现任书记可写；
//     其它支委（非书记/副）403；name 治理字段不受 config 写口影响
//   · 主题预设白名单净化（非法值忽略）
...
test('org 档案：现任书记/本支部副书记（副书同权）可写本支部 org 域；其它支委 403；越权 name 不生效', async () => {
  ...
  // 副书同权（2026-09-09 书记批）：本支部副书记（p14，br-b1）同现任书记可写 config 域
  const dep = await login('p14');
  const rDep = await patchConfig(dep, { config: { themePreset: 'green', headerTitle: '副书记改的页眉' } });
  assert.equal(rDep.status, 200, '本支部副书记可写 org 域（副书同权）');
  const bDep = await rDep.json();
  assert.equal(bDep.config.themePreset, 'green');
  assert.equal(bDep.config.headerTitle, '副书记改的页眉');
  assert.equal(bDep.name, '光华管理学院本科生党支部', '副书记写亦不影响治理字段');

  const orgc = await login('p11'); // 组织委员（支委但非书记/副）
  const r2 = await patchConfig(orgc, { config: { themePreset: 'sky' } });
  assert.equal(r2.status, 403, '非书记/副的其它支委不可写');
});
