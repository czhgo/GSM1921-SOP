...
>
> 🗣 **黑话审查**（书记指令 2026-08-03 T194 追加，D 表述专项）：重点识别 **AI 自己编造的黑话**——概念过拟合、无依据层级标签、孤立概念、生造术语、同义名词堆砌等。判定标准：① 术语在权威源/上级文件中是否有依据（T1 术语须与原文一致）② 是否可被目标读者直接理解 ③ 是否存在更通俗的同义替代表达。处置：识别出的黑话一律提交书记判定，AI 不得自行代改为自造术语。适用于全仓库用户文档，可多轮复用（T-195）。
...
| T-195 | **黑话审查**（表述维度专项，可多轮）：书记指令（2026-08-03 T194 H5.6 抽样时追加）："我们也要开启一下表述上的 黑话审查，特别是【AI自己编造的黑话】"。审查对象：全仓库用户文档中 AI 编造的黑话（概念过拟合/无依据层级标签/孤立概念/生造术语/同义名词堆砌）。判定标准：① 术语在权威源/上级文件是否有依据（T1 术语须与原文一致）② 是否可被目标读者直接理解 ③ 是否存在更通俗的同义替代表达。处置：识别出的黑话一律提交书记判定，AI 不得自行代改为自造术语。维度已沉淀至 H5.8.2 D 表述（最高权重）专项，可多轮复用 | H5 书记评议 + sample-diff-learning + H5.8.2 D 表述 | 全仓库用户文档 | ⏳ 待启动 |
