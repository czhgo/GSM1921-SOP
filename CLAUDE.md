...
| ID | 事项 | 引用流程 | 状态 |
| --- | --- | --- | --- |
| R-22 | **遗留（系统派生通知的信任边界）**：`notices` 写门的「系统派生」通道以客户端自述字段 `systemDerived` 放行（否则成员提交思想汇报、纪检确认考勤等业务副作用通知在 API 模式被静默拦掉）；该字段客户端可伪造 → 彻底方案＝把这几类通知改为**由所属业务端点服务端派生**（思想汇报提交/考勤确认/赋权等），需界定落点后实施 | SYSTEM_ROLE_PERMISSION §9h + 附录⑦ 写权判据 | 待办 |
