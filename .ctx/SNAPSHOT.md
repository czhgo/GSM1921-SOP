...
date: "2026-09-10"
version: "v21"
milestone: "2026-09-10 增量收口：活动写入改造 S-1~S-4（议程逐人通过/未通过+自动进成员确认链+发展议程简化+默认内容精简+写入 UI 规范）+ 卡片普查与合并（取消 KPI 五连卡·明确不设出勤率 KPI；补 5 处操作入口）+ 待办逐条催办与兜底白名单（副书同权）+ 归属显示时序修复与未登录中性占位 + 门面通用化与功能地图置顶 + 只读端视觉对齐 + 全站卡片对比度收口（WCAG AA 0 违规）+ 黑话第4/5轮与原话复核余批 + 功能实现评议下一轮启动 + 版本串 20260910a / CODE_VERSION 93"
---
...
> **上版**: v20 (2026-09-09)
> **变更来源**: 2026-09-09 晚~09-10 增量（T-076~T-077 区间，见 v21 里程碑行；测试/文件计数一律以实测为准）
