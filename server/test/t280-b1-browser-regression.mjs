...
// ── U1 通知阅读待办直达（leader，构造 actionable 通知）──
{
  ...
  await page.evaluate(async () => {
    const { NoticeTodoDeriver } = await import('/src/services/todo.js?v=20260829h');
  ...
  await page.evaluate(async (id) => {
    const { TodoStore } = await import('/src/services/todo.js?v=20260829h');
    TodoStore.delete(id);
  }, createdId);
...
// ── U2b 报名审核直达（org 构造 act-15 报名待办 → activity.html）──
{
  ...
  const todoId = await page.evaluate(async () => {
    const { TodoStore, TodoCategory, TodoActionType, TodoSourceType } = await import('/src/services/todo.js?v=20260829h');
...
// ── U3 组长赋权待办直达（构造活动创建赋权待办 → write tab + act-1 详情）──
{
  ...
  await page.waitForTimeout(2200);
  const todoId = await page.evaluate(async () => {
    const { LifecycleTodoDeriver, TodoStore } = await import('/src/services/todo.js?v=20260829h');
