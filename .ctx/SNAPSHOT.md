...
date: "2026-09-13"
version: "v31"
milestone: "2026-09-13 续批（第十八批）：三向失同步彻查 + R-23 ① 闭环。书记三问（书记界面为何出现「书记的催办」/ 不得出现「KPI 以此计量」/ 解决 R-23 思想汇报通知判定）＋「上述问题都不是孤例，必须彻查」。**总根因**＝通知消费端（铃铛/首页未读/角标/待办未读条）从未按受众过滤（只特判 audience==='committee'）→ 签发人收到自己下发的催办；通知发布页所选受众数组从未生效。**修**：NoticeStore.list 统一三档受众门（committee / audience 角色数组 / actionRoles 目标角色）；催办落款去硬编码「书记提醒：」+ 签发人取真实角色；清用户可见 KPI/以此计量/完成率（概况改「待归档材料 N 条（只报缺口）」）；R-23 ①＝思想汇报建服务端表 thought_reports + authorize 按表复算 + 新增 flushSnapshot + 受众锁定组织委员；同类彻查＝反馈指派占位 ID（u_*）全量迁真实成员 ID（含种子 issues.json 与 CACHE_VERSION 4）、4 台「我的处置」取真实 personId、issues 处置审计 authorRole 取真实角色、日历 createdBy 去 u_sec、销项文案改「书记/副书记」；防回潮＝新增静态哨兵 server/test/ux-guard.test.mjs（5 项）。验证：定向 22/22 + ux-guard 5/5；全量 482 用例 481/482（唯一失败＝multi-user-write 并发写，单独复跑 2/2 全过＝串行满负荷时序抖动）；真机串行 dogfood 7 项全 PASS。新登记 R-24（API 模式反馈链路两处失同步）。版本串 20260912k / CODE_VERSION 131"
---

# System Snapshot — v31

> 当前活跃基线。历史快照见 `.ctx/snapshots/`。
> **生成（v31）**: 2026-09-13 续批 — 第十八批（三向失同步彻查 + R-23 闭环，权威见 2026-09-EXECUTION_LOG.md 追加批次 18）：**来源**——书记三问（① 书记界面为何出现「书记的催办」＝内控漏洞？② 不得出现「KPI 以此计量」；③ 解决 R-23 思想汇报通知判定逻辑）＋「上述问题都不是孤例，必须研究彻查」。**① 通知消费端受众门（总根因）**：实报真相＝**不是自己催自己**，而是 `NoticeStore.list()` 只特判字符串 `audience==='committee'`，**从未按 `audience` 数组/`actionRoles` 过滤** → 签发人收到自己下发的催办（铃铛未读 +2）；且通知发布页所选受众（`audience` 数组）**从未生效**（选「党小组组长」实际全员可见）。修＝`list()` 统一三档受众门（`committee` 本支部支委层 / `audience` 角色数组 / `actionRoles` 目标角色；无受众＝广播；无会话不收窄）。**② 签发人身份**：催办落款去掉硬编码「书记提醒：」，`actorRole`/`publishedBy` 改取当前真实角色（书记/副书记，修「副书记签发被记成书记」）。**③ 术语**：清用户可见「KPI／以此计量／完成率」——概况指标「归档完成率（%）· 目标 100%」→「待归档材料 N 条（只报缺口，不设比率目标）」、`review.js` 与 `help.html` 同族扫净（另 help.html 黑话 留痕→记录／降级→显示／兜底→备用入口／应到口径→应到名单规则）。**④ R-23 ① 闭环**：思想汇报建服务端表 `thought_reports`（db.js/`resources.js` 映射/`api-adapter`/快照 payload/`init` 拉取），`thought-report-submitted` 的 `authorize` 改**按表复算**（表内无该汇报→403；提交人本人或组织委员→放行），前端新增 `flushSnapshot()` 确保提交先落服务端表再触发通知；该通知**受众锁定组织委员**（原无受众＝全站广播）。**⑤ 同类彻查（身份不同源）**：反馈指派由演示占位 ID（`u_sec`/`u_org`/`u_leader_1~3`）统一迁到**真实成员 ID**（`p13`/`p11`/`p1~p4`；`feedback-tab` 动态按角色解析 + 4 台「我的处置」取 `AuthStore` 真实 personId），修掉「委员/组长收不到『了解进展』请求」「三组长共用 `u_leader_1`」两处真实功能缺口；种子 `docs/data/issues.json` 同步迁移 + `CACHE_VERSION 3→4` 强制清旧缓存；`issues.js` 处置审计 `authorRole` 由写死 `'secretary'` 改取真实角色（副书记不再被记成书记）；日历 `createdBy` 去 `'u_sec'`；`resolution-followup`/help 销项文案「书记」→「书记/副书记」（副书同权）。**⑥ 防回潮**：新增静态哨兵 `server/test/ux-guard.test.mjs` 5 项（用户可见话术黑名单／落款销项主语／反馈占位 ID／通知受众门／R-23 按表复算）。**验证**：定向 22/22 + ux-guard 5/5；全量 **482 用例 481/482**（唯一失败＝`multi-user-write` 并发写，**单独复跑 2/2 全过** → 串行满负荷时序抖动，与既有 R-5 同类，据实留痕不假绿）；**真机串行 dogfood 7 项全 PASS**——催办后书记铃铛 3→3（不回自己）且 `actionRoles=['leader']/['org-commissioner']`、概况正文与 tooltip 无 KPI/完成率、反馈指派目标显示「组织委员（高翔宇）」等真实姓名（DOM 无 `u_org`）、组织委员「我的处置」收到指派、思想汇报通知**仅组织委员可见**（提交人 p3 与纪检 p10 铃铛均无）。**新登记 R-24**（API 模式反馈链路两处失同步：书记台反馈空 0 条 vs 公开页 4 条；草稿「通过」不落库）。版本串 **20260912k** / CODE_VERSION **131**
> **生成（v30）**: 2026-09-12 续批 — 第十批（dogfood 七身份体验问题全修，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 11）：**走查** 7 身份串行真机 dogfood（书记/组织委员/纪检委员/宣传委员/党小组组长/普通党员/党委组织员，浅色+深色各一遍，只读全域点击），产出 **71 条真实复现问题** → 去重 **12 类系统性缺陷 S1~S12 + 单点**；书记裁定「**全修 S1~S12 + 单点**」。**S1~S12**：S1 通知详情单一取数口（先 hydrate 再按 id 取；派生通知按 id/sourceId/noticeId 从待办现算重建，确认读取兜底销待办）→ 消除全部入口「通知不存在或已过期」；S2 `?tab=` 深链与内容区一致（消除「记忆 tab 异步回写覆盖 URL tab」竞态）；S3 待办条数与概况/角标/首页未读同源；S4 「今天」落点不空；S5 党委只读下钻同步封死写入口；S6 宣传「去归档」落点 + 归档页筛选后按钮（事件委托）；S7 深色徽章色义恢复（裸 `.badge` 深色规则限定 `:not([class*="badge--"])`，**样式表特批**）；S8 空白 Toast（下发/上报恢复文字）；S9 统计行可下钻；S10 人数口径同源（本支部在册）；S11 提交校验与反馈；S12 危险操作二次确认。**单点**：反馈 `undefined` 兜底+条目键盘可达、首页登录态标题、人才库搜索筛选、通知行键盘可达、活动草稿跨刷新、周报周次派生+倒序、专班截止态、组长上传下拉排除已取消并标「已上传」、看板卡与活动列表行键盘可达、组长台副标题「复盘提交」→「复盘状态查看」（对齐组长复盘只读裁定）。**验证**：module-load 1/1；**全量 475/475 全过**（此前一次连续跑出现 4 项 `page.goto` 超时，单独复跑 16/16 全过=串行资源竞争抖动）；真机回归 13 项 + 定点复验 4 项全 PASS。**乙部**：R-23（R-22 收口后残留：思想汇报无服务端表 → 只能按提交人判定；部分 kind 展示值取自 payload；通知草稿类标题正文由发布人填）。**批次 16「触点即落点」全局 UX 反思**：书记实报「组长活动查询点开后详情出现在最下方」→ 两路独立 dogfood 走查收敛出**系统性模板问题**（共用单例详情容器固定在列表末尾），命中 **7 处**（组长台活动管理 `#leader-act-detail` 点首条需下滚 ~1684px；专班查看/管理 `#tfv-detail`＝**5 台共用**；组织台考察上传；书记台反馈管理与公共反馈页＝视图切换后详情在列表上方需反向找；成员台复盘行内展开超视口；书记台活动查询折叠仅露 25px）；**统一修复**＝新增 `docs/src/components/detail-anchor.js`（`anchorDetailToTrigger`／`scrollDetailIntoView`）并改造 7 处 → **真机复核 8 项全 PASS**（组长台详情由 doc y 2030 → 紧贴卡片 y 416、随触点移动、收起重开不错位；其余各处均在视口内；四页无红错）。**批次 17 R-22 闭环**：系统派生通知**收归服务端**——新增 kind 注册表（19 类，`authorize` 服务端复算 + `build` 服务端生成落点）、端点 `POST /api/v1/system-notices`（201/403/400/401）、前端 `addSystem`、15 处调用点迁移、关闭旧 `systemDerived` 通道、文案单一源 `system-notice-templates.js`；**集成回归已修**（API 模式只写服务端导致下发历史/铃铛/待办看不到 → 改为同步本地镜像 + 成功替换/失败回收，两文件复跑 2/2 通过）；探针取真值确认服务端 201/403/400 各就位。版本串 **20260912i** → **20260912j** / CODE_VERSION **120**；**权限专项（批次 14）**：制度↔代码↔界面三向比对（8 角色×10 动作）产出 9 项不一致；真机 API 实证三处缺口——普通成员可建「支委会」活动（201）、可改 `voteConfig.voterIds` 篡改表决名单（200）、副书记截止表决被 403；已修：**后端活动写门**（非支委层 403、组长限党小组会/主题党日）、**副书同权补齐**（表决截止 + 通知发布白名单 + 通知落款不再硬编码「书记」）、**组长上传位加「本组」约束**（考勤/考察，`_activityPartyGroup`＝hostGroup 优先、缺省回退组织者小组）、**纪检兼组织者上传位补齐**、**组长改派支部级活动角色封口**（按钮禁用+拦截）、**mock 表态补应到名单校验**；**R-16 闭环**（服务层透出 `activityId` + 台账「活动」列可点 + 台账活动名改 mockDB 优先，修掉 API 模式显示 `act-xxxx`）。**书记裁定**：R-17 维持现状（仅正式表决无记名）、活动创建权保持现状（支委层保留）、P.14 暂不推送。**权限体验真机走查（7 项）**：PASS 5（组长上传位＝本组/支部级活动角色只读/台账行可点/普通成员无建活动入口且活动详情仍可表决/副书记发通知落款正确）；FAIL 2 当场复修——副书记看得到「表态汇总」但「截止表态」按钮**不渲染且无提示**（前端 `canLock` 未随 server 同步）→ 改 `canLock` 为书记/副书记 + 无权限时明示「截止表态由书记/副书记操作」；组长上传下拉在第二/三党小组**空且无说明**（种子仅覆盖第一党小组）→ 补空态说明并登记 R-21。版本串 **20260912f** / CODE_VERSION **116**
> **生成（v29）**: 2026-09-12 续批 — 第九批（列表可点扫修 + 意见反馈真匿名 + 正式表决无记名，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 10）：**A. 卡内实体条目可点扫修**（书记裁定「能改就改」；判据由 **≤1 跳放宽为 ≤2 跳**）：审计出 **9 类**不可点实体条目并全部补入口与落点——书记台「**活动查询**」行→`activity.html?id=`（实证落到「9月支部党员大会」）、党委监控近期活动、宣传台归档/待归档行、党委支部卡补「进入支部」（`branch-demo-nav`）、**人才库成员卡→名册 1 跳**、纪检活动流程监督行、书记台党小组进展待复盘行、组长组员进展待复盘行、参与者我的考察行；非缺口 9 项逐条注依据。**B. 意见反馈真匿名**：新增 `issues` 表 + `GET/POST /api/v1/issues`（PATCH 仅书记）；**不存任何可反查提交人的字段（书记亦不可见）**，仅存随机 token 的 `tokenHash`（无 personId、无盐）用于判重/限频；**处置结果公开发布**、取消定向回推；历史 `_realPersonId` 读取时清除；新增 `issue-anonymity.test.mjs` 6/6。**C. 正式表决无记名 + 匿名模式可选**：`voteConfig.ballotMode: named|anonymous`（发起时选定、同场统一；**制度明文 formal 场景强制锁定 anonymous**，UI 只读 + 服务端改 named→400）；无记名数据两段式（**参与记录** personId+votedAt 供催办与人数核验 + **tally** 计数行，**逐人选项不落库**）；书记端矩阵改「已投/未投 + 汇总」、门槛改参与记录+tally；扩展表决测试含「无 personId→选项映射」匿名性断言。**判据入库**：附录⑦ 修订「操作入口判据」（≤2 跳 + 卡内实体条目必须可点）+ 新增「**匿名判据**」（真匿名边界 / 无记名两段式 / 强制锁定 / 其余实名）。**乙部**：余 R-16（台账行可点残余）/ R-17（无记名延伸范围待裁）；版本串 **20260912a** / CODE_VERSION **111**
> **生成（v28）**: 2026-09-12 续批 — 第八批（R-15 深色收官 + 黑话第 7 轮，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 9）：**A. R-15 深色模式收尾轮（书记指令「快速收尾」）**——`docs/src/about.css` 深色 token `--ab-ink-faint #7E7870`→`#948E85` + 新增深色分支（`--ab-red`/`--ab-red-deep` 口径）：目标 `.ab-tl-time` 4.15→**5.58**、`.ab-tl-detail-article` 3.82→**5.14**、`.ab-exploration-scene-note` 4.15→**5.58**；同族抽样 11 项全部 ≥4.5（tag 3.22→6.50、answer 2.29→5.68、insight 2.37→5.88 等），**浅色 21 项逐项零变化**；深色违规 67→5（余为装饰编号，标注豁免）。**收官声明：深色对比不再开新轮**。**B. 黑话审查第 7 轮**（6 候选逐条裁定）：①**回潮·高**「漂移」→「**失同步**」（67 处 / 40 文件，含门面 README/CONTRIBUTING、.ctx 文档、content 十余篇、docs/src 与测试用例名；入库防回潮）；②向导页（部署/换壳工具）**只去内部术语、保留命令**：「验证点」→「检查项」、「写库」→「写入系统」、「留痕」→「记录变更」（保留 `npm test`/`?reset=1`/清单/编号）；③「幂等」→「重复提交自动覆盖上次选择」；④「落库」→「保存/写入系统」+ 同族「口径」**29 处**扫净；⑤空态「时间锚点」→「该场景暂无需按时间安排的事项」；⑥「用户第一接触点」→「用户最先看到的内容」。**判据入库**：附录③ 第 7 轮记录 +「漂移→失同步」；附录⑦ 追加「深色对比收官」。**乙部：当前无未闭环项**；版本串 **20260912a** / CODE_VERSION **109**
> **生成（v27）**: 2026-09-11/12 续批 — 第七批（R-14 样式表专批，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 8）：**书记专批范围逐条**：**特批 1** `docs/src/styles.css` 参考文献灰字 `.ref-site-desc`/`.ref-doc-meta`/`.ref-doc-size` `--neutral-400`→`--neutral-500`（浅 2.54→**4.83**，深色 6.96 不退化）；**特批 2** `docs/src/about.css` 浅色族——`--ab-ink-faint #94897A`→`#6C6254`（`.ab-page-runner` 3.24→5.64、`.ab-tl-time` 3.24→5.64、`.ab-tl-detail-article` 3.37→5.87、`.ab-exploration-*` 3.24→5.64）、`.ab-tl-stage` `#ABA191`→`var(--ab-stage-from,#756B5D)`（2.40→**4.93**）、`.ab-tl-no--decision`→`#7C5C14`（2.28→**5.83**）+ 深色分支防回退；装饰编号 `.ab-tl-no`/`.ab-review-card-no` **豁免不动**；**特批 3** status-badge pending 双态分档——浅 `#7A838F`（3.39→**4.84**）/ `html.theme-dark` `#AEB6C2`（6.44→7.37）。**控制组 20 行零变化**（ctrlDiff=0），console 0 error，module-load 148/148 + link-integrity + help-e2e 全绿。**生命周期退出**：R-14 闭环退出乙部，新立 **R-15**（深色 `--ab-ink-faint:#7E7870` 在 3 个无深色分支 `ab-*` 元素 4.15/3.82/4.15，需另行专批）；版本串随当日日期滚动 **20260912a** / CODE_VERSION **107**
> **生成（v26）**: 2026-09-11 续批 — 第六批（R-13，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 7）：**R-13 浅色对比存量核销**。**定性（只读探针：7 工作台全 tab + 11 根页，浅色态）**：灰字 `#9CA3AF` 命中 296 = **真违规 68 / 伪影 228**（折叠侧栏 225 + `display:none` 3）；浅色全集 <阈值 369 = 真违规 122 / 伪影 247——**约 3/4 为「默认折叠侧栏」伪影**（深色轮次未按此归类，口径差异据此说明），真违规约 1/4。**收口（组件侧，不擅改样式表）**：`help.html` `.help-card-rel-line` 38 处 2.54→4.83；`dashboard/stats.js` 专班大字 2.15→3.19、零值/单位 2.54→4.83；`text-gray-400→text-gray-500` 7 处 2.42→4.62–4.83；`settings.html` caption/激活项/`.appearance-note` →4.62–8.70；**同族扩散扫清**（sidebar/根页/header/references/settings-entry/custom-select/person-picker ~30 处）——浅色 **2.15–2.54→3.19–8.70**，**深色同项零退化**，豁免项（装饰/图标/禁用/骨架/深色覆盖）注明依据；**浅色剩余真违规仅 12 处＝登记项**。**口径入库**：伪影判定（折叠侧栏隐藏/`display:none`/瞬时/装饰/遮挡）与「浅色存量核销」结论。**乙部**：R-13 闭环退出，仅余 **R-14**（需改样式表：styles.css `.ref-*`、about.css 浅色约 52 处、status-badge pending 双态单 token 议题——**待书记专批**）；版本串 **20260911a** / CODE_VERSION **105**
> **生成（v25）**: 2026-09-11 续批 — 第五批（R-12 + 黑话第 6 轮，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 6）：**A. R-12 深色对比第四轮（组件侧）**——全量扫描 7 角色全 tab + 11 根页浅/深两态，修 10 文件（assign 头像、archive 当前页、settings 外观、wizard/党委 config、prop weekly、secretary feedback 等同族）；发现并**特批修 styles.css 真缺陷**（L4303 注释内 `*/` 提前闭合致深色 `.doc-card` 整条失效 → help td 1.48→11.35；另新增深色 `.help-toc-item` 2.48→12.61）；**再特批** `about.css` 两处（`.ab-page-runner` 4.15→5.55、`.ab-chapter-eyebrow` 3.22→6.51，装饰编号豁免）。**B. 黑话审查第 6 轮**（书记逐条裁定 6 候选）：①UI「闭环」**回潮清理** 13 处→办结/流转完成/标记已外发（保留授权「确认闭环」）；②去「落地抓手」整句重写，并衍生**书记新裁定「数字不硬编码」**；③「颗粒度」→「分层」（3 文档 7 处）；④设置页去工程语 12 处（`= policy-defaults`/`（why）`/`config.modules`/L2/L3 → 用户语言）；⑤「范式」→「做法/标准」（注释+文档+help）；⑥「确权」**回潮**→「成员变更确认」族，并按「全清同族」扫净（确权 0 残留；范式仅余库恩式学术引文与判例本身）；**硬编码计数清扫**（8 条真风化 + 同族残留 → 动态口径，help-e2e/function-map-sync 断言同步校准）；**判据入库**：附录③ 三条（颗粒度→分层 / 范式→做法 / 数字不硬编码 + 确权回潮）。**乙部**：R-12 闭环退出，仅余 **R-13**（浅色灰字 245 处存量待核：真违规/探针伪影）；版本串 **20260911a** / CODE_VERSION **103**
> **生成（v24）**: 2026-09-11 续批 — 第四批（A~D，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 5）：**滞留清理（书记裁定）**——乙部只保留未闭环可执行项，完成项与「闭环注记」一律撤出（权威登记=执行日志+REVIEW_QUEUE），如需再启动须**重新注册**新条目/轮次并标注；撤出乙部 4 条历史完成注记（T-279/280/282/283）与闭环记录块、丙部 2 条已完成行（T-281/T-237）；**A. R-10 全补三条写链**（API 形态语义端点，均含同支部 + 字段白名单 + 注入 role/branchId→400 / 越权→403：`POST /members/:id/residence-status`（书记侧）、`PATCH /members/:id/profile` + `POST /members`（组织委员，阶段字段仍唯一走 develop-stage）、`POST /members/:id/transfer-out`（软标记转出、原行保留）；`users` 写权矩阵零改动；客户端 api 分支分流接线，mock 不变）；**副书同权裁定**：书记侧三端点 + `member-change-requests/:id/confirm` 纳入（单一源 `SECRETARY_AND_DEPUTY_ROLES`；`approve` 与 `profile`/`POST members` 保持专属，依据 §9b 并列独立角色）；**B. R-11 深色第三批**（组件侧 12 文件：`text-gray-500` 族 1.37→5.90、`bg-cyan-50 text-cyan-700` 1.39→8.83、`text-amber-800` 1.95→8.27、「·我」2.27→6.74；豁免=头像首字/向导 disabled；未提出 styles.css 提案）；**C.** 新一轮评议**暂不启动**；**D.** 收口。**乙部**：R-10/R-11 闭环退出，仅余 **R-12**（深色第四轮：未扫描页分数浅底类残留）；版本串 **20260911a** / CODE_VERSION **101**
> **生成（v23）**: 2026-09-11 续批 — 第三批（A→E，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 4）：**A. R-8**（深色第一批：`.btn-accent-soft`「催办」×4 + 浅底分工标签×3 → 组件侧 `--acc-text-dark` color-mix 覆盖，深 2.52→5.78 / 2.43→6.92）；**B.** 拖拽调序改 **Playwright 真实拖拽机测**（拖动/重排/持久化/核心锁定/越界回滚 + console 0 error，16/16，实现无缺陷）；**C.** 名册成员变更确认链 **API 形态修复（方案 B）**：新增语义端点 `POST /api/v1/members/:id/develop-stage`（限书记 + 同支部 + 字段白名单；越权 403、注入 role 400），客户端 api 分支改调，并修 `person.js` name 必填误伤局部更新（双形态实证：书记 200 落库、mock 不退化）；**D. R-9**（深色第二批：组件侧 10 文件 + **特批 styles.css 3 处**：`.tab-btn-active` 浅/深分支 color-mix、深色 `.is-today` 覆盖 → 深 1.74→8.31；浅/深两态目标项 0 违规）；**E.** 新一轮评议**暂不启动**（排期表保持为空）；**判据入库**：双形态 + 双主题 + 机测优先（附录⑦）。**乙部**：R-8/R-9/C 专项闭环退出，新立 R-10（API 形态其余写链 403 隐患）/R-11（深色第三批残留）；**全量 `npm test` 457/457 全过**（起 3000 服 + 干净演示库）；版本串 **20260911a** / CODE_VERSION **99**
> **生成（v22）**: 2026-09-10 续批 — 同日第二批（A→E 全链，权威见 2026-09-EXECUTION_LOG.md T-2026-09-077 追加批次 3）：**A. R-6 五项功能遗留全闭环**（①通知对象级深链：11 个生产点补 targetUrl/targetType+targetId + 静态守卫；`workspace-shell` 支持 `?tab&highlight` 定位与高亮 ②代归档：放行书记/副书记进入宣传台**仅归档兜底面** + 归档 tab 增「待归档」区 ③代填复盘：复盘表单下沉 `services/review.js` 单一实现，书记入口复用同表并留痕「【由党支部书记代填】」；顺修 bootstrap 调未定义函数白屏 bug ④催办文案定稿=中性事务式（〈业务域·事项〉+ 时限），待办页与概况催办同源 ⑤党委「进入支部」放开 API 会话下钻，**只读**横幅 + 写权零放宽）；**B. R-7**（styles.css **特批**最小改 4 行：`.ov-sub-tab-active`/`.tint-pill` 文字色 color-mix 深档 + 深色分支；浅/深两态 0 违规）；**C. 最小三成本第 6 轮 6 条**（发展数据只读化+名册补阶段日期 →「唯一写位」；组长建活动默认预选+高级折叠，点击 7→3；无真实日期不显示假值；归档上传外发改行内可选；党委总览披全院数字/台账披支部明细；周报周次与起止自动派生）；**D. 门面口径统一**（8 页 title + 2 处页脚 + help 正文「示例组织（光华管理学院本科生党支部）」）；**E. 生命周期退出**：R-6/R-7、C①~C⑥ 全部从乙部退出，乙部仅余 **R-8**（深色模式 7 处其它类残留）；版本串 20260910a / CODE_VERSION **97**
