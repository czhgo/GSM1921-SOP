...
| `index.html` | `web` | **Interactive SOP Web View entry point (v6.0 Single-File Architecture).** Pure static SPA. CDN: Tailwind CSS v3 Play. Fixed red-gradient header. Left sidebar with role-filter buttons. Main timeline-based WWH (Who/What/How) view. Zero fetch(), zero marked.js, zero table elements. Business data hardcoded; JS state machine (SSOT {domain,role}) in inline script. Injected `sopDatabase` const (10 scenarios from 常见工作场景快速指南.md v1.7). | SPA, 单文件架构, 党建红, 时间轴, 角色切换, sopDatabase |
...
| File | YAML type | Abstract | Key Content Tags |
|------|-----------|----------|-----------------|
| `docs/党支部管理与实务经验沉淀.md` | `experience_doc` | Management experience knowledge base for cross-term handover. 3 modules: org structure reform, process optimization, digital collaboration. 5-item decision anti-patterns appendix. | 管理经验, 换届交接, 组织架构, 流程优化, 人机协同 |
| `docs/SOP优化提案反馈卡.md` | `feedback_card` | **Feedback submission form for SOP optimization proposals.** §1 Scope (4-dimension taxonomy: Permanent Principle / Global Constraint / Role/Delegation Constraint / Scenario-Specific Constraint). §2 Pain point (file path + execution difficulty). §3 Rule change suggestion. §4 AI processing area (auto-filled). | SOP反馈, 提案卡, 四维分类, 优化建议 |
...
| `参考资料/党小组会/党建活动SOP流程图-初版-白文琦&孙睿.pdf` | PDF | Initial draft SOP flowcharts by 白文琦 & 孙睿. **Superseded** — static flowchart era has ended; Web UI will replace. | ~~Superseded by 工作流程图-定人定责定岗.md~~ — that file has been deprecated in Session 23. | ⚠️ Not directly readable. **Archived, not operationally relevant.** |
...
| `申报材料模板/工作记录类/党支部工作记录.docx` | DOCX | Standard **三会一课 work record form** (.docx). The mandatory template for all 三会一课 archiving. Cannot be AI-read but its existence and usage instructions are documented throughout the SOP. | Usage documented in: `流程指南/宣传委员工作流程指南.md` §2, `流程指南/常见工作场景快速指南.md` 场景1A | ⚠️ Not directly readable. **Operational reference only — AI cannot validate form content.** |
...
| G3 | ~~`_quick_cards/` 整目录及 `流程指南/工作流程图-定人定责定岗.md`~~ | **✅ 已出清（Session 23）** — 静态速查卡与流程图已全部删除，由未来 Web 视图替代 | — |
