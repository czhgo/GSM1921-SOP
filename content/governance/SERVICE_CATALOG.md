...
>
> 引用流程：H8.1（党建与党务工作理论）→ H8.3（专班）→ H8.5（赋权关系链）→ [DATA_ARCHITECTURE.md](../design/DATA_ARCHITECTURE.md)（§登录态打桩设计）→ [COMMISSIONER_FRAMEWORK.md](../strategy/COMMISSIONER_FRAMEWORK.md) → [DATA_ARCHITECTURE.md](../design/DATA_ARCHITECTURE.md)（登录系统设计前置）
...
> 角色权限依据 `docs/src/services/auth.js` 中 `ViewModeStore` 的 `canManage / canWriteActivity / canRecruitTaskForce / canAuthorize / canInitiateTaskForce` 等判定函数，以及 [ROLE_CLASSIFICATION.md §9b](ROLE_CLASSIFICATION.md)、[COMMISSIONER_FRAMEWORK.md §C](../strategy/COMMISSIONER_FRAMEWORK.md) 的权限矩阵综合得出。

---

## 一、服务分类框架

系统服务按党建与党务工作理论（H8.1）分为三大类：
...
- **角色权限**：
  ...
  - 深度参与者：自身（仅可见自己参与的活动）
  - 普通成员：只读（公开信息）
- **关联制度**：[COMMISSIONER_FRAMEWORK.md §审批流程规范 §一](../strategy/COMMISSIONER_FRAMEWORK.md)（活动创建审批，自上而下无需审批 / 自下而上需审批）
...
- **角色权限**：
  ...
  - 党小组组长 / 组织者 / 深度参与者：只读
  - 普通成员：无
- **关联制度**：[COMMISSIONER_FRAMEWORK.md §A.4~A.8](../strategy/COMMISSIONER_FRAMEWORK.md)（专班核心定义与生命周期）、[COMMISSIONER_FRAMEWORK.md §审批流程规范 §二](../strategy/COMMISSIONER_FRAMEWORK.md)（专班创建审批）
...
- **角色权限**：
  ...
  - 普通成员：无
- **关联制度**：[DATA_ARCHITECTURE.md §1.2](../design/DATA_ARCHITECTURE.md)（数据流步骤 ①~⑨）、[FLAT_DESIGN.md](../strategy/FLAT_DESIGN.md)（组织者与深度参与者扁平化）
...
- **关联制度**：[COMMISSIONER_FRAMEWORK.md §C.1a 考察管理](../strategy/COMMISSIONER_FRAMEWORK.md)、[DATA_ARCHITECTURE.md §五](../design/DATA_ARCHITECTURE.md)（考勤 vs 考察维度）
...
- **角色权限**：
  ...
  - 深度参与者：只读（接收交接）
  - 普通成员：无
- **关联制度**：[FLAT_DESIGN.md](../strategy/FLAT_DESIGN.md)（组织者与深度参与者分工交接）

### 2.6 考勤管理服务

- **所属类别**：党务工作
- **功能描述**：记录党员 / 预备党员的出勤 / 请假 / 缺勤状态（0-1 变量，基础层），由纪检委员确认后录入总表。
...
- **角色权限**：
  ...
  - 深度参与者：只读
  - 普通成员：自身（查询自己历次活动考勤）
- **关联制度**：[COMMISSIONER_FRAMEWORK.md §C.1a 考勤管理](../strategy/COMMISSIONER_FRAMEWORK.md)、[COMMISSIONER_FRAMEWORK.md §审批流程规范 §四](../strategy/COMMISSIONER_FRAMEWORK.md)（考勤确认审批）
...
- **关联制度**：[纪检委员工作流程指南](../sop/纪检委员工作流程指南.md)（补课制度母本）
...
- **关联制度**：[组织委员工作流程指南](../sop/组织委员工作流程指南.md)、[COMMISSIONER_FRAMEWORK.md §C.1b](../strategy/COMMISSIONER_FRAMEWORK.md)
...
- **角色权限**：
  ...
  - 纪检委员：只读（制度文件引用区只读）
  - 党小组组长 / 组织者 / 深度参与者 / 普通成员：无
- **关联制度**：[宣传委员工作流程指南](../sop/宣传委员工作流程指南.md)、[COMMISSIONER_FRAMEWORK.md §C.1b](../strategy/COMMISSIONER_FRAMEWORK.md)
...
- **角色权限**：
  ...
  - 深度参与者 / 普通成员：无
- **关联制度**：[COMMISSIONER_FRAMEWORK.md §审批流程规范 §五](../strategy/COMMISSIONER_FRAMEWORK.md)（复盘审批）、[COMMISSIONER_FRAMEWORK.md §C.1a 活动监督复盘](../strategy/COMMISSIONER_FRAMEWORK.md)
...
依据 [DATA_ARCHITECTURE.md §8.2 党建与党务同源说明](../design/DATA_ARCHITECTURE.md)，党建板块与党务板块在后台指向同一份数据（`mockDB` / `ActivityRecordStore`），差异仅在于展示切面：

| 数据源 | 党建工作台展示切面 | 党务管理展示切面 |
|--------|------------------|----------------|
| `mockDB.attendances` | "你的出勤记录"（服务同学们的个人视角） | "全支部的考勤分析"（供支委会决策的管理视角） |
| `mockDB.inspections` | "你的考察记录"（个人参与视角） | "全支部的考察汇总"（纪检确认 + 组织建档） |
| `mockDB.taskforces` | "活跃专班进展"（组织者执行视角） | "专班名单同步 + 工作量"（纪检监督视角） |

### 4.4 赋权链与权限传导

赋权关系链（`AUTHZ_CHAIN`，源自 `auth.js`）决定视图可见范围与模式推导：
...
宣传委员 ⇄ 纪检委员 ⇄ 组织委员（三委员同级互看，管理者只读）

> **核心原则**：赋权本身不需要审批（[COMMISSIONER_FRAMEWORK.md §审批流程规范 §三](../strategy/COMMISSIONER_FRAMEWORK.md)），但赋权范围受限于赋权者的权限边界。组织委员只能在已获批专班中赋权；党小组组长只能在自己创建的活动内赋权。
...
- [DATA_ARCHITECTURE.md](../design/DATA_ARCHITECTURE.md) — 参与者数据流设计（数据流 + §登录态打桩设计）
- [ROLE_CLASSIFICATION.md](ROLE_CLASSIFICATION.md) §九 — 角色权限矩阵（权限矩阵权威源）
- [DATA_ARCHITECTURE.md](../design/DATA_ARCHITECTURE.md) — 登录系统设计前置规范（原 LOGIN_STUB §六~§十一）
- [COMMISSIONER_FRAMEWORK.md](../strategy/COMMISSIONER_FRAMEWORK.md) — 支委系统设计（专班 + 权限矩阵 + §审批流程规范）
- [ROLE_CLASSIFICATION.md](./ROLE_CLASSIFICATION.md) — 文件角色分类体系定义
- [FLAT_DESIGN.md](../strategy/FLAT_DESIGN.md) — 组织者与深度参与者的扁平化设计
