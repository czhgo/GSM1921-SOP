...
| server/routes/resources.js | 2026-09-21 | — | [工程师]+[AI] | 25 资源 list/create/update + bootstrap + snapshot（2026-09-21 批次 120：`COMMISSIONER_WRITE` 加 `fileSpaceRecords` / `imageRecords`——上传口元数据写口收进支委层，回应 `D-448` ④） |
...
| server/test/form-loop-registry.mjs | 2026-09-21 | — | [工程师]+[AI] | 校验点台账（2026-09-21 批次 129 **补登**——本表原先无此行；本批 8 条 `line` 按实文同步`R-80`：`inspector.js` 4 条 · `references.js` 4 条） |
...
| docs/src/components/inspector.js | 2026-09-21 | — | [工程师]+[AI] | 检查器组件（产出物区/分类型关闭条件；2026-09-21 批次 129 议程行新增「报送党员大会表决」勾选位与「已定：报送党员大会表决」显示，随行位移台账 4 条行号同步） |
...
| docs/src/services/agenda-follow-up.js | 2026-09-21 | — | [工程师]+[AI] | 议程跟办服务（2026-09-21 批次 129 **补登**——本表原先无此行；本批「讨论文件」分支按 `purpose:'institution'` 分流到制度链，`recordAgendaResult` 增 `reportToPartyMeeting`，非制度文件仍走原归档） |
