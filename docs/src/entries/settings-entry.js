...
import { readLoginSnapshot } from '../core/login-snapshot.js?v=20260916a';
import { ROLE_LABELS, ROLE_PAGE_MAP, getAccentColors } from '../core/constants.js?v=20260916a';
import { resolveAppliedAccentRole } from '../core/theme.js?v=20260916a';
import { appearanceControlsHTML, bindAppearanceControls } from '../components/appearance-controls.js?v=20260916a';
import { icon } from '../core/icons.js?v=20260916a';
import { escHtml as esc } from '../core/utils.js?v=20260916a';
import { getCapabilities } from '../core/registry.js?v=20260916a';
import {
  coreTabIdsOf, sameIdOrder, applyPersonalTabOrder, readPersonalTabOrder,
  savePersonalTabOrder, resetPersonalTabOrder,
} from '../services/preferences.js?v=20260916a';
...
import { registerApiAdapter, setDataSource, init as dataInit } from '../core/data-adapter.js?v=20260916a';
import { ApiAdapter } from '../core/api-adapter.js?v=20260916a';

// 2026-09-09 支部归属显式化/审计内核：变更记录展示与回滚按钮需要操作者姓名、单键回滚白名单
import { getPersonName } from '../services/person.js?v=20260916a';
import { CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260916a';
...
function loadAuth() {
  if (!_authModule) _authModule = import('../services/auth.js?v=20260916a');
  return _authModule;
}
...
async function buildMyWorkspaceModel(role, personId) {
  ...
  await import(`../modules/capabilities/${stem}-workspace.js?v=20260916a`);
  ...
  const { applyTabPolicy, getBranchIdOfPerson } = await import('../services/branch.js?v=20260916a');
...
async function renderBranchGovSection(panel, sectionId) {
  ...
  try {
    const br = await import('../services/branch.js?v=20260916a');
    ...
    if (sectionId === 'branch-info-wizard') {
      ...
      await import('../modules/capabilities/secretary-workspace.js?v=20260916a');
...
async function openWizardEmbed(panel) {
  ...
  try {
    const { mountOrgSetupWizard } = await import('../components/org-setup-wizard.js?v=20260916a');
