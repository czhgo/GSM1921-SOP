...
import { getRosterStats, RESIDENCE_KEY } from '../../../services/roster.js?v=20260910a';
import { getCommitteeName } from '../../../services/branch.js?v=20260910a';
import { escHtml as esc } from '../../../core/utils.js?v=20260910a';
import { tokenOf } from '../../../core/version-token.js?v=20260910a'; // P0 域写版本戳（spec §二.4）
import { PREVIEW_KEY } from '../../../services/org-base-data-preview.js?v=20260910a'; // 基础数据预览 raw 源
import { memoizeRender } from '../../../components/memoize-render.js?v=20260910a'; // P2 渲染守卫（spec §四.1）
...
function _branchCardsSkeleton() {
  const card = `
    <div class="card rounded-xl p-4" style="min-height:216px;">
...
function _renderGovernance(el, activities, branches, noticeCount) {
  ...
  const cardsHtml = rows.map(r => `
    <div class="card rounded-xl p-4 flex flex-col" data-branch-card="${esc(r.id)}">
