...
import { getDataSource } from '../core/data-adapter.js?v=20260901s';
...
import { MockAdapter } from '../core/mock-adapter.js?v=20260901s';
...
export function saveDB() {
  ...
  if (getDataSource() === 'api') {
    import('../core/data-adapter.js?v=20260901s').then(({ persist }) => persist()).catch((e) => {
...
export function createActivity(data) {
  return _withDelay(() => {
    ...
    if (!newItem.assignments || newItem.assignments.length === 0) {
      import('./todo.js?v=20260901s').then(({ LifecycleTodoDeriver }) => {
