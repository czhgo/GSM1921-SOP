...
| 深金文字 | `#854D0E` | ██ | **实色金底上的文字/数字**（tab、选中项、金底按钮；对比度 4.85:1）；浅金底上仍可用 `#B45309` |
| 金浅底 | `rgba(255,215,0,0.12~0.14)` | ██ | 浅金色背景（类型徽章、图标底等小装饰） |
| 金实底 | `#FFD700` | ██ | **实色金底**（tab 激活态、cs-menu 选中项、chips 选中态、待办行动按钮，2026-08-08 二轮定稿） |
| 金边框 | `#EAB308`（实色底场景）/ `rgba(255,215,0,0.35~0.45)`（浅底场景） | ██ | 金色边框（tab 激活态取实色，强调卡、类型徽章取半透明） |
...
| 品牌·党徽金 | `#FFD700`（配套：**tab 激活态 = 实色金底 `#FFD700` + 边框 `#EAB308` + 深金文字 `#854D0E`**，2026-08-08 二轮定稿——半透明 0.14/0.30 叠白均偏淡偏橙不金；类型徽章等小装饰仍用 0.12/0.35 浅底） | **tab 激活态（2026-08-08 起）**、主题党日、项目分工标签、考察成长元素、待办行动按钮 |
...
- 菜单选中项 = 实色党徽金（背景 `#FFD700`，文字 `#854D0E`，2026-08-08 二轮定稿，与 tab 激活态同档）
