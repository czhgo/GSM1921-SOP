...
> 本指南是党支部管理引擎（GSM1921-SOP）设计思路的**最先进最完整体现**，整合了 SOP 文档优化方法论、系统架构设计方案、数据映射规则、优化提案机制与 Mock/真实鉴权辨析，为 SOP-系统联动维护提供一站式参考。
...
> **权威源**：[COMMISSIONER_FRAMEWORK.md §C](../strategy/COMMISSIONER_FRAMEWORK.md)（赋权关系链，原 PERMISSION_MATRIX.md §二）。本表为该权威源在系统架构中的切面视图，冲突时以权威源为准。
...
- 注：组织委员是写入型角色（专班），不在支撑型路径中。组织委员的"维护人才库"是基于考察信息的信息流处理，不是"归档"原始材料（依据 [USAGE_POLICY.md §1.4.3](USAGE_POLICY.md) 组织委员职责规则）
...
*本指南整合了 SOP 优化方法论、系统架构设计、数据映射规则、优化提案机制与 Mock/真实鉴权辨析。2026-05-06 v4.0 更新：全面反映 T25-T29 架构决策——MPA 架构、Mode 统一、三委员看板体系、日历限定、赋权链全量重写、系列活动模型。*
