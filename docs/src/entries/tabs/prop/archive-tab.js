...
import { solidAccentStyle, ARCHIVE_FALLBACK_ROLES } from '../../../core/constants.js?v=20260919j';
import { showToast, downloadCSV, downloadBlob, downloadUrl, _fmtDate, escHtml } from '../../../core/utils.js?v=20260919j';
import { persist, getAuthToken, getApiBaseUrl } from '../../../core/data-adapter.js?v=20260919j';
import { mockDB } from '../../../core/domain.js?v=20260919j';
import { bumpToken } from '../../../core/version-token.js?v=20260919j'; // P0 域缓存失效（spec §二.3）
import { loadActivities, listPublicityDrafts, setPublicityDraftStatus, PUBLICITY_DRAFT_STATUS } from '../../../services/activity.js?v=20260919j';
import { isApiMode } from '../../../services/runtime.js?v=20260919j';
import { AuthStore } from '../../../services/auth.js?v=20260919j';
import { getPersonName } from '../../../services/person.js?v=20260919j';
import { generateId } from '../../../core/id.js?v=20260919j';
import { addExternalDispatch, loadExternalDispatches } from '../../../services/external-dispatch.js?v=20260919j';
// A② 归档缺口判据单一源（支书台「宣传材料待归档」实时组同源）：已归档但无归档记录的活动
import { getArchiveGapActivities, getEndedUnarchivedActivities } from '../../../services/secretary-overview.js?v=20260919j';
// 活动归档写口（与支书台活动管理同源：软删 archived=true + 级联完成下属任务）
import { BranchService } from '../../../services/runtime.js?v=20260919j';
// 统一检索引擎（支书 2026-09-13 裁定）：第一列是活动的表格一律接入（关键词 + 分面；≤8 行自动不渲染检索条）
import { renderFilteredList } from '../../../components/list-filter.js?v=20260919j';
...
export function renderContent(ctx) {
  ...
  // B4（2026-09-12）：宣传概况「待归档活动 N 个」判据 = 已结束未归档活动 → 归档页同判据列明细并可直接归档
  ...
  container.querySelector('#archive-ended-list')?.addEventListener('click', async (e) => {
    ...
    try {
      ...
      showToast('success', '活动已归档，请在上方「待归档」区补充宣传材料');
      renderContent(ctx);
    } catch (err) {
      btn.disabled = false;
  ...
  container.querySelector('#archive-list')?.addEventListener('click', async (e) => {
    // B3（2026-09-12）：归档推进按钮同走容器委托——筛选/搜索重写列表后仍可弹出材料确认浮窗
    const advanceBtn = e.target.closest('.archive-advance-btn');
    if (advanceBtn) {
      e.stopPropagation();
      const record = _loadArchiveRecords().find(r => r.id === advanceBtn.dataset.recordId);
      if (!record || record.status === 'archived') return;
      _showArchiveAdvancePopover(record, advanceBtn, ctx);
      return;
    }
