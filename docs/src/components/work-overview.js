...
import { dutyCardHtml } from './workforce-duty-card.js?v=20260914i';
import { TodoStore, seedTodos, TodoStatus } from '../services/todo.js?v=20260914i';
import { IssueStore } from '../services/issues.js?v=20260914i';
import { AuthStore } from '../services/auth.js?v=20260914i';
import { solidAccentStyle, dotDarkVars, isActivityEnded, isActivityArchived } from '../core/constants.js?v=20260914i';
import { loadActivities } from '../services/activity.js?v=20260914i';
import { loadActiveAttendanceRecords } from '../services/attendance.js?v=20260914i';
import { loadInspectionRecords, getOverdueRecords } from '../services/inspection.js?v=20260914i';
// S3③（2026-09-12）：补课口径统一——概况补课缺口与「补课制度」表同源（services/makeup.js）
import { loadMakeupTasks } from '../services/makeup.js?v=20260914i';
import { TaskForceRecordStore } from '../services/taskforce.js?v=20260914i';
import { listPendingByReceiver, confirmExternalDispatch } from '../services/external-dispatch.js?v=20260914i';
...
async function _renderOverviewDetail(container, detail, accent, onBack) {
  ...
  if (detail.kind === 'activity') {
    const { renderActivityView } = await import('./activity-view.js?v=20260914i');
    renderActivityView(host, { highlightId: detail.id, accent });
  } else {
    const { renderTaskforceView } = await import('./taskforce-view.js?v=20260914i');
