...
import { rolesForPage, ARCHIVE_FALLBACK_ROLES } from '../../core/constants.js?v=20260915g';
...
registerCapability({
  ...
  tabs: () => {
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/prop/todo-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    // 工作概况（支书 2026-08-10 裁定：全部角色新增——汇报/卡点/在办三区总览 + 条线数据注入）
    { id: 'overview', label: '工作概况', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/prop/overview-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    // 分组「我的职责」= 2026-09-14 裁定按行为性质四组之一；本轮（2026-09-15）五台统一：原「党建」组改名「我的职责」
    { id: 'tasks', label: '宣传任务', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/prop/tasks-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    { id: 'kanban', label: '项目看板', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/prop/kanban-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    { id: 'weekly', label: '周报报送', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/prop/weekly-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    { id: 'archive', label: '档案归档', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/prop/archive-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    ...
    { id: 'tf-view', label: '知情查看', groupLabel: '知情查看', render: (ctx) => import('../../entries/tabs/prop/tf-view-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    // 本轮（2026-09-15）五台统一：原「反馈」组并入「制度与答复」（与支书台同名），组内以「我的处置」为成员
    { id: 'my-dispatch', label: '我的处置', groupLabel: '制度与答复', render: (ctx) => import('../../entries/tabs/prop/my-dispatch-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    ];
