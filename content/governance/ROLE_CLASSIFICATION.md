...
- **`[人]` 废弃**：原 [人] 文件按内容重新判定为 `[用户]` 或 `[用户]+[AI]`
- **`[人机]` 废弃**：原 [人机] 文件按内容重新判定为 `[用户]+[AI]` 或 `[工程师]+[AI]`
- **"正交双维度"废弃**：原"受众维度×AI 权限维度"正交模型于 2026-07-11 合并为单一维度
...
| `content/governance/` | `[工程师]+[AI]` | 系统治理 |
...
- 父目录 `content/sop/` = `[用户]+[AI]` → 其下子目录均继承 `[用户]+[AI]` 角色
...
4. **文档治理任务**：SSOT_INDEX.md → content/governance/DOC_MAP.md
...
| content/governance/DOC_MAP.md | 三类受众定义与本文件一致（合并为单维度） | ✅ 2026-07-11 |
...
> 本节为角色权限矩阵的权威源，对齐 `docs/src/services/auth.js` 的 ROLE_PERMISSIONS/PROJECT_PERMISSIONS/AUTHORIZE_CHAIN。数据流设计与界面实现路径见 [DATA_ARCHITECTURE.md](../design/DATA_ARCHITECTURE.md)。
