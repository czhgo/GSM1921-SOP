...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', render: (ctx) => import('../../entries/tabs/secretary/todo-tab.js?v=20260907a').then(m => m.renderContent(ctx)) },
    ...
    { id: 'calendar', label: '活动管理', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/secretary/calendar-tab.js?v=20260907a').then(m => m.renderContent(ctx?.appState)) },
    // L4 支部工作地图（2026-09-03 书记裁决放行）：支部分工 · 平铺/按人双视图（书记/副书记共用书记台）
    { id: 'work-map', label: '支部分工', groupLabel: '党建', render: (ctx) => import('../../entries/tabs/secretary/work-map-tab.js?v=20260907a').then(m => m.renderContent(ctx)) },
    ...
    { id: 'report-up', label: '上报党委', groupLabel: '对接党委', render: (ctx) => import('../../entries/tabs/secretary/report-up-tab.js?v=20260907a').then(m => m.renderContent(ctx)) },
  ],
});
