...
import { readLoginSnapshot } from '../core/login-snapshot.js?v=20260922j';
import { ROLE_LABELS, ROLE_PAGE_MAP, getAccentColors } from '../core/constants.js?v=20260922j';
import { resolveAppliedAccentRole } from '../core/theme.js?v=20260922j';
import { appearanceControlsHTML, bindAppearanceControls } from '../components/appearance-controls.js?v=20260922j';
...
} from '../services/preferences.js?v=20260922j';
// 批4（2026-09-09 支书批「域参数」）：制度默认单一源 = policy-defaults（设置页展示「制度默认」行与域参数默认值）
import { POLICY_DEFAULTS, ACTIVITY_APPROVAL_MODES, ACTIVITY_APPROVAL_MODE_LABELS } from '../core/policy-defaults.js?v=20260922j';
...
import { registerApiAdapter, setDataSource, init as dataInit } from '../core/data-adapter.js?v=20260922j';
...
import { CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260922j';
...
function loadAuth() {
  if (!_authModule) _authModule = import('../services/auth.js?v=20260922j');
  return _authModule;
}
...
async function buildMyWorkspaceModel(role, personId) {
  ...
  await import(`../modules/capabilities/${stem}-workspace.js?v=20260922j`);
  ...
  const { applyTabPolicy, getBranchIdOfPerson } = await import('../services/branch.js?v=20260922j');
...
async function renderBranchGovSection(panel, sectionId) {
  ...
  try {
    const br = await import('../services/branch.js?v=20260922j');
    ...
    if (sectionId === 'branch-info-wizard') {
      ...
      await import('../modules/capabilities/secretary-workspace.js?v=20260922j');
...
async function openWizardEmbed(panel) {
  ...
  try {
    const { mountOrgSetupWizard } = await import('../components/org-setup-wizard.js?v=20260922j');
