...
async function collectAll(page) {
  return page.evaluate(async () => {
    const mock = await import('/src/mock/index.js?v=20260921g');
    const seed = await import('/src/mock/seed.js?v=20260921g');
    const accounts = await import('/src/mock/accounts.js?v=20260921g');
    return {
      PEOPLE: mock.PEOPLE,
...
function auditLifecycle(data) {
  ...
  //    2026-09-16 基线刷新（批次 47-Y）：锚点 2026-09-05 → **2026-09-17**（今天的日期）。
  ...
  for (const att of data.ATTENDANCE_RECORDS) {
    const act = actById(att.activityId);
    if (act && act.date > '2026-09-17' && act.status !== 'cancelled') issues.push(`[lifecycle] 未来活动 ${att.activityId}(${act.date}) 已有考勤 ${att.id}`);
  }
