...
related_files:
  ...
  - content/guides/DOCUMENTATION_MAP.md
---
...
4. **文档治理任务**: SSOT_INDEX.md → content/guides/DOCUMENTATION_MAP.md
...
| 维度 | 覆盖率 | 说明 |
|------|--------|------|
| 文件角色标注 | 100% | 所有仓库文件均有角色标记 |
| 权限边界定义 | 100% | 三种角色的读写权限明确 |
| 协作方式定义 | 90% | 跨角色修改规则已定义，但自动化执行尚未实现 |
| 存储机制 | 100% | 存储分区与访问控制明确 |
| 读取机制 | 95% | 读取优先级已定义，但 AI 自动按角色加载尚未实现 |
...
| v1.1 | 角色标签自动化（copilot-instructions.md 中添加角色识别规则） | 待定 |
| v2.0 | 权限执行自动化（Agent Skill 实现自动权限校验） | 待定 |
...
| content/guides/DOCUMENTATION_MAP.md | 权威性层级与角色分类对应 | ✅ |
| .github/copilot-instructions.md | 写盘权限规则与本文件权限边界一致 | ✅ |
| .ctx/CONTEXT.md §3 文件访问权限 | Route ID 权限表与本文件协作方式一致 | ✅ |
| ROADMAP.md §二 角色三分类 | 定义与本文件一致 | ✅ |
