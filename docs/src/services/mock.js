...
import { getDataSource, notifyDataLoaded, persist } from '../core/data-adapter.js?v=20260922j';
import { bumpToken, resetAllTokens } from '../core/version-token.js?v=20260922j'; // P0 域缓存失效（spec §二.3/§二.4）
...
import { MockAdapter } from '../core/mock-adapter.js?v=20260922j';
...
import { handleInitResetIfRequested, stripSeedRecordsIfInitState } from './init-reset.js?v=20260922j';
// 批4（2026-09-09 支书批「域参数」）：数据加载完成 → 读侧有效默认注入
// （当前人所属支部 config.policyOverrides merge 进 POLICY_DEFAULTS；无 overrides = 保持默认）
import { applyEffectivePolicyDefaultsForPerson } from './branch.js?v=20260922j';
...
export function saveDB() {
  ...
  //
  // 2026-09-17 批次 49 更正：原实现用**动态 import** `.then(({persist}) => persist())` ——
  // 动态 import 至少延后一个任务，而紧随其后的成功提示会在**快照尚未排程**时就完成等待，
  // 判据落空（「存好了才报成功」在这条链上失效）。本文件第 10 行本就有到 data-adapter 的
  // 静态 import，直接改用静态 `persist` 即可，零新增依赖、也不再有异步空窗。
  if (getDataSource() === 'api') persist();
}
...
export function createActivity(data) {
  return _withDelay(() => {
    ...
    if (!newItem.assignments || newItem.assignments.length === 0) {
      import('./todo.js?v=20260922j').then(({ LifecycleTodoDeriver }) => {
