...
| ID | 事项 | 引用流程 | 状态 |
| --- | --- | --- | --- |
| R-19 | **遗留（后端通知写门缺失）**：`notices` 前端有发布/管理角色白名单（已补副书记），但 server `POST/PATCH/DELETE /notices` 仍 `requireAuth`——任一登录成员可直连写通知，属「前后端口径不一致」残余 | SYSTEM_ROLE_PERMISSION §9h + 附录⑦ 写权判据 | 待办 |
| R-20 | **遗留（「应到」两口径同名）**：表决「应到」＝有表决权党员（预备党员无表决权），考勤「应到」＝正式＋预备；两处同名字段口径不同且界面未解释，用户会以为漏人 | H10 最小三成本（理解成本）+ 附录⑥ 术语一致 | 待办 |
| R-21 | **遗留（演示种子只覆盖第一党小组）**：党小组会种子活动组织者仅 p1/p2（第二组仅一条已取消），「组长上传位＝本组」收紧后 p2/p4 下拉为空；补第二/第三党小组小组会种子（涉及活动计数/多组用例基线，需单独评估） | 附录⑦ + 演示种子完整性 | 待办 |
