...
import { loadAttendanceRecords, loadActiveAttendanceRecords, listLowAttendanceSessions } from './attendance.js?v=20260922f';
import { loadActivities } from './activity.js?v=20260922f';
import { loadInspectionRecords, getOverdueRecords } from './inspection.js?v=20260922f';
import { TaskForceRecordStore } from './taskforce.js?v=20260922f';
import { loadActivityReviews, loadActiveActivityReviews } from './review.js?v=20260922f';
import { NoticeStore } from './notice.js?v=20260922f';
import { TodoStore, seedTodos, TodoCategory, TodoActionType, REALTIME_GROUP_DOMAIN, WORK_DOMAIN } from './todo.js?v=20260922f';
import { tokenOf } from '../core/version-token.js?v=20260922f'; // P0 域缓存失效（spec §二.3/§二.4）
import { PEOPLE } from '../mock/index.js?v=20260922f';
import { getPersonById } from './person.js?v=20260922f';
import { ROLE_LABELS, isActivityEnded, isActivityArchived } from '../core/constants.js?v=20260922f';
import { mockDB, AttendanceStatus, ReviewStatus } from '../core/domain.js?v=20260922f';
import { persist } from '../core/data-adapter.js?v=20260922f'; // 周报审核写口落盘（SOP-B-40 ②）
...
export const SecretaryTodoDeriver = {
  ...
  _aggAttendanceRemind() {
    ...
    const gaps = activities.filter(a =>
      isActivityEnded(a) && a.date && !POLICY_DEFAULTS.attendance.noAttendanceTypes.includes(a.type) && // 132：不考勤的会议类型（支委会）不提示「考勤待录入」（单源 = policy attendance.noAttendanceTypes）
    ...
    return this._mkGroup('attendance-remind', '考勤待录入', TodoCategory.REVIEW, TodoActionType.REVIEW,
      `活动结束>${POLICY_DEFAULTS.attendance.entryRemindDays}天未录入考勤 → 纪检确认 → 考勤明细`,
      gaps.map(a => ({
        id: a.id,
        activityId: a.id,
        name: a.title,
  ...
  _aggAttendanceConfirm() {
    ...
    return this._mkGroup('attendance-confirm', '考勤待复核', TodoCategory.REVIEW, TodoActionType.REVIEW,
      '纪检已确认考勤 → 支书复核 → 考勤明细', records, 'confirm');
  },
