...
import { dutyCardHtml } from './workforce-duty-card.js?v=20260922i';
import { TodoStore, seedTodos, TodoStatus } from '../services/todo.js?v=20260922i';
import { IssueStore } from '../services/issues.js?v=20260922i';
import { AuthStore } from '../services/auth.js?v=20260922i';
import { solidAccentStyle, dotDarkVars, isActivityEnded, isActivityArchived } from '../core/constants.js?v=20260922i';
...
import { loadActiveAttendanceRecords } from '../services/attendance.js?v=20260922i';
import { loadInspectionRecords, getOverdueRecords } from '../services/inspection.js?v=20260922i';
...
import { listPendingByReceiver, confirmExternalDispatch } from '../services/external-dispatch.js?v=20260922i';
...
import { renderFilteredList } from './list-filter.js?v=20260922i';
...
export async function renderWorkOverview(container, { role, personId, accent = '#B91C1C', prefix = '' }) {
  ...
  const myActs = filterActivitiesForViewer(loadActivities(), role).filter(a =>
...
function _lineBlockers(role) {
  ...
  if (role === 'disc-commissioner') {
    ...
    const activities = filterActivitiesForViewer(loadActivities(), role);
    ...
    const pendingAuth = filterActivitiesForViewer(loadActivities(), role).filter(a =>
...
function _lineProgress(role) {
  ...
  if (role === 'disc-commissioner') {
    ...
    const acts = filterActivitiesForViewer(loadActivities(), role).filter(a => !isActivityEnded(a) && a.status !== 'cancelled');
...
async function _renderOverviewDetail(container, detail, accent, onBack) {
  ...
  if (detail.kind === 'activity') {
    const { renderActivityView } = await import('./activity-view.js?v=20260922i');
    renderActivityView(host, { highlightId: detail.id, accent });
  } else {
    const { renderTaskforceView } = await import('./taskforce-view.js?v=20260922i');
