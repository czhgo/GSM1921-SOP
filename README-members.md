...
| 角色 | 工作台里有什么（职责页） |
|------|------------------------|
| **书记 / 副书记** | 今天、待办、全局概况（按维度/按人）、活动管理、支部分工、赋权管理、支部配置、通知发布、专班查看、党小组进展、反馈管理、上报党委 |
...
| 层 | 位置 | 职责 |
|----|------|------|
| **前台（页面）** | `docs/`（18 页：11 根页 + 7 工作台） | 页面骨架，零硬编码逻辑 |
...
| 阶段 | 状态 | 内容 |
|------|------|------|
| 页面与工作台体系 | ✅ 已完成 | 11 根页 + 7 工作台（18 页）；登录直达「今天」；9 域待办；支部配置向导 5 步 |
...
3. **配色与术语**：侧边栏主题选择换强调色；术语权威源 `content/03_doc_system/USAGE_POLICY.md`（党建红/党徽金为固定合规底线）
4. **支部配置**：书记工作台「支部配置」/ wizard.html 内 5 步向导可视化完成（模块启停、分工、术语指引、验证重置）
