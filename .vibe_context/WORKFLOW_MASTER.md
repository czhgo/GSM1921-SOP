...
## 🔐 数据安全与 ACL 协议 (Data Security & Access Control)

> **架构约束级别：最高**。本协议为未来生成 Supabase Row-Level Security (RLS) 策略的绝对依据，不得绕过或降级。任何变更须经支委会集体决议。

### [数据安全与 ACL 协议]

| # | 数据类型 | 可见范围 | 读写权限 | 说明 |
|---|----------|----------|----------|------|
| A1 | **考勤信息 (Attendance)** | 所有支部成员公开可见 | 只读（immutable） | 纪检委员可在活动当日录入；事后更正须经支书授权 |
| A2 | **考察信息 (Evaluation)** | 仅支委可见/可改 | 支委读写；普通成员仅限查本人 | 推荐意见、谈话记录属最高机密级，仅支书和经办支委可查 |

**铁律**：
1. **考勤信息**：作为客观事实，必须向所有支部成员**公开可见，但不可篡改**（只读）。
2. **考察信息**：作为主观敏感数据，**仅对支委可见并具备读写权限**；普通成员仅限查询本人的考察记录，绝对隔离。
