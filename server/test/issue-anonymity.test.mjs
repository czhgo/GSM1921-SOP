...
import { hashSubmitterToken } from '../../docs/src/core/constants.js?v=20260919k';
...
test('mock 形态：常规出口（成员/支书）看不到真身；党委核查出口看得到；本地库留真身且留痕', async () => {
  ...
  try {
    ...
    const out = await page.evaluate(async () => {
      const { IssueStore } = await import('/src/services/issues.js?v=20260919k');
      const { PersonStore } = await import('/src/services/person.js?v=20260919k');
      localStorage.clear();
      ...
      const fb = await import('/src/entries/tabs/secretary/feedback-tab.js?v=20260919k');
      fb.renderContent();
      ...
      const pcTab = await import('/src/entries/tabs/party-committee/issue-review-tab.js?v=20260919k');
