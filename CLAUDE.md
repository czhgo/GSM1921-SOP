...
| R-3/R-4/R-5 | **已清（2026-09-10）**——R-3 门面页通用化（index/about/help 首屏补通用定位句；about 首屏经 `about-entry.js` 落地）；R-4 只读端视觉对齐（**书记特批 inspector.js 只读端** + activity-entry / activity-view：去 `text-[10px]`、灰底非标卡改 `card rounded-xl`）；R-5 测试隔离加固（b3-1 自重置前置态，连跑两次均 5/5） | COMPONENT_SPEC + 禁改特批 + 测试系统 | ✅ 已清 |
| W-1 | **已清（2026-09-10，书记五项）**——① 待办**逐条催办**（全 9 域定向通知 + 会话内冷却）＋**兜底直执白名单**（补代归档/代复盘入口；复核确认/成员变更/销项已有）＋**副书同权**（议程结果区与议程编辑）；② **卡片普查与合并**（KPI 五连卡取消·明确不设出勤率 KPI、出勤趋势入异常队列、阶段分布入滞留卡、上报统计改页首内联行、思想汇报说明卡与材料标准卡消失、看板专班工作量并入项目卡、纪检超期提醒入表头徽标、人才库计数入统计条；补入口：成员出勤明细+补课、项目卡/看板条目/转正提示跳转、党委监控「进入支部」、党委台卡片外壳统一 `card`）；③ 闭环评议**样本 1**（参与待办直达活动详情 + 列表报名/表态入口）＋ 4 条判据入 REVIEW_QUEUE 附录⑦；④ 归属显示**时序修复**＋未登录中性占位「示例组织（未登录）」；⑤ 见 R-3/R-4/R-5 行 | H60.6/60.7 + 附录⑦ 判据 + 禁改特批 | ✅ 已清 |
| R-6 | 遗留待办：通知弱定位（无 targetUrl 时落列表而非目标对象）、代归档未深链到宣传归档页（现为活动定位+提示）、代填复盘无承载表单、催办通知文案待书记定稿、党委「进入支部」在 API 会话按设计拒绝 | H60.7 | 待办 |
