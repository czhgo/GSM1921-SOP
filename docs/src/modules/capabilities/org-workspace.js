...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/org/todo-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    // 工作概况（支书 2026-08-10 裁定：全部角色新增——汇报/卡点/在办三区总览 + 条线数据注入）
    { id: 'overview', label: '工作概况', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/org/overview-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    // 分组「我的职责」= 2026-09-14 裁定按行为性质四组之一（工作台/我的职责/知情查看/制度与答复）；
    //   本轮（2026-09-15）五台统一：原「党建」组改名「我的职责」（与支书台同名，组名不再按业务域分）。
    { id: 'inspection', label: '考察上传', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/org/inspection-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    // D8 裁决批二（2026-09-08）：思想汇报（初阅为组织委员高频每日动作）前移至考察上传之后——
    // 与成员发展域族（名册/人才库/发展数据）聚拢，且初阅高频前置（表 C 调序裁定）。
    { id: 'thought-review', label: '思想汇报', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/org/thought-review-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    { id: 'taskforce', label: '专班管理', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/org/taskforce-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    // 成员名册（立项⑥ B波 2026-09-06：新增/行内编辑/删除 双形态持久；人才库=发展观察视图=只读画像，分工不重复建设）
    { id: 'roster', label: '成员名册', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/org/roster-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    { id: 'talent', label: '人才库', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/org/talent-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    { id: 'development', label: '发展数据', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/org/development-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    ...
    { id: 'tf-view', label: '知情查看', groupLabel: '知情查看', render: (ctx) => import('../../entries/tabs/org/tf-view-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
    // 本轮（2026-09-15）五台统一：原「反馈」组并入「制度与答复」（与支书台同名）；
    //   组内以「我的处置」为成员，替代原独立「反馈」组（同功能不再两名）。
    { id: 'my-dispatch', label: '我的处置', groupLabel: '制度与答复', render: (ctx) => import('../../entries/tabs/org/my-dispatch-tab.js?v=20260922h').then(m => m.renderContent(ctx)) },
  ],
});
