...
### 2026-08-30 · 未尽事项推进（T-206 确认表 + T-281 草案 + AI逃逸第四轮 + secretary 减负评估）

- **书记指令**：继续推进所有未尽事项；需询问必须 ask user question；建议事项基于上下文可直接决策
- **AskUserQuestion 确认**：自主推进（secretary todo-tab 减负 / AI逃逸第四轮 / T-118 收尾）· T-281「AI 先起草草案」· T-206「整理确认表」· push 暂不
- **T-206 确认表**（14 项逐条复核）：7 项已解决（#1 计数 17 一致 / #2 P-046 融入 P-002 / #3 标题重拟 T-253 / #4 AI 理解零残留 / #5-6 原话升正文 / #10 已迁出索引 / #11-12 编号重组）· 4 项建议授权 AI 修（#7 引号 / #8 标点 / #9 日期格式 / #13 出处字段）· 2 项待书记（#1 索引核对 / #14 过时示例指定）——交付书记确认
- **T-281 复核结论（2026-08-30 书记纠正）**：本条目**已过时**——书记 2026-08-28 已以「战略模糊自觉」总纲解决（AI 不替书记收口定性；P-005 已含 08-28 书记合并原话），上轮 AI 起草的草案（替书记定性 P-005/P-009/P-014 收口）违反该总纲，**撤回不落地**。**教训（书记明确）**：拿过时任务拷问书记是错的；推进任务前先复核乙部状态与最新总纲
- **AI逃逸捕捉第四轮**（扫 01_strategy + 02_institution）：**无实质逃逸点**——两目录为内容文档（论断/制度/叙事）非提示词规则文档；权限矩阵 Y*「原则上但业务上一般不使用」为有意的权限弹性设计、DEVELOPMENT_PATH「必须」为官方条文/内容表述；**提示词体系全量收敛**（REVIEW_QUEUE 附录⑦ 登记）
- **secretary todo-tab 减负评估**：与 todo-tab-shell 共性仅 ~15%（三型详情 confirm/remind/seed + 一键确认 + 待答复收件箱 + 专班审批 modal + 赋权面板展开均角色特有）——套壳需大改 shell 且集中在书记最高频工作台，**维持独立**（胆大心细设计判断，净减目标已由 M8 达成）
- **T-118 收尾**：文档体系已随本轮 README/清理/第四轮扫描同步（根 README 复用章、04 README、SNAPSHOT、server/README、DOC_MAP 均核对）

---

### 2026-08-30 · 书记纠正三件事（T-281 过时 + 表达纪律加强 + 审议机制 harness 规则）

- **书记纠正①（T-281 过时）**：书记指出民主集中制问题已解决，AI 拿过时任务拷问——复核确认：2026-08-28 书记以「战略模糊自觉」总纲（CLAUDE.md L46）解决（AI 不替书记收口定性、保持战略模糊），P-005 已含 08-28 合并原话"程序在所有人之上，所以扁平；但程序中，支委个人都因为更大的责任而被要求嵌入地更深"；上轮 AI 草案（替书记定性）违反该总纲，**撤回**；乙部 T-281 归档 ✅ 已完成
- **书记纠正②（表达纪律加强，强烈要求）**：**禁止 = 与 ≠ 符号**（"真的很讨厌"，任何文档/审议草案/REVIEW 相关文字一律禁用）+ **「不是…而是…」反论全面禁止**（原"须审议后写入"升级为全面禁止）——落地：CLAUDE.md H10 书记原话表达纪律新增第④条 + 升级第②条；USAGE_POLICY §2.3 新增规则 6；清理 project_memory / EXECUTION_LOG 中 AI 新写的 = ≠ 残留（历史评议记录 DECISION_LOG 等保留为事实）
- **书记纠正③（审议机制 harness 规则）**：**涉及审议时，所谓给上下文，不是输出一个报告，而是基于 ask user question 机制，在需要决策的地方提供需要的内容**——落地：CLAUDE.md H50.2 新增「审议交付机制」补丁（AskUserQuestion 逐点决策，禁长报告；问题自带选项与推荐）
- **教训**：①推进任务前先复核乙部状态与最新总纲（勿拿过时任务拷问书记）；②AI 表达禁用符号懒政（= ≠）与排他反论；③审议交付用 ask 逐点决策而非长报告

---

### 2026-08-30 · W4 反论评议首轮（启用 sample-diff-learning）+ T-206 机械项授权修订

- **书记指令**：T-206 机械项授权 AI 修（引号/标点/日期/出处）+ 残余 2 项 AI 先做列候选 + W4 反论首轮启动；反论 3 处「而非」**逐个让书记过，启动 sample-diff-learning**
- **W4 反论首轮**（抽样 02_institution，3 样本逐条 AskUserQuestion）：
  - 样本 1（宣传 SOP 每周一报送）：**书记纠正「每周一是学工系统的规定，没有什么原因」**——AI 自造的频率权衡理由错误 → 改为「时间节点：每周一是学工系统的规定」
  - 样本 2（照片两级分档）/样本 3（纪检复盘三态）：书记确认**同为 AI 自造理由** → 正面化改写（删权衡理由）
  - **模式沉淀**：diff-patterns.md 新增 **LA-03 External Rule Self-Made Justification**（外部规定自造理由——时间节点/分级/状态设计有外部规定或一句话事实时不得自造权衡理由，真实原因不明须问书记；Boolean 条件）
  - 边界确认：书记原话中的反论保留原貌（P-001/P-008 等），AI 推导/设计说明类清理
- **T-206 机械项**：ROLE_CLASSIFICATION 清 ≠/= 符号（访客非角色/COMMISSIONER_ROLES 两处语义）；P-002 出处行去解释性字段（子本扁平化）；P-003 日期后缀**书记裁定不补**（日期不可考）；引号统一（正文/讲解块「」）列后续候选
- **REVIEW_QUEUE 附录①**：反论首轮已完成登记（检查要点新增「外部规定自造理由」）

---

### 2026-08-30 · W4 反论扩展扫完成（13 处设计考量逐条送审）+ 书记三大纠正复核 + 自动广播立项

- **书记纠正复核（三项均已在上一轮落地，本次核实确认）**：① T-281 民主集中制已由 2026-08-28 战略模糊自觉总纲解决、乙部归档 ✅ 已完成——AI 确认不再拿过时任务拷问；② =/≠ 符号禁令 + 「不是…而是…」全面禁止已入 CLAUDE.md H10 ②④ + USAGE_POLICY 规则 6，本轮清理 project_memory 历史「=」残留 5 处（术语定义/权限模型/上下文文件定义等改自然语言）；③ 审议交付机制（AskUserQuestion 逐点决策、禁长报告）已入 CLAUDE.md H50.2——本轮 W4 扩展扫全程以此机制逐条送审
- **W4 扩展扫**（02_institution 全部 13 处「设计考量」逐条 AskUserQuestion 送审，含首轮 3 样本）：
  - 样本 4-9 书记裁决**保留**：专班看板（+ 协调观：书记把「协调」看作最小三成本原则的具体实现，已入组织委员 SOP）/ 考勤锁定 / 唯一节点 / 只读权限 / 反向核对（均已核实网页实现到位）
  - 样本 5 思想汇报：书记问「代码中实现了吗」→ 核查发现 D-241 决策为线下手写、系统未承载 → 书记裁决**启动数字化**（党员提交文本→组织委员归档→考察调用记录）；引入**功能实现评议**专项（REVIEW_QUEUE 附录⑧）
  - 样本 10 定人定责条条为主：保留 + 书记补**兼任原则**「可以兼任，但一般情况下尽可能不要兼任」→ 整合定人定责 L132
  - 样本 11 核心群分层：书记引出**「资料交接渠道定位」大问题**（网页/小程序 vs 微信群）→ 裁决**混合模式**（现场协调在微信群、资料归档交接在系统）；待办：补核心群「读完手册会建群会工作」手册 + 工作群规范（建群/收尾/解散）
  - 样本 12 Q2 影响面排序：书记裁决**删除**——场景不真实（支委平级无矛盾指令）+ 固定排序与扁平化冲突 → 已删党小组手册 Q2 及设计考量、编号顺延（Q3/Q4→Q2/Q3、Q5→Q4）、insights 书记仲裁条目同步（去排序）
  - 样本 13 事后报备：书记裁决**报备分「纯知情/需决策/需配合」三类且不局限**——该报备的要多报备（支委扩大群说一声知情即可）、涉及多方协同须区分；已改写定人定责 L236/L238
- **自动广播立项（书记批准）**：系统探索自动发送信息广播 + 自动同步文案落地 + 网页文案对需微信群交代事项加「请前往微信群……」小提示——现状盘点：站内通知（notices+赋权通知）、邮件双通道（mailer-hooks）、数据变更广播（data-adapter）已具基础；SOP 信息流（考勤同步核心群→支委扩大群→归档）依赖微信群人工转发，系统缺自动同步文案/广播模板 → 推进路径：现状盘点→文案模板草案→书记审阅→实施（REVIEW_QUEUE 附录⑧ 立项②）
- **insights 表达清理**：L322 书记仲裁条目去「组织>纪检>宣传」排序；L327 标题「看≠做」→「看与做相区分」；L333 反论句式正面化（越界的信息可见性会诱导越界的操作意图）
- **验证**：全部为文档治理改动，无 JS 代码变更；npm test 基线不受影响

---

### 2026-08-30 · T-206 引号统一执行（SECRETARY_PRONOUNCEMENTS.md 全文逐处核对）

- **书记授权**（T-206 确认表 4 项机械项之一）：「统一（正文「」原话内引号保留），全文逐处核对修订」
- **执行规则**：正文讲解块（`>` 引用块）中的弯引号统一为「」；书记原话（`- “…”` 列表项 / 独立原话段）内引号保留原貌（含出处行引用原话的注，如 P-014 出处行"是书记原话的一部分"保留）
- **修订清单**（约 20 处讲解块）：L12「grill 书记后的话」/ L14「为什么采取这个制度」等 / L22「人」「工具」/ L26「人人可提」「高年级课业重」/ L38「管理事、服务人」「程序」等 5 处 / L41「经验沉淀」「路线级论断」/ L64「通过组织获得成长」「完整叙事」/ L78+L80「管理事，服务人」「组织性如何判定的过程」/ L82「核心价值」「宝贵机会」等 / L84「项目产出声明」/ L101「管理事，服务人」/ L103「两张皮」/ L107「系统产出是什么」/ L111 出处「管理事，服务人」/ L126「创新创业」/ L128「想象等级森严」/ L141「程序在所有人之上」/ L161「有先例的工作」/ L171-173「方兴未艾」「有先例的工作」/ L186-192「党支部」「在框架内」等 4 处 / L211「没人管、没程序」/ L219「可持续的管理」/ L238「没有上下级」/ L248「条」「块」/ L260「集中容易，分工难」/ L283「不承担直接发展考核党员的职能」/ L299-303「条块能不能做」等 3 处 / L335「没有经济负担」/ L337「在没先例的组织形态中创造先例」/ L351「是否有助于恢复成员的对话能力」/ L376-378「组织下命令、成员执行」等 2 处 / L386「已迁出/已删除论断」/ L410-423 附录区（含 ≠ 清理：「党建工作不等同于创新探索」等 3 处 +「管理事，服务人」「在框架内说真话」）
- **书记原话保留原貌**：P-001~P-017 全部原话（含内嵌单引号'组织性'等）与出处行原话引用未动
- **验证**：Grep 复核——正文讲解块无弯引号残留，剩余弯引号均为原话/出处引用

---

### 2026-08-30 · 四立项推进：核心群手册+工作群规范（合并 §6）+ 自动广播草案 + 思想汇报算法归档

- **书记两点指导**：① 核心群手册**不新建文档**——content 文件已过多，需「适度合并同类项」；② **算法归档原则务必重视**——思想汇报提交之后已在后台，算法自动归档，不需要人工「归档环节」
- **核心群手册+工作群规范（合并进党小组组长工作手册 §6，未新建文档）**：§6 开头加「微信群与系统的分工（混合模式）」定位段（现场协调在微信群、资料归档交接在系统，群内信息最终落系统归档）；6.1 核心群后新增「建群与工作指引（读完就会建、会工作）」（建群时机/建群动作/日常怎么工作/工作结束怎么收尾）；6.5 后新增「6.6 工作群规范（建群/收尾/解散）」（工作群有生命周期，收尾=复盘+归档确认+群内说明，解散=一次性活动收尾后解散/长期专项明确用途保留）
- **自动广播文案模板草案 v1（REVIEW_QUEUE 附录⑧ 立项②）**：统一结构「站内通知 + 需微信交代事项加『请前往微信群……』提示；已落系统事项标注『系统已自动归档』」——7 场景模板（活动策划完成/考勤已确认/宣传稿同步/思想汇报已提交/材料外发确认/待办提醒/汇报反馈）
- **思想汇报数字化·算法归档（书记 2026-08-30 决策落地）**：D-241 被推翻——党员系统内提交文本 → 算法自动归档归集至个人档案 → 组织委员考察时调用，无人工归档环节；一改具改同步 9 处（组织委员 SOP 信息流/任务流/设计考量/检查清单、定人定责 L70/L178、纪检 SOP L25、常见场景 L334、USAGE_POLICY L50、COMMISSIONER_FRAMEWORK L93、INDEX L35、MODULE_UI_DESIGN L59/L154、SERVICE_CATALOG L38、insights 两处、D-241 决策记录追加更新）
- **待实施（代码层）**：思想汇报提交入口 + 算法自动归集 + 组织委员调用视图（数据模型/前端，未动代码）

---

### 2026-08-30 · 思想汇报数字化代码实施（书记决策落地，算法归档原则）+ 自动广播草案通过

- **书记批准**：自动广播文案草案 v1 通过可实施；代码层**先做思想汇报**（推荐）
- **数据层**：`core/domain.js` mockDB 新增 `thoughtReports` 集合 + THOUGHT_REPORT 路由改「系统自动归档（算法归集至个人档案）」；`core/mock-adapter.js` saveDB/loadDB/restoreNicheCollections 三处持久化接线；`mock/thought-reports.js` 新增 seed 3 条（p6 发展对象/p7 积极分子/p16 预备党员）+ mock/index.js 导出
- **服务层**：`services/thought-report.js`（新文件）——addThoughtReport（提交即归档，无人工归档环节）、listThoughtReportsByPerson（按人归集倒序）、countThoughtReportsByPerson；持久化为空时回退 seed（与 inspection.js 同模式）
- **前端**：`entries/tabs/visitor/thought-report-tab.js`（新文件）——参与者「思想汇报」tab：提交表单（字数提示 1500+/积极分子豁免）+ 历史列表，提交后 Toast「已自动归档」；`visitor-workspace.js` 注册 tab（version 20260830）；`entries/tabs/org/development-tab.js` 增强——候选人卡片加「思想汇报 N」角标 + 点击展开查看该候选人全部思想汇报（组织委员考察调用侧）
- **验证**：npm test **52/52 全绿**（129.6s）+ module-load **99/99 通过**（含新文件）；GetDiagnostics 零错误；tab-bar flex-nowrap+overflow-x-auto 布局安全（新增 tab 横向滚动不溢出）

---

### 2026-08-30 · 自动广播代码落地 3 场景 + 考勤内控决策（书记批准范围）

- **书记批准**：自动广播落地选 3 场景全做（活动创建→建群提示 / 材料外发→微信提示 / 考勤确认→归档通知）；**考勤内控书记裁决保持现状**（信任组长源头审校，靠复盘/抽查监督，不改自动确认机制）
- **考勤内控现状澄清（书记担忧回答）**：当前纪检"确认"考勤=异常驱动复核——出勤/已补由组长上传时源头审校（submittedBy=组长）自动确认不进队列，纪检只复核异常（缺勤/请假，打 recordedBy=纪检ID）；组长勾选参会人员无现场证据环节，存在"未参加报成参加"无法系统发现的缺口——书记裁决保持现状（属组织内控信任机制，靠复盘/抽查）
- **落地 ① 活动创建→建群提示**（`services/decision-tree.js`）：writeActivityWithSOP 成功后 _broadcastActivityCreated——「活动已创建，请建核心群：请前往微信群建立核心群（按党小组组长工作手册 §6 拉齐成员、发送开场说明），现场协调在微信群进行，资料归档与交接在系统完成」targetUrl=leader.html
- **落地 ② 考勤确认→归档通知**（`entries/tabs/disc/attendance-tab.js`）：纪检确认动作点加 NoticeStore——「考勤已确认并归档：系统已自动归档至考察档案，组织委员可直接读取使用」targetUrl=org.html
- **落地 ③ 材料外发→微信提示**（`services/external-dispatch.js`）：addExternalDispatch 后广播——「材料外发待确认：请前往微信群完成交接，收到后回系统确认」targetUrl=visitor.html
- **验证**：module-load **99/99 通过** + GetDiagnostics 零错误；REVIEW_QUEUE 附录⑧ 立项② 更新为「已实施」

---

### 2026-08-30 · 全仓「= / ≠」与「不是…而是…」残留清扫（书记禁令彻底落地）

- **触发**：objective 完成度审计——规则层面（H10 ②④ / USAGE_POLICY 规则6 / H50.2）已在 CLAUDE.md 落实，但历史残留仍在活跃文档中，书记「不要再出现」意味着历史残留亦须清理
- **清理边界（W4 确认）**：书记原话/裁决引用保留原貌；历史 DECISION_LOG/EXECUTION_LOG 保留为事实；references/ 外部学术文献不动；代码语境（`isBrand = true`、`?v=`、`≤30字符`、数学公式）保留；AI 推导类反论与语义性 = / ≠ 一律正面化
- **清理范围**（活跃文档逐处串行，避免并行覆盖）：
  - `SECRETARY_PRONOUNCEMENTS.md`：讲解块 5 处反论正面化（P-002/P-003/P-006/P-008/P-013）
  - `insights/工程演进与设计方法论.md`：25+ 处反论/符号正面化（命名/截图/看板/沉淀闭环/查询/视图/字体/孤零零段落/矩阵/幂等/打卡化/排序/弦长/重构/档案袋等）
  - `insights/党支部管理与实务经验沉淀.md`：5 处（组织性反论/人才库/系列专班/速查表/考量段）
  - `README.md`（开源复用入口）：5 处正面化（经验可传承/设计理念/完整叙事/理念非口号/信息可见性）
  - `CLAUDE.md`：3 处（修订校验/顺链接读下游/项目产出声明）
  - `OPERATIONS_GUIDE.md`/`PROCESS_GUIDE.md`：4 处（衔接/轮次/沉淀列/入库粒度）
  - `SNAPSHOT.md`：3 处（信息密度/正交维度/看与做）
  - `04_web_design`：COMPONENT_SPEC 2 处、COLOR_SYSTEM 6 处、DATA_MODEL 2 处、DATA_FLOW 1 处、SOP_WEB 2 处、DEPLOYMENT_AUTH_MODEL 1 处、ROLE_SSOT 1 处、CHECKLIST 3 处、ABOUT_DESIGN_SYSTEM 1 处、ARCHITECTURE_EVOLUTION 5 处、WECHAT/PKU_INTEGRATION 各 1 处
