...
| ② 角色识别层 | `getAccentColors(role)` / 侧边栏「工作台颜色选择」 | ✅ 受（可个性化） | 各角色主题色 | 身份归属 + 个人偏好：色条、角色标签、人员 chips、左侧边线、主按钮、工作台内强调色 |
...
- **主题色应用范围在工作台内可适当放宽**（书记 2026-08-08）：工作台内除 tab 之外的强调色可挂靠主题色；侧边栏选择器统一命名「**工作台颜色选择**」（浅色/深色模式与颜色选择同一排）。
