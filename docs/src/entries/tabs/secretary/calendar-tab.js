...
import { getAppState, setState } from '../../../core/state.js?v=20260922f';
import { showToast, escHtml as esc } from '../../../core/utils.js?v=20260922f';
import { scrollDetailIntoView } from '../../../components/detail-anchor.js?v=20260922f';
import { populateMonthSelector, renderCalendarByActivities } from '../../../components/calendar.js?v=20260922f';
import { renderInspectorFromState, _draftMaterialCount } from '../../../components/inspector.js?v=20260922f';
import { computeSecretaryStats } from '../../../services/roles.js?v=20260922f';
import { PersonPicker } from '../../../components/person-picker.js?v=20260922f';
import { openModal, closeModal } from '../../../components/modal.js?v=20260922f';
import { DecisionTreeState, renderWorkflowPanel, writeActivityWithSOP } from '../../../services/decision-tree.js?v=20260922f';
import { loadActivities, listBrandProposals } from '../../../services/activity.js?v=20260922f';
import { renderQueryView } from '../../../components/query-view.js?v=20260922f';
import { icon } from '../../../core/icons.js?v=20260922f';
import { loadAttendanceRecords } from '../../../services/attendance.js?v=20260922f';
import { liveMembers, PersonStore } from '../../../services/person.js?v=20260922f';
// 数据域接线收口（2026-09-03）：支部成员名单经 services/person.js 获取（原直连 mock PEOPLE）
// 实时视图（非快照）：成员增删即时可见——见 services/person.js liveMembers 说明
const PEOPLE = liveMembers();
import { NoticeStore } from '../../../services/notice.js?v=20260922f';
import { BranchService } from '../../../services/runtime.js?v=20260922f';
import { ACTIVITY_CLASSIFICATION, classifyActivityType, normalizeActivityType, dotDarkVars, SCENARIO_WRITE_IDS, SCENARIO_LABELS } from '../../../core/constants.js?v=20260922f';
...
import { collectAgendaRows, buildAgendaCandidates, AGENDA_CANDIDATE_GROUPS } from './agenda-form.js?v=20260922f';
import { defaultVoteConfig, isDecisionScenario, resolveVoterIds, isAnonymousForced } from '../../../services/vote-config.js?v=20260922f';
import { AuthStore } from '../../../services/auth.js?v=20260922f';
import { getBranchIdOfPerson, getBranchById, applyWorkflowBlockPolicy } from '../../../services/branch.js?v=20260922f';
// 支部文件读侧收敛点（2026-09-10）：会前草案下拉经 branch-doc 服务读取（按归属支部过滤，跨支部不可见）
import { listDocs as listBranchDocs, isAgendaDraftDoc } from '../../../services/branch-doc.js?v=20260922f';
// L3 S4（2026-09-03）：主题党日工作流块 manifest 驱动试点（入口守卫 + 表单元数据单一源）
import { BLOCK_MANIFESTS, THEME_PARTY_DAY_MANIFEST } from '../../../workflow/blocks/manifests.js?v=20260922f';
// B1（2026-09-12）：党委下钻支部的演示只读视图判定（单一源 = modules/branch-demo-nav.js）
import { isReadonlyBranchDrilldown } from '../../../modules/branch-demo-nav.js?v=20260922f';
...
function bindQueryToggle() {
  ...
  if (queryToggle && queryCollapsible && !queryToggle.dataset.bound) {
    ...
    queryToggle.addEventListener('click', () => {
      ...
      // 触点即落点（全局 UX 反思批次 2026-09-13）：展开后首个结果行此前落在视口之下（只露出 25px），
      // 补一次滚动让展开区可见
...
// 纪检考勤明细与全局概况 KPI 为专职读位，不动；异常处理位 = 纪检考勤管理。
...
function _attendanceSummaryHtml(actId, recordsByActivity) {
  ...
  // R-20（2026-09-13）：考勤「应到」＝正式＋预备党员（剔除滞留），与表决「应到（有表决权党员）」口径不同
  return `<div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-gray-500" title="考勤应到＝正式＋预备党员（剔除滞留），与表决「应到（有表决权党员）」口径不同">考勤：${parts.join(' · ')}</div>`;
}
...
function openWriteModal() {
  ...
  if (isReadonlyBranchDrilldown()) {
    showToast('error', '党委演示只读视图：不可写入活动（写操作按角色权限拒绝）');
    return;
  }
  openModal({
    id: WRITE_MODAL_ID,
    ...
    width: '720px',
    accentColor: _accentHex(),
    // 浮窗页脚就近深链（2026-09-19 批次 99 立；2026-09-21 批次 138 收进 modal.js 单一源）：本浮窗要定的
    // 制度口径（票决门槛 / 应到名单核对 / 会议考勤类型与记录人）集中在设置的「支部制度参数」分区，
    // 就地给去该分区的入口（设置页顶部有「← 返回上一页」）。渲染在 .modal-body 之外的页脚 ⇒ 切步骤不消失。
    settingsLink: { href: './settings.html#branch-policy-params', text: '活动制度口径（票决门槛 / 应到名单核对 / 会议考勤类型与记录人）→ 支部制度参数' },
    onMount: (panel) => {
...
/** 填充议程行「会前草案」下拉（判据＝`branch-doc.js::isAgendaDraftDoc`：普通文件草案 ＋ 制度链上尚未成为现行版的两种态；异步，空态引导去资料查询写入） */
async function _hydrateDraftDocs(row) {
  ...
  try {
    ...
    const drafts = docs.filter(isAgendaDraftDoc);
    select.innerHTML = `<option value="">${drafts.length ? '选择会前草案…' : '暂无会前草案'}</option>`
      + drafts.map(d => `<option value="${d.id}"${d.id === row.dataset.docId ? ' selected' : ''}>${d.title || d.fileName || '未命名草案'}</option>`).join('');
...
async function handleSubmitActivity() {
  ...
  try {
    const activityData = {
      ...
      executor: 'secretary',
      supervisor: null,
      // 2026-09-13 dogfood 同类彻查：创建人审计取真实登录成员（原写死占位 ID 'u_sec'）
      createdBy: AuthStore.getCurrentUser()?.personId || null,
    ...
    // 5. 预拟通知「只跑一次」（2026-08-05）：仅在表单填写了标题时发布一条通知，单次调用不重复发。
    // R-22（2026-09-13）：系统派生通知改由服务端生成（kind 注册表复算授权 + 落点；标题/正文为发布人所填）
    if (noticeTitle) {
      NoticeStore.addSystem('activity-notice-draft', activity.id, {
        title: noticeTitle,
...
// 绑定走 `#wp-agenda-list` 的事件委托：`change` 覆盖「选中会前草案」；`click` 覆盖「删除议程」那处
// 只清空 select.value、不发 change 的复位路径。
function _agendaDocMaterialEl(select) {
  const row = select?.closest?.('.wp-agenda-row');
  if (!row) return null;
  let el = row.querySelector('.wp-agenda-doc-material');
  if (!el) {
    el = document.createElement('p');
    el.className = 'wp-agenda-doc-material text-[11px] text-gray-500 mt-1 hidden';
    el.title = '过程材料（试点意见 / 征求意见 / 修改说明）随草案归档，本处只作数量提示';
    el.textContent = '已附材料 0 件';
    (row.querySelector('.wp-agenda-doc-hint') || select).insertAdjacentElement('afterend', el);
  }
  return el;
}

async function _syncAgendaDocMaterial(select) {
  const el = _agendaDocMaterialEl(select);
  if (!el) return;
  if (!select.value) { el.classList.add('hidden'); return; }
  try {
    // 经 branch-doc 服务读（与下拉同源；按 id 取单份，同 inspector.js 的单条解析口径）
    const doc = (await listBranchDocs()).find((d) => d.id === select.value);
    if (!doc) { el.classList.add('hidden'); return; }
    el.textContent = `已附材料 ${_draftMaterialCount(doc)} 件`;
    el.classList.remove('hidden');
  } catch (e) {
    console.warn('[calendar] 已附材料提示计算失败：', e);
    el.classList.add('hidden');
  }
}

function _syncAllAgendaDocMaterials() {
  document.querySelectorAll('#wp-agenda-list .wp-agenda-doc').forEach((s) => { _syncAgendaDocMaterial(s); });
}
...
async function _loadAgendaCandidateSources() {
  ...
  const [{ TaskForceRecordStore }, { IssueStore }, { loadDevStageOverrides }] = await Promise.all([
    import('../../../services/taskforce.js?v=20260922f'),
    import('../../../services/issues.js?v=20260922f'),
    import('../../../services/member-confirmation.js?v=20260922f'),
  ]);
  ...
  try {
    const docs = await listBranchDocs();
    out.draftDocs = docs.filter(isAgendaDraftDoc); // 同一判据（含制度草案 / 待党员大会表决，2026-09-21 批次 129）
