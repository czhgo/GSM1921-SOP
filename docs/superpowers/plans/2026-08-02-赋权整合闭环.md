...
| 2 | M1 核心 | `docs/src/services/auth.js`（三合一写穿：authorize/revoke/syncProjectRoles） |
...
| 6 | M3 | `mock.js`（todo 兜底条件）+ `ws-leader-entry.js`（创建内联赋权） |
...
```bash
git add docs/src/services/auth.js
git commit -m "feat(auth): authorize 三合一写穿主源，新增 syncProjectRoles/recordProjectGrants/recordProjectRevokes"
```
...
- [ ] **Step 4: `formHtml` 增加"活动角色"区块**

`docs/src/entries/ws-leader-entry.js` L481-484（描述 textarea 之后）插入：

```js
      <div class="mb-4">
        <label class="text-xs text-gray-500 mb-1.5 block font-medium">活动角色（选填，创建即赋权）</label>
...
- [ ] **Step 5: `_renderWriteContent` 渲染后初始化 PersonPicker**
...
```js
  ...
  if (dtOrgEl) {
    ...
    _dtOrgPicker = new PersonPicker({ mode: 'multi', placeholder: '选择组织者', accentColor: accent, onSelect: () => {} });
...
- [ ] **Step 6: 提交处理器组装 `assignments` + 顺路通知**
...
```js
    try {
      ...
      // T-190：创建时内联赋权（顺路产生）——组织者/深度参与者写入主源 assignments
      const organizerIds = _dtOrgPicker ? _dtOrgPicker.getSelected() : [];
      ...
      if (assignments.length > 0) activityData.assignments = assignments;

      const { activity } = await writeActivityWithSOP(activityData, scenarioId, targetDate);
...
> 注意：原代码解构 `const { taskCount } = await writeActivityWithSOP(...)` 需改为 `const { activity } = await ...`，`taskCount` 的引用保持不变（仍由解构对象提供）——请按上面完整替换 try 块开头，`taskCount` 之后的使用点（L632 `if (taskCount > 0)`）不变。
...
git commit -m "feat(leader): 创建活动内联赋权 organizer/deep，选人后不再派生赋权待办"
```
