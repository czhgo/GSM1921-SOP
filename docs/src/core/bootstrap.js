...
import { MilestoneStore } from '../services/milestones.js?v=20260914c';
import { CrossPageState } from './cross-page-state.js?v=20260914c';
import { getBasePath } from './utils.js?v=20260914c';
import { enhanceSelects } from '../components/custom-select.js?v=20260914c';
// 立项⑦ B波 演示放行门（单一源，与「进入支部（演示）」按钮同口径）
import { isPartyStaffBranchDemoAllowed } from '../modules/branch-demo-nav.js?v=20260914c';
// A② 归档兜底放行门（2026-09-10）：支书/副支书 archive=Y 兜底权限——可进入宣传台归档兜底面
import { isArchiveFallbackPage } from './constants.js?v=20260914c';
...
import { getAppliedAccentColors } from './theme.js?v=20260914c';
import { registerApiAdapter, init } from './data-adapter.js?v=20260914c';
import { ApiAdapter } from './api-adapter.js?v=20260914c';
import { getCapabilities } from './registry.js?v=20260914c';
// M4 数据源注册化：副作用导入触发 mock/api 数据源能力注册，bootstrap 经注册表选择数据源
import '../modules/capabilities/data-source.js?v=20260914c';
// M6（2026-08-30）：共享组件能力随全局引导注册（todo-list/calendar/custom-select），所有页面可发现组件清单
import '../modules/capabilities/components.js?v=20260914c';
