...
date: "2026-08-30"
version: "v18"
milestone: "测试基建规范化（17 测试文件）+ 部署文档 §五/§六 落地（mailer/report/定时）+ 权限收敛 S1-S10 + 最小三成本第 5 轮 + 插件化 M1-M4 + 考勤 UI 重设计 + 过程性脚本清理"
---
...
> **上版**: v17 (2026-08-11)
> **变更来源**: 最小三成本原则评议轮（2026-08-08~10）+ 书记工作台按人视图重构 + 减负空间 + 全栈同步（2026-08-10）+ 根目录文档更新（2026-08-10）+ 甲部 Harness 重组（2026-08-11）+ 2026-08-12~30 增量（见 v18 里程碑行）
...
├── CLAUDE.md                   ← 上下文入口（甲部 H10-H100 约束力三层 + 乙部执行 + 丙部待决策）
...
| 文件 | 状态 | 说明 |
|------|------|------|
| `CLAUDE.md` | ✅ | 上下文入口：甲部(H10-H100 约束力三层)+乙部(执行)+丙部(待决策) |
...
| `workspace/visitor.html` | 子页面 | 成员工作台（活动动态+专班进展+考勤+日历/列表切换+待办） |
...
| 理论 | 核心公式 | 详细文档 |
|------|---------|---------|
| 党建与党务工作 | 党建工作指党的自我建设（政/思/组/作/纪五建）；党务工作指党内事务的具体管理（党员发展/组织生活等）；两者都是"管理事，服务人" | USAGE_POLICY.md §1.1.1 + DATA_ARCHITECTURE.md |
...
| 参与者 | 成员工作台（个人视角：查看支委工作成果+个人考勤+待办） |
