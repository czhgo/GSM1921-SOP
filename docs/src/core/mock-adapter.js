...
import { mockDB, SCHEMA_VERSION } from './domain.js?v=20260811c';
import { generateId } from './id.js?v=20260811c';
...
import { ACTIVITIES } from '../mock/activities.js?v=20260811c';
import { SEED_TASKS, SEED_ASSIGNMENTS, SEED_ARCHIVE_RECORDS, SEED_SIGNUPS } from '../mock/seed.js?v=20260811c';
// Seed 增量合并用（2026-08-05）：attendance.js/notices.js 为纯数据模块，
// 经 services/person.js（只依赖 domain/people）→ 无指向本文件的循环依赖
import { ATTENDANCE_RECORDS } from '../mock/attendance.js?v=20260811c';
import { MOCK_NOTICES } from '../mock/notices.js?v=20260811c';
...
export const MockAdapter = {
  ...
  loadDB() {
    ...
    import('./data-adapter.js?v=20260811c').then(({ notifyDataLoaded }) => notifyDataLoaded())
      .catch(() => {});
  },
  ...
  activities: {
    ...
    create(data) {
      return _withDelay(() => {
        ...
        import('../services/todo.js?v=20260811c').then(({ LifecycleTodoDeriver }) => {
    ...
    archive(id) {
      return _withDelay(() => {
        ...
        import('../services/notice.js?v=20260811c').then(({ NoticeStore }) => {
  ...
  notices: {
    ...
    create(data) {
      return _withDelay(() => {
        ...
        _saveToStorage();
        import('../services/todo.js?v=20260811c').then(({ NoticeTodoDeriver }) => {
