...
import { setState, STATE, getAppState } from '../core/state.js?v=20260922j';
import { ROLE_COLORS, ROLE_LABELS, ROLE_THEME_CLASS, COMMISSIONER_ROLES, SECRETARY_ROLES, ACTIVITY_CLASSIFICATION } from '../core/constants.js?v=20260922j';
import { _fmtChinese, showToast, escHtml as esc } from '../core/utils.js?v=20260922j';
import { icon } from '../core/icons.js?v=20260922j';
import { openModal, closeModal } from './modal.js?v=20260922j';
// 数据域接线收口（2026-09-03）：支部成员名单经 services/person.js 获取（原直连 mock PEOPLE）
// 实时视图（非快照）：成员增删即时可见——见 services/person.js liveMembers 说明
const PEOPLE = liveMembers();
import { getPersonById } from '../services/person.js?v=20260922j';
import { BranchService } from '../services/runtime.js?v=20260922j';
import { AuthStore } from '../services/auth.js?v=20260922j';
import { liveMembers, PersonStore, getPersonName } from '../services/person.js?v=20260922j';
import { statusBadgeHtml, bindStatusBadge, badgeHtml } from './badges.js?v=20260922j';
import { persist, getAuthToken, getApiBaseUrl, getAdapter } from '../core/data-adapter.js?v=20260922j';
import { recordAgendaResultForActivity } from '../services/agenda-follow-up.js?v=20260922j';
// 制度链（2026-09-21 批次 129）：制度草案议程项在「记录结果」旁给一个「报送党员大会表决」勾选位——
// 判据单一源在 branch-doc.js（勿在界面另写一份 purpose/status 判断）。
import { isInstitutionDraftAgendaItem } from '../services/branch-doc.js?v=20260922j';
...
import { createEditableAgenda, normalizeEditedAgenda } from '../services/agenda-editing.js?v=20260922j';
// 议程更新后通知全员（活动锚定，targetType/targetId 供归档联动）
import { NoticeStore } from '../services/notice.js?v=20260922j';
import { fetchVotes, submitVote } from '../services/committee-vote.js?v=20260922j';
import { optionSetOf, resolveVoterIds, OPTION_SETS, isAnonymousActivity } from '../services/vote-config.js?v=20260922j';
import { renderVoteSummary } from './vote-summary-panel.js?v=20260922j';
import { loadAttendanceRecords } from '../services/attendance.js?v=20260922j';
import { loadInspectionRecords } from '../services/inspection.js?v=20260922j';
import { loadActivityReviews } from '../services/review.js?v=20260922j';
import { mockDB, OutputType, deriveOutputRoute, ReviewStatus, AttendanceStatus } from '../core/domain.js?v=20260922j';
...
export const ACTIVITY_LIFECYCLE = {
  ...
  pending_approval:{ label: '待批',   variant: 'warning' },
...
export function deriveActivityLifecycleStatus(activity, allTasks) {
  ...
  if (activity.status === PENDING_APPROVAL_STATUS) return 'pending_approval';
...
//  主题党日 → 支书关闭：
//    考勤确认 + 考察确认 + 复盘确认 + 宣传归档
//  专班 → 组织委员解散（另见 ws-org-commissioner-entry）：考察确认 + 工作量报告
...
export function checkActivityCloseConditions(activity) {
  ...
  // 其他活动类型无强制关闭前置
  if (reviewRequestOf(activity) && isReviewRequestEligibleActivity(activity) && !isReviewReturned(review)) missing.push('追加复盘（支委会要求）尚未交回'); // 2026-09-21 批次 135 · 裁定二「按推荐档落」：只判到「交回」不到「确认」；三会一课不适用
...
/** 草案已附材料件数（SOP-B-13 第 4 项 · D-305）：过程材料跟草案走（附件 / 网页正文各计 1 件），
 *  不新增「材料包」——议程项只做数量只读提示，材料本体仍归那份草案；导出供创建表单议程行「同款提示」复用（calendar-tab.js）。 */
export function _draftMaterialCount(doc) {
  return (doc && doc.fileName ? 1 : 0) + (doc && String(doc.bodyText || '').trim() ? 1 : 0);
}

/** 议程类型徽章（2026-09-01 支书点验链路 ①/②）：讨论文件 → 草案标题 ＋ 已附材料件数；待讨论名单 → 人数与目标阶段（S-2 只显目标） */
function _agendaTypeBadges(a) {
  ...
  if (isKind('discussion-file')) {
    ...
    // SOP-B-13 第 4 项（D-305）：草案还在时给一条只读提示，不提供第二个材料库入口
    if (doc) {
...
function renderInspectorDetail(activity, tasks, managementRole) {
  ...
  const isSecretary = !!_user && SECRETARY_ROLES.includes(_user.role); // 支书专属（单一源 constants.js::SECRETARY_ROLES）——活动信息编辑等仍归支书；品牌认定已归支委层（见下方 isCommittee）
  ...
  if (activity.status === PENDING_APPROVAL_STATUS && !isArchived) {
    ...
    const aprModeLabel = ACTIVITY_APPROVAL_MODE_LABELS[aprMode] || aprMode; // 档位标签单一源（policy-defaults；2026-09-22 批次 150）
    html += '<div class="mt-3 flex items-center gap-2 flex-wrap">';
  ...
  if (approveBtn) {
    approveBtn.addEventListener('click', async () => {
      if (!window.confirm('批准该活动？批准后即发布（全支部可见）。')) return;
      const res = approveActivity({ activityId: activity.id, by: currentUserId, role: _user?.role, mode: activityApprovalMode() });
      if (!res.ok) { showToast('error', res.reason || '批准失败'); return; }
      showToast('success', '已批准发布');
      await reloadAfterBrandWrite();
    });
  }
  const rejectBtn = document.getElementById('inspector-reject-btn');
  if (rejectBtn) {
    rejectBtn.addEventListener('click', async () => {
      const note = window.prompt('不批准该活动（将终止）。可填写退回意见：', '');
      if (note === null) return;
      const res = rejectActivity({ activityId: activity.id, by: currentUserId, role: _user?.role, mode: activityApprovalMode(), note });
      if (!res.ok) { showToast('error', res.reason || '驳回失败'); return; }
      showToast('success', '未批准，该活动已终止（已取消）');
      await reloadAfterBrandWrite();
    });
  }
  ...
  const renderEditor = () => {
    ...
    block.querySelector('#agenda-edit-save').addEventListener('click', async () => {
      ...
      try {
        ...
        // R-22（2026-09-13）：系统派生通知改由服务端按 kind 生成（不再前端自述可信标记）
        try {
          NoticeStore.addSystem('activity-agenda-updated', activity.id, { activityTitle: activity.title });
        } catch (ne) {
