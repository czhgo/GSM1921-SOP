...
test('AV5 线上党员大会：书记发起 → 预备党员只读/403 → 正式党员 UI 表决 → 硬校验拦截 → 补足后通过', async () => {
  ...
  const created = await secPage.evaluate(async ({ title, voterIds, ag1, ag2 }) => {
    const { getAdapter } = await import('/src/core/data-adapter.js?v=20260901m');
    ...
    const { mockDB } = await import('/src/core/domain.js?v=20260901m');
  ...
  await prepPage.evaluate(async () => {
    const { persist } = await import('/src/core/data-adapter.js?v=20260901m');
  ...
  const p5Row = await memberPage.evaluate(async ({ activityId, ag1 }) => {
    ...
    const { enableApiMode } = await import('/src/services/runtime.js?v=20260901m');
    enableApiMode(token);
    const { submitVote } = await import('/src/services/committee-vote.js?v=20260901m');
  ...
  try {
    ...
    await secPage.evaluate(async ({ id }) => {
      const { setState } = await import('/src/core/state.js?v=20260901m');
      const { mockDB } = await import('/src/core/domain.js?v=20260901m');
