...
| D 表述 | 内容概括与文件实际内容不符 | 书记对照原文判定（先例：「27 条路线级论断」→「27 条论断，P-045/P-047 为元命题引子」） | 按书记统一改法 |
...
| T-207 | **全仓减量第二波续·剩余文件全面过一遍**：书记指令（2026-08-04）："剩下的文件都要全面地过一遍"。对第二波已瘦身文件之外的全部 content/ .md 逐一过一遍（含 SERVICE_CATALOG 18k / MODULE_UI_DESIGN 17k / COMMISSIONER_FRAMEWORK 16.5k / 常见工作场景快速指南 15k / 党支部管理与实务经验沉淀 12k / CHECKLIST / SSOT_INDEX 等），按 H5.7 文件瘦身原则执行语义瘦身，逐批提交书记审议 | H5.7 文件瘦身 + H4.2 丙部待决策机制 | content/ 全部 .md | ⏳ 待启动（书记推进 UI 工作后优先） |
