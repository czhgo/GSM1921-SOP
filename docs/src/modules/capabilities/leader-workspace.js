...
registerCapability({
  ...
  tabs: () => [
    ...
    { id: 'todo', label: '待办', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/leader/todo-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    { id: 'overview', label: '工作概况', groupLabel: '工作台', coreTab: true, render: (ctx) => import('../../entries/tabs/leader/overview-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    // 分组「我的职责」= 2026-09-14 裁定按行为性质四组之一；本轮（2026-09-15）五台统一：原「党建」组改名「我的职责」
    { id: 'write', label: '活动管理', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/leader/write-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    { id: 'attendance', label: '考勤上传', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/leader/attendance-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    { id: 'inspection', label: '考察上传', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/leader/inspection-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    ...
    { id: 'members', label: '组员进展', groupLabel: '我的职责', render: (ctx) => import('../../entries/tabs/leader/members-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    ...
    { id: 'tf-view', label: '知情查看', groupLabel: '知情查看', render: (ctx) => import('../../entries/tabs/leader/tf-view-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
    // 本轮（2026-09-15）五台统一：原「反馈」组并入「制度与答复」（与支书台同名），组内以「我的处置」为成员
    { id: 'my-dispatch', label: '我的处置', groupLabel: '制度与答复', render: (ctx) => import('../../entries/tabs/leader/my-dispatch-tab.js?v=20260915g').then(m => m.renderContent(ctx)) },
  ],
});
