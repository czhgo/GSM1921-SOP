...
// - 关键服务调用均在页面 evaluate 内以 ?v=20260914g 导入（与页面共享模块实例，防 mockDB 双实例分裂）。
...
test('AV5 线上党员大会：支书发起 → 预备党员只读/403 → 正式党员 UI 表决 → 硬校验拦截 → 补足后通过', async () => {
  ...
  await prepPage.evaluate(async () => {
    const { persist } = await import('/src/core/data-adapter.js?v=20260914g');
  ...
  const p5Row = await memberPage.evaluate(async ({ activityId, ag1 }) => {
    ...
    const { enableApiMode } = await import('/src/services/runtime.js?v=20260914g');
    enableApiMode(token);
  ...
  try {
    ...
    await secPage.evaluate(async ({ id }) => {
      const { setState } = await import('/src/core/state.js?v=20260914g');
      const { mockDB } = await import('/src/core/domain.js?v=20260914g');
