...
全局概况、活动管理（会务日历）、支部分工（工作地图，平铺/按人）、赋权管理、通知发布、专班查看（知情权：无职责≠无知情权）、党小组进展（跨组切换只读掌握·组内待答复可「请组长关注」，支书不代组长答复）、反馈管理、上报党委（见 3.5）。待办含「待答复」「专班待议」置顶入口。支部治理类操作（换组织向导 / 模块组合 / 工作台默认顺序等）已从工作台迁出，统一收口于侧边栏右下角「设置 → 支部治理」（支书 / 副支书共台同权，见 3.1 设置中心说明）。
