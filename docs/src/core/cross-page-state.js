 ...
 * 自检逻辑见 bootstrapPage()。
 */
const CODE_VERSION = 169; // 2026-09-10/11 收口批（活动写入 S-1~S-4 + 卡片普查合并 + 催办/兜底白名单 + 归属时序修复 + 门面通用化 + 只读端对齐 + 对比度 AA 收口 + 功能实现评议 8 项 + R-6~R-11 闭环 + 最小三成本第 6 轮 6 条 + 门面口径统一 + 深色三批收口 + 拖拽机测 + 成员写链语义端点与副书同权）+ 全站 ?v=20260911a
