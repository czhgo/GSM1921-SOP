...
import { dutyCardHtml } from './workforce-duty-card.js?v=20260920g';
import { TodoStore, seedTodos, TodoStatus } from '../services/todo.js?v=20260920g';
import { IssueStore } from '../services/issues.js?v=20260920g';
import { AuthStore } from '../services/auth.js?v=20260920g';
import { solidAccentStyle, dotDarkVars, isActivityEnded, isActivityArchived } from '../core/constants.js?v=20260920g';
...
import { loadInspectionRecords, getOverdueRecords } from '../services/inspection.js?v=20260920g';
// S3③（2026-09-12）：补课口径统一——概况补课缺口与「补课制度」表同源（services/makeup.js）
import { loadMakeupTasks } from '../services/makeup.js?v=20260920g';
import { TaskForceRecordStore } from '../services/taskforce.js?v=20260920g';
import { listPendingByReceiver, confirmExternalDispatch } from '../services/external-dispatch.js?v=20260920g';
...
async function _renderOverviewDetail(container, detail, accent, onBack) {
  ...
  if (detail.kind === 'activity') {
    const { renderActivityView } = await import('./activity-view.js?v=20260920g');
    renderActivityView(host, { highlightId: detail.id, accent });
  } else {
    const { renderTaskforceView } = await import('./taskforce-view.js?v=20260920g');
