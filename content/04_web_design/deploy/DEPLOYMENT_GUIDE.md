...
> **定位**：本文件是系统的「对外总案」+「部署落地唯一权威源」两用文档（2026-09-04 支书裁决 D8：deploy/ 按「阅读对象」组织——需要一个相对完整的文件去和学校/党校交涉对接，本文件即为此而设，自包含、可直接拿去讲）。
>
...
> - **对内的读者**（支书/工程师/AI/未来接手者）：部署路径决策与对外诉求以本文件为唯一权威源；专项设计细节（认证模型 / 微信协同与小程序 / 北大党校与智慧党建对接）各自归位对应专项文件，本文件 **四** 为专项细节指针（各留 1-3 行结论摘要），落地实施细节（代码就绪度 / 真实部署 checklist / 风险）见**附录**。
>
...
**一句话给三方**：给支部成员——一套真实组织的数字化工作台；给支书/支委——管理事、服务人、对外迎检汇报的台账底座；给学校（计算中心/组织部/党校）——一个**数据边界清晰、可对接可交付**的支部侧系统（详细诉求见 §三）。
...
> **本章是全篇重点，自包含**——对接对象、我方诉求、系统能力、数据流边界（数据从哪来到哪去、与外部系统单向/双向边界）、前置条件、对接步骤、对外可交付内容，全部在本章内讲清，支书不必再拼装。专项设计细节（认证门控 / 微信小程序 / 北大爬取与同步机制）只在此留摘要，细节指针见 §四。
...
> 登录门控（谁能进页面/谁能写/谁能下载）的四层模型见 [AUTHENTICATION_MODEL.md](AUTHENTICATION_MODEL.md)（§四 摘要见本文件 §4.1——对外读者可跳过，§4 为对内专项指针章）。
...
| 迎检外链 / 归档包 | 人工（出） | 支书生成临时查看链接 / 导出归档包面向上级审查 | [WECHAT_INTEGRATION.md](WECHAT_INTEGRATION.md) §3.2 |
...
| 项 | 说明 | 责任方 |
|----|------|--------|
| 域名 + ICP 备案 | 正式上线必需的 Web 入口 | 支书 / 计算中心 |
...
> 本节是「支书去交涉时能拿出什么」的清单——演示怎么给、文档给哪些、数据出口在哪。
...
- **迎检**：支书可生成临时查看链接（可设有效期）/ 导出归档包，面向上级审查（宣传墙 + 档案分层见 [WECHAT_INTEGRATION.md](WECHAT_INTEGRATION.md) §3）。

### 3.8 邮件发送通道：对接学校邮件系统（通用 SMTP）

> **定位**：本系统与学校邮件系统之间的发送通道（2026-08-29 支书大尺度更新新增）。本系统只做「发送」，邮箱由学校/支部提供 SMTP 服务。**读者对象：计算中心对接人**——本节自包含，不需要本项目任何上下文。
...
| 场景 | 触发点 | 收件人 |
|------|--------|--------|
| ① 通知发布 | 支书/副支书在「通知发布」发布通知 | 通知受众（全体/党小组/指定角色） |
| ② 待办提醒 | 任何新待办生成（审批/报名/考察/交接） | 待办责任人 |
| ③ 反馈/汇报 | 意见反馈提交、一键汇报发出 | 汇报对象（支书/纪检等） |
...
- [ ] 验证：支书发布一条通知 → 目标收件人邮箱收到邮件（正文含通知内容 + 直达链接）
...
> **定位**：本系统产生的支部数据（党员/活动/考勤/学习）**按日批量上报**至学校智慧党建平台 / 党校系统，供平台收录与统计（2026-08-29 支书大尺度更新新增）。**读者对象：计算中心对接人**——本节自包含。
