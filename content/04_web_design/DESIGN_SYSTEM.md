...
| 深金文字 | `#B45309` | ██ | 金浅底/金底上的文字（对比度 4.9:1 达标） |
...
| `--accent-participant` | `#B45309`（三件套=主题党日胶囊） | ██ | 普通参与者 | 金色系（深金） | 「金」主题色 = 主题党日胶囊：亮金底 `rgba(255,215,0,0.12)` + 深金字 `#B45309` + 亮金边框 `rgba(255,215,0,0.35)`（2026-08-08 四审改，书记原话"金色就该和主题党日胶囊一致"） |
...
| 品牌·党徽金 | `#FFD700`（配套：深金文字 `#B45309` / 浅底 `rgba(255,215,0,0.12)` / 边框 `rgba(255,215,0,0.35)`） | 强调卡边框、主题党日、项目分工标签、考察成长元素、待办行动按钮 |
| 主题色 X | `getAccentColors(role)`（participant「金」默认 = 主题党日胶囊三件套：亮金底 `rgba(255,215,0,0.12)` + 深金字 `#B45309` + 亮金边框 `rgba(255,215,0,0.35)`，2026-08-08 四审改；见 §2.3.2） | **tab 激活态**（浅底 + 边框 + X 文字，2026-08-08 三审起回归主题色）、主按钮、标题装饰条、主要标签、选中项、chips |
...
- **「金」主题色 = 主题党日胶囊**（书记 2026-08-08 四审定稿，原话"金色就应该和主题党日的胶囊颜色一致，那个我觉得很漂亮！"）：`ACCENT_COLORS.participant` 增配 `bg/border` 覆盖字段（亮金底 `rgba(255,215,0,0.12)` + 深金字 `#B45309` + 亮金边框 `rgba(255,215,0,0.35)`），`getAccentColors` 优先消费覆盖；侧边栏色板「金」色块显示亮金 `#FFD700`。文字色仍守深金 `#B45309` 保对比度，金不再因对比度牵制而变色。
...
| 3 | 党徽金（`--party-gold`） | 不反白、不荧光，保持庄重；金底文字一律深金 `#B45309`（延续 §2.7 金点描边约定） |
