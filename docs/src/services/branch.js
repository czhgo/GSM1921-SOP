...
import { PARTY_COMMITTEE } from '../mock/branches.js?v=20260913f';
import { getAdapter, persist, getDataSource } from '../core/data-adapter.js?v=20260913f';
import { listCapabilities } from '../core/registry.js?v=20260913f';
// P1a 单向权威（2026-09-03）：config 净化唯一实现 = core/config-clean.js（server PATCH /branches/:id/config 同源）
import { sanitizeConfigBlocks, sanitizeConfigModules, sanitizeConfigWorkforce, sanitizeConfigOrg, sanitizeConfigPolicyOverrides, applyBranchPolicyOverrides } from '../core/config-clean.js?v=20260913f';
// 审计内核共享常量（2026-09-09 支书批）：why 透传/单键回滚白名单/历史上限单一源 = config-clean
// （server resources.js 同源 import，双形态防失同步）
import { CONFIG_HISTORY_MAX, CONFIG_ROLLBACK_WHAT, CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260913f';
...
export async function rollbackBranchConfig(branchId, { by = null, targetEntryAt, index } = {}, why) {
  ...
  if (getDataSource() === 'api') {
    try {
      const { ApiAdapter } = await import('../core/api-adapter.js?v=20260913f');
