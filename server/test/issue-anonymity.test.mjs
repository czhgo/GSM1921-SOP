// role: [工程师]+[AI]
// issue-anonymity.test.mjs — 意见反馈「真匿名」验收（2026-09-12 书记裁定）
// 覆盖：①匿名提交后任何接口/存储（含书记视角）都取不到提交人 ②实名按现口径记 personId
//      ③tokenHash 不可反查（不含 personId / 原始 token） ④处置结果公开可见（处置权仅书记）
...
import { hashSubmitterToken } from '../../docs/src/core/constants.js?v=20260912k';
...
test('API ①匿名提交：落库/接口（含书记视角）均取不到提交人', async () => {
  ...
  assert.ok(got, '书记可见该反馈');
  for (const k of IDENTITY_KEYS) assert.ok(!(k in got), `书记视角不得含身份字段 ${k}`);
...
test('API ④处置结果公开可见；处置/回复权仅书记', async () => {
  const member = await login('p5');
  const sec = await login('p13');
  const created = await (await postIssue(member, {
...
test('mock 形态：匿名提交后存储/书记端界面均无提交人；实名可见；tokenHash 前后端同源', async () => {
  const browser = await chromium.launch({ headless: true });
  try {
    const page = await browser.newPage();
    await page.goto(`${base}/index.html`, { waitUntil: 'domcontentloaded', timeout: 15000 });
    const out = await page.evaluate(async () => {
      const { IssueStore } = await import('/src/services/issues.js?v=20260912k');
      const { PersonStore } = await import('/src/services/person.js?v=20260912k');
      localStorage.clear();
      ...
      // 书记视角：渲染反馈管理 tab，打开匿名条目的详情面板
      loginAs('p13', 'secretary');
      const tc = document.createElement('div');
      tc.id = 'secretary-tab-content';
      document.body.appendChild(tc);
      const fb = await import('/src/entries/tabs/secretary/feedback-tab.js?v=20260912k');
      fb.renderContent();
    ...
    assert.equal(draftAnon.author, '匿名', '匿名草稿作者不得为真实 personId（书记草稿区亦不可追溯）');

    // 实名：按现口径
    assert.equal(out.real.submittedBy, 'p5');
    assert.equal(out.real.anonymous, false);

    // 书记端界面：匿名条目与详情均不显示真实提交人姓名/身份
    assert.ok(out.p5Name && out.p5Name !== 'p5', '演示成员 p5 可解析为姓名');
    assert.ok(!out.listRowHtml.includes(out.p5Name), '书记端列表行不得出现匿名提交人姓名');
    assert.ok(out.listRowHtml.includes('匿名'), '书记端列表行应显示「匿名」');
    assert.ok(!out.detailHtml.includes(out.p5Name), '书记端详情不得出现匿名提交人姓名');
    assert.ok(!out.detailHtml.includes('真实提交人'), '「真实提交人（仅书记可见）」机制已移除');
    assert.ok(out.detailHtml.includes('提交人：匿名'), '书记端详情提交人显示匿名');
  } finally {
    await browser.close();
  }
});
