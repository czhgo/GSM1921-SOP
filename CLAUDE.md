...
| ID | 事项 | 引用流程 | 状态 |
| --- | --- | --- | --- |
| R-23 | **遗留（R-22 收口后残留）**：① 思想汇报在服务端无表 → `thought-report-submitted` 只能按「提交人本人」判定（建议为 thoughtReports 建表后收紧为「提交人或有权阅处角色」）；② 部分 kind 的展示值（活动名/姓名等）仍取自 payload（非安全字段，落地值已由服务端掌控）；③ 通知草稿类（活动/专班/党委下发）的标题正文由发布人填、服务端仅掌控受众与落点 | SYSTEM_ROLE_PERMISSION §9h + 附录⑦ 写权判据 | 待办 |
