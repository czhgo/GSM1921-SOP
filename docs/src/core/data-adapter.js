...
export async function init() {
  ...
  if (DATA_SOURCE === 'mock') {
    ...
    try {
      ...
      const { mockDB } = await import('./domain.js?v=20260912b');
      ...
      try {
        ...
        try {
          const { restoreNicheCollections } = await import('./mock-adapter.js?v=20260912b');
      ...
      if (!mockDB.attendances.length || !mockDB.inspections.length) {
        try {
          const { ATTENDANCE_RECORDS } = await import('../mock/attendance.js?v=20260912b');
          const { INSPECTION_RECORDS } = await import('../mock/inspection.js?v=20260912b');
      ...
      if (!mockDB.todos.length) {
        try {
          const { SEED_TODOS } = await import('../services/todo.js?v=20260912b');
