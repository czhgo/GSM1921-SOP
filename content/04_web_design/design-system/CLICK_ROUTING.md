...
2. **纯展示不加 hover 伪装**（COMPONENT_SPEC §4.2 硬性规则 4）：纯展示行/卡不得有 hover 上浮/变底（已清理：首页统计卡前三张、支书考勤明细行）。
...
- 已治理：未登录首页点击 `user.role` 空引用 TypeError（main-entry.js 判空，T-304 方案 B）；首页统计卡 hover 伪装（stats.js）；支书考勤明细行 hover 伪装（calendar-tab）。
