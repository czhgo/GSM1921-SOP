...
| ID | 事项 | 引用流程 | 状态 |
| --- | --- | --- | --- |
| R-16 | **遗留（台账行可点残余）**：组长「考勤/考察上传」、纪检「考察总表」等**台账行**仍不可点——主实体＝人×活动记录且无详情页，需在服务层 `inspectionToLong` 暴露 `activityId` 后方可补链（超出「最小入口」范围） | 附录⑦ 操作入口判据（≤2 跳 + 卡内实体条目必须可点） | 待办 |
| R-17 | **待裁（无记名延伸范围）**：专班表决 / 支部分工等 **deliberative 流程**是否也纳入无记名（现各自默认记名、未联动 tally）；书记裁定后再实施 | 附录⑦ 匿名判据 | 待书记裁 |
| R-18 | **遗留（组长台账明细无筛选）**：组长「考勤上传」「考察上传」明细表 40~53 行平铺，无活动筛选/分组，长列表查找成本高（dogfood 组长视角） | H10 最小三成本（信息成本）+ 附录⑦ 查找成本 | `docs/src/entries/tabs/leader/` | 待办 |
