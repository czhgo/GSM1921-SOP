...
test('线上支委会：支书发起→委员表态→支书截止→通知闭环→API 校验', async () => {
  ...
  const created = await secPage.evaluate(async () => {
    const { getAdapter } = await import('/src/core/data-adapter.js?v=20260921p');
  ...
  const { vote, notified } = await orgPage.evaluate(async ({ activityId }) => {
    ...
    const { submitVote } = await import('/src/services/committee-vote.js?v=20260921p');
  ...
  const { locked, reminded } = await secPage.evaluate(async ({ activityId }) => {
    const { lockVotes } = await import('/src/services/committee-vote.js?v=20260921p');
