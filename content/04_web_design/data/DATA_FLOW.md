...
#### 4.5.1 成员流入/流出写路径（2026-09-14 批次 25；撤销流出补于批次 29、流入端点分流补于批次 30）

> **成员流入 → 台账 + 档案 + 账号（三写同源）**：一次登记三处一致——`memberFlows` 记一笔（`direction='in'`）+ `PersonRecord` 建档（采集姓名/学号/届别/党小组）+ 账号层建号（账号 = 学号，口令 = 支部统一默认口令）。**API 形态走专用端点** `POST /members/intake`（写门 = 组织委员 + 支书/副支书，与 §9i 同源；接线选项 `saveMember(record, { memberFlowIntake: true })`）——名册 tab 的「新增成员」仍走 `POST /members`（R-10 组织委员专属），两者是**同一实现体、两个写门**（2026-09-14 批次 30 支书裁定 Q-23-10：维持 §9i，不放宽名册越权面）。
...
### 4.7 视图按需取用原则与判例（承接原 insights §4.9 + §6.13，2026-09-04 分流）

> **承接说明**：2026-09-04 自 content/insights/工程演进与设计方法论.md 分流——§4.9（原则陈述版本）+ §6.13（D-205 判例版本）同置。核对原母本指针（DATA_FLOW §三）：本文档此前无该原则展开（数据架构总文件 2026-08-24/2026-09-03 拆分精简后未迁入），故本次按"补入"而非"补强"处理。出处注记：（原 insights §4.9 / §6.13）。
...
> **元数据**
>
> - 出处：D-205 + 原 insights §6.13（判例，见 4.7.2）+ CLAUDE.md H90（外部权威源索引）
> - 母本位置：本文件 §4.7（2026-09-04 承接补入）
> - 判例位置：本文件 4.7.2（原 insights §6.13）
> - 索引位置：✅ CLAUDE.md H90（外部权威源索引）

#### 4.7.2 判例：视图按需取用三条原则（D-205）（原 insights §6.13）

> **与 4.7.1 的关系**：4.7.1 是原则陈述版本，本节是判例版本（D-205 判例）。两者互为补充——4.7.1 给出原则，本节给出落地判例。
...
### 4.8 看板数据必须从正式数据源动态派生（H-1 判例，承接原 insights §6.14，2026-09-04 分流）

> **承接说明**：2026-09-04 自 content/insights/工程演进与设计方法论.md §6.14 分流（与 4.7 视图按需取用同置——均属"视图层数据来源"规范）。出处注记：（原 insights §6.14）。
