...
import { ROLE_LABELS, ROLE_PAGE_MAP, getAccentColors } from '../core/constants.js?v=20260911a';
import { resolveAppliedAccentRole } from '../core/theme.js?v=20260911a';
import { appearanceControlsHTML, bindAppearanceControls } from '../components/appearance-controls.js?v=20260911a';
...
} from '../services/preferences.js?v=20260911a';
...
import { registerApiAdapter, setDataSource, init as dataInit } from '../core/data-adapter.js?v=20260911a';
import { ApiAdapter } from '../core/api-adapter.js?v=20260911a';

// 2026-09-09 支部归属显式化/审计内核：变更记录展示与回滚按钮需要操作者姓名、单键回滚白名单
import { getPersonName } from '../services/person.js?v=20260911a';
import { CONFIG_ROLLBACK_KEYS } from '../core/config-clean.js?v=20260911a';
...
async function buildMyWorkspaceModel(role, personId) {
  ...
  const { applyTabPolicy, getBranchIdOfPerson } = await import('../services/branch.js?v=20260911a');
...
async function renderBranchGovSection(panel, sectionId) {
  ...
  try {
    ...
    if (sectionId === 'branch-info-wizard') {
      ...
      await import('../modules/capabilities/secretary-workspace.js?v=20260911a');
...
async function openWizardEmbed(panel) {
  ...
  try {
    const { mountOrgSetupWizard } = await import('../components/org-setup-wizard.js?v=20260911a');
