...
function _renderTab(tab) {
  ...
  if (tab === 'archives') {
    ...
    tc.innerHTML = `<div class="card rounded-xl p-5 border-l-4" style="border-left-color:var(--accent-prop-commissioner-light);"><h4 class="font-title-cn text-sm font-bold text-gray-700 mb-3">图片管理</h4><div class="space-y-4"><div class="p-4 bg-gray-50 rounded-lg"><h5 class="text-xs font-semibold text-gray-600 mb-2">上传图片 + 标注</h5><div class="grid grid-cols-1 md:grid-cols-2 gap-3"><div><label class="block text-xs text-gray-500 mb-1">选择图片</label><input type="file" id="pub-image-file" accept="image/*" class="text-xs w-full" /></div><div><label class="block text-xs text-gray-500 mb-1">拍摄日期</label><input type="date" id="pub-image-date" value="${today}" class="text-xs w-full border border-gray-300 rounded px-2 py-1" /></div><div><label class="block text-xs text-gray-500 mb-1">图片标题</label><input type="text" id="pub-image-title" placeholder="如：3月主题党日全景" class="text-xs w-full border border-gray-300 rounded px-2 py-1" /></div><div><label class="block text-xs text-gray-500 mb-1">拍摄主体</label><input type="text" id="pub-image-subject" placeholder="如：人物/场景/物件" class="text-xs w-full border border-gray-300 rounded px-2 py-1" /></div><div class="md:col-span-2"><label class="block text-xs text-gray-500 mb-1">关联活动（可选）</label><select id="pub-image-activity" class="text-xs w-full border border-gray-300 rounded px-2 py-1"><option value="">— 不关联 —</option>${activityOptions}</select></div></div><button id="pub-image-upload-btn" class="btn-primary mt-3" style="padding:6px 16px;font-size:0.75rem;">上传并保存</button><span id="pub-image-msg" class="text-xs text-gray-400 ml-2"></span></div><div><h5 class="text-xs font-semibold text-gray-600 mb-2">照片墙</h5><div id="prop-commissioner-image-gallery"><p class="text-xs text-gray-400">数据加载中...</p></div></div></div></div>`;
