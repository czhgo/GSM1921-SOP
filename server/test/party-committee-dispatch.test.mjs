...
// 验收（设计 §5 P3 双向通道下发半侧 + 支书裁定「复用通知」）：
//   ① 党委组织员在党委工作台「下发通知」选支部下发 → 下发历史即时可见
//   ② 目标支部的支委层成员（支书储子禾 p13）在通知铃铛看到该条（标「党委下发」）
...
test('P3 党委下发：支委层收件可见（支书），党委自身/普通成员不可见', async () => {
